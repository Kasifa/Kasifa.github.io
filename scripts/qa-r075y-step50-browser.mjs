...
const outputRoot = resolve(process.env.R075Y_STEP50_BROWSER_QA_OUTPUT ?? "/tmp/r075y-step50-browser-qa");
const externalBase = process.env.R075Y_PUBLIC_BASE_URL;
...
const target = {
  id: "note",
  path: "/notes/r0-75y.html",
  h1Zh: /强分离多谐波族的完整 signed-flux 付款/,
  h1En: /Complete signed-flux payment for strongly separated multimode families/i,
};
const results = [];
...
try {
  for (const scenario of scenarios) {
    ...
    url.searchParams.set("qa", "r075y-step50");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        step50MathJaxErrors: document.querySelectorAll("#s-393 mjx-merror, #s-394 mjx-merror, #s-395 mjx-merror, #s-396 mjx-merror, #s-397 mjx-merror, #s-398 mjx-merror, #s-399 mjx-merror, #s-400 mjx-merror").length,
        ...
        figureSections: document.querySelectorAll('#figure, [src*="assets/r075y"], [href*="assets/r075y"]').length,
        stronglySeparated: /STRONGLY SEPARATED|strongly separated|强分离/i.test(visibleText),
        qSquared: /EXPLICIT q\^2 COST|explicit[^\n]{0,80}q\^2|显式[^\n]{0,80}q\^2/i.test(visibleText),
        clustersOpen: /CLUSTERS OPEN|unresolved[^\n]{0,100}clusters[^\n]{0,80}(?:OPEN|open)|簇拥[^\n]{0,100}开放/i.test(visibleText),
    ...
    assert.equal(audit.step50MathJaxErrors, 0, `${id} Step 50 MathJax errors`);
    ...
    assert.equal(audit.figureSections, 0, `${id} no R0.75Y formal figure`);
    for (const key of ["stronglySeparated", "qSquared", "clustersOpen", "versionM", "noveltyBoundary", "clayBoundary"])
