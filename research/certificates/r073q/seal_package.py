#!/usr/bin/env python3
"""Create or verify the two-stage provenance seal for R0.73Q."""
...
def run_git(arguments: list[str], binary: bool = False) -> str | bytes:
    ...
    require(
        ...
        "git command failed: git " + " ".join(arguments) + ": " + completed.stderr.decode("utf-8", "replace").strip(),
    )
...
def build_manifest(source_commit: str | None) -> dict[str, object]:
    ...
    return {
        ...
        "inventory": inventory,
        "release": "R0.73Q",
        "schemaVersion": "r073q-finite-heat-flow-manifest-v1",
