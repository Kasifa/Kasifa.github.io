...
// Render and cryptographically bind the complete Chinese R0.74S Step 17 note
// and its one-off major route-correction recap. Bindings certify publication
// bytes and PDF structure, not mathematical correctness.
...
try {
  ...
  const title = "R0.74S｜闭流线复现否定所有次线性绝对时间尾";
  ...
  const binding = {
    schemaVersion: "r074s-step17-note-synchronized-pdf-binding-v1",
    release: "R0.74S",
    kind: "note",
    ...
    claimBoundary: {
      ...
      criticalP1Scaling: "ABSOLUTE_TEMPORAL_VARIATION_ASYMPTOTIC_A_CUBED_PAYMENT_ASYMPTOTIC_A_CUBED",
      criticalEndpoint: "FALSE_S444_BY_RECURRENT_CLOSED_STREAMLINE",
      recurrentClosedStreamline: "PROVED_REGULAR_LEVEL_SET_OF_SAME_TAYLOR_SMOOTH_EXACT_NSE",
      absoluteTemporalTail: "FALSE_FOR_ALL_P_AT_LEAST_ONE_AND_ALL_BETA_BELOW_ONE",
      completePayment: "ASYMPTOTIC_A_CUBED",
      absoluteTemporalVariation: "ASYMPTOTIC_A_CUBED",
      signedPositiveExcursion: "ASYMPTOTIC_A_SQUARED",
      s472FixedDeletionPositiveExcursion: "OPEN",
      directHybridTerminalFluxGate: "OPEN_NOT_REFUTED",
      q12: "OPEN",
      fixedScaleInequality: "OPEN_Q1",
      primaryFiniteCertificate: "PASS_12_GROUPS_4325_CASES_11_STRUCTURAL_2_DEPENDENCIES",
      independentRubyCertificate: "PASS_7_GROUPS_294_ASSERTIONS_4_LOCKS_20_SEMANTIC_32_NEGATIVE_3_PATHS_14_REPRODUCIBILITY",
      continuumPaymentMachineProved: false,
      noveltyOrPriorityClaimed: false,
      formalFigure: "STEP17_RECURRENT_CLOSED_STREAMLINE_FOUR_PANEL_ANALYTIC_EXACT_FIELD_NOT_SIMULATION_OR_DNS",
  ...
  const noteResult = { kind: "note", pageCount: structure.pageCount, sha256: sha256(pdf), bytes: pdf.length };

  const recapHtmlRelative = "public/recap-r0-61-r0-74s.html";
  const recapPdfRelative = "public/recap-r0-61-r0-74s.pdf";
  const recapProvenanceRelative = "research/r074s_recap_pdf_render.json";
  const recapBindingRelative = "research/r074s_recap_pdf_bindings.json";
  const recapUrl = `http://127.0.0.1:${address.port}/recap-r0-61-r0-74s.html?lang=zh`;
  await run(process.execPath, ["scripts/render-note-pdf.mjs", recapUrl, recapPdfRelative, "-", recapHtmlRelative, recapProvenanceRelative], { env: { PDF_RENDER_ROOT: root, PDF_PUBLIC_ORIGIN: "https://kasifa.github.io" } });
  const [recapHtml, recapPdf, recapProvenanceBytes] = await Promise.all([
    readFile(resolve(root, recapHtmlRelative)),
    readFile(resolve(root, recapPdfRelative)),
    readFile(resolve(root, recapProvenanceRelative)),
  ]);
  const recapStructure = inspectPdf(recapPdf, recapPdfRelative);
  const recapTitle = "R0.61–R0.74S 重大路线修正回顾｜从 scalar no-go 到 signed endpoint functional";
  if (recapStructure.title !== recapTitle) throw new Error(`recap PDF title drift: ${recapStructure.title}`);
  const recapProvenance = JSON.parse(recapProvenanceBytes);
  if (recapProvenance.loadedDocument?.equalsSourceHtml !== true || recapProvenance.loadedDocument?.sha256 !== sha256(recapHtml) || recapProvenance.source?.publicOrigin !== "https://kasifa.github.io") throw new Error("recap render provenance mismatch");
  const recapBinding = {
    schemaVersion: "r074s-step17-recap-synchronized-pdf-binding-v1",
    release: "R0.74S",
    kind: "recap",
    publicChineseHtml: { path: recapHtmlRelative, bytes: recapHtml.length, sha256: sha256(recapHtml) },
    publicPdf: { path: recapPdfRelative, bytes: recapPdf.length, sha256: sha256(recapPdf), pageCount: recapStructure.pageCount, title: recapStructure.title, structure: recapStructure },
    provenance: { path: recapProvenanceRelative, bytes: recapProvenanceBytes.length, sha256: sha256(recapProvenanceBytes), sourceUrl: recapProvenance.source.url, loadedMainDocumentEqualsSourceHtml: true },
    claimBoundary: {
      majorRouteCorrectionRecap: true,
      step16SpecialSeparatrixAbsoluteVariation: "ASYMPTOTIC_A_SQUARED",
      step17RecurrentClosedStreamlineAbsoluteVariation: "ASYMPTOTIC_A_CUBED",
      target: "FIXED_DELETION_HYBRID_OR_SIMULTANEOUS_HEIGHT_SIGNED_FUNCTIONAL",
      q12: "OPEN",
      q1: "OPEN",
      s407: "OPEN",
      s472: "OPEN",
  ...
  await writeFile(resolve(root, recapBindingRelative), `${JSON.stringify(recapBinding, null, 2)}\n`);
  const recapResult = { kind: "recap", pageCount: recapStructure.pageCount, sha256: sha256(recapPdf), bytes: recapPdf.length };
  process.stdout.write(`${JSON.stringify({ applied: true, release: "R0.74S", outputs: [noteResult, recapResult] }, null, 2)}\n`);
} finally {
