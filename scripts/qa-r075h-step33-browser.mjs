...
const outputRoot = resolve(process.env.R075H_STEP33_BROWSER_QA_OUTPUT ?? "/tmp/r075h-step33-browser-qa");
const externalBase = process.env.R075H_PUBLIC_BASE_URL;
...
try {
  for (const scenario of scenarios) {
    ...
    const url = new URL("/notes/r0-75h.html", baseUrl);
    url.searchParams.set("lang", scenario.lang);
    url.searchParams.set("qa", "r075h-step33");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        figureSections: document.querySelectorAll('#figure, [src*="r075h"], [href*="assets/r075h"]').length,
        pureTransportProved: /PURE TRANSPORT PROVED/.test(visibleText),
        signedPositivePart: /SIGNED POSITIVE PART/.test(visibleText),
        terminalTube: /TERMINAL TUBE/.test(visibleText),
        fixedLiftNoSeam: /FIXED LIFT \/ NO SEAM/.test(visibleText),
        oneThirdBenchmarkGain: /R\^\(1\/3\) BENCHMARK GAIN/.test(visibleText),
        favorableRate: /RATE -4279\/238140000/.test(visibleText),
        diffusiveExtensionOpen: /DIFFUSIVE EXTENSION OPEN/.test(visibleText),
        e24Open: /E\.24 OPEN/.test(visibleText),
        notNseFunctional: /NOT AN NSE SOLUTION FUNCTIONAL/.test(visibleText),
    ...
    assert.equal(audit.figureSections, 0, `${scenario.id} no R0.75H formal figure`);
    assert.equal(audit.pureTransportProved, true, `${scenario.id} pure-transport boundary`);
    assert.equal(audit.signedPositivePart, true, `${scenario.id} signed positive-part boundary`);
    assert.equal(audit.terminalTube, true, `${scenario.id} terminal-tube boundary`);
    assert.equal(audit.fixedLiftNoSeam, true, `${scenario.id} fixed-lift boundary`);
    assert.equal(audit.oneThirdBenchmarkGain, true, `${scenario.id} one-third benchmark gain`);
    assert.equal(audit.favorableRate, true, `${scenario.id} favorable rate`);
    assert.equal(audit.diffusiveExtensionOpen, true, `${scenario.id} diffusive extension boundary`);
    assert.equal(audit.e24Open, true, `${scenario.id} E.24 open boundary`);
    assert.equal(audit.notNseFunctional, true, `${scenario.id} benchmark functional boundary`);
