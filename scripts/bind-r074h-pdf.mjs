...
// Render and bind the complete Chinese R0.74H note. This certifies publication
...
const release = "r074h";
const noteSlug = "r0-74h";
const title = "R0.74H｜环带通量修复：冻结局部坐标中的双区间闭合";
...
try {
  ...
  if (structure.title !== title) throw new Error("R0.74H PDF title drift");
  ...
  ) throw new Error("R0.74H render provenance mismatch");

  const binding = {
    schemaVersion: "r074h-synchronized-pdf-binding-v1",
    release: "R0.74H",
    ...
    claimBoundary: {
      ...
      evidenceClassesSeparated: ["PROVED", "FINITE", "OPEN", "LITERATURE BOUNDARY", "NOT CLAY"],
