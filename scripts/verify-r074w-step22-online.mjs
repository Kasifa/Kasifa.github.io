...
const baseUrl = new URL(process.env.R074W_PUBLIC_BASE_URL ?? "https://kasifa.github.io/");
const commit = process.env.R074W_DEPLOYED_COMMIT ?? "unversioned";
const figureId = "fig-r074w-remote-adjacent-inward-threshold";
const publicFigureRoot = `public/figures/r074w/${figureId}`;
const sha256 = (bytes) => createHash("sha256").update(bytes).digest("hex");

const fixedObjects = [
  ["public/notes/r0-74w.html", "/notes/r0-74w.html"],
  ["public/notes/r0-74w.pdf", "/notes/r0-74w.pdf"],
  ...
  [`public/assets/r074w/${figureId}.pdf`, `/assets/r074w/${figureId}.pdf`],
  [`public/assets/r074w/${figureId}.png`, `/assets/r074w/${figureId}.png`],
  [`public/assets/r074w/${figureId}.svg`, `/assets/r074w/${figureId}.svg`],
];

const figureNames = execFileSync(
  "git",
  ["ls-tree", "-r", "--name-only", "HEAD", `research/figures/r074w/${figureId}`],
...
const objects = [
  ...
  ...figureNames.map((name) => [
    ...
    `/figures/r074w/${figureId}/${encodeURIComponent(name)}`,
  ]),
];
assert.equal(objects.length, 37, "Step 22 online publication inventory");
