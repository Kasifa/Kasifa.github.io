...
const protectedArtifacts = {
  "public/notes/clay-b-pressure-mechanism-screen-20260906.html": "bd21ab2689715ba67dd20309be18f049d89434a5d16813c47dd5160aac7e8ec5",
...
const independentChapters = [
  ...
  ["CB.15", "clay-b-ancient-constant-screen-20260906"],
];

test("the HTML-only AncientConstantScreen note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-ancient-constant-screen-20260906.html");
  for (const marker of [
    "ClayB-AncientConstantScreen-20260906", "CB.15", "真实 NS 演化仍不足以", "Genuine NS evolution still does not",
    "第一次精确缩放", "The first exact scaling", "终点峰值再归一化", "After terminal-peak normalization",
    "没有复制同一固定初值", "does not reproduce the complete history", "不是一般常量分类", "not a counterexample to a general classification",
    "PROVED METHOD OBSTRUCTION", "GENUINE NS SEGMENTS", "FIXED-HISTORY NOT REPRODUCED", "OPEN", "NOT CLAY",
  ...
  assert.equal(note.includes("/notes/clay-b-ancient-constant-screen-20260906.pdf"), false);
  assert.equal(note.includes("R0.76M"), false);
});

test("homepage keeps the solid R0 sequence separate from the dashed CB.1–CB.15 lane", async () => {
  ...
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-ancient-constant-screen-20260906"));
  ...
  const positions = independentChapters.map(([chapter, slug]) => {
    ...
    assert.ok(home.slice(row, row + 2000).includes(chapter), `${chapter} route label`);
    return row;
  });
  assert.deepEqual(positions, [...positions].sort((a, b) => a - b));
  assert.equal((home.match(/class="route-overview independent-release-spotlight"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-ancient-constant-screen"/g) ?? []).length, 1);
  assert.equal((literature.match(/id="clay-b-ancient-constant-screen-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-ancient-constant-screen-20260906"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-ancient-constant-screen-20260906.pdf"), false);
  assert.equal(site.version, "2.59");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 15);
  assert.equal(site.latestIndependentNote, "ClayB-AncientConstantScreen-20260906");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-ancient-constant-screen-20260906.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.latestIndependentChapter, "CB.15");
  assert.equal(site.nextIndependentChapter, "CB.16");
  assert.equal(manifest.siteVersion, "2.59");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestPublication.releaseId, "clay-b-ancient-constant-screen-20260906");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-PressureMechanismScreen-20260906");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.15");
});
...
test("historical milestones, endpoint, prior note, and retained PDFs remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-ancient-constant-screen-20260906.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-ancient-constant-screen-20260906.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-ancient-constant-screen-20260906.pdf")));
});
