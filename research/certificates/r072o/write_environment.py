#!/usr/bin/env python3
"""Capture the two runtime environments used by the R0.72O audit."""
...
def common() -> list[str]:
    git_executable = subprocess.check_output(
        ["which", "git"], text=True
    ).strip()
    return [
...
def main() -> None:
    producer = [
        "route=producer Python exact rational monomial audit",
        f"pythonExecutable={sys.executable}",
        f"pythonVersion={sys.version.replace(chr(10), ' ')}",
        *common(),
    ]
    independent = [
        "route=independent JavaScript BigInt rational monomial audit",
    ...
    print("R0.72O producer and independent environment records written")
