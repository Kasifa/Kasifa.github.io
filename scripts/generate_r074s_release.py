...
RELEASE = "r074s"
CODE = "R0.74S"
TITLE = "R0.74S｜删除已付分支后的 residual best-N gate"
...
<title>{TITLE}</title><meta name="description" content="删除已付分支后，完整 clock tail 精确归约为两个 residual mechanisms 共用的一个 best-N gate；S.243 仍开放">
...
<body><nav class="top"><a href="/research-review.html">研究首页</a><span>R0.74S · 2026-09-03</span></nav><main><header class="hero"><div class="hero-inner"><div><div class="section-no">研究笔记 R0.74S · 完整中文版本</div><h1>{TITLE}</h1><p>四个已付分支只收取一次 \\(Q\\) ledger 与一次 cubic ledger；剩余两类机制共享同一个 residual best-\\(N\\) gate。<strong>THE REDUCTION IS EXACT; THE PDE PACKING ESTIMATE S.243 IS OPEN. NOT CLAY.</strong></p><div class="labels"><span class="label">PROVED</span><span class="label">EXACT PARTITION</span><span class="label">ONE Q LEDGER</span><span class="label">ONE CUBIC LEDGER</span><span class="label">RESIDUAL GATE</span><span class="label">NOT CLAY</span></div></div><div class="stamp"><strong>状态 · R0.74S STEP 10</strong><p>S.223–S.247：PROVED / AUDITED</p><p>P_beta + P_Q：ONE 6B_Q</p><p>P_sigma + P_LE：ONE C5</p><p>residual：R_sh ∪ R_x</p><p>T/6 &lt; r &lt; T/2</p><p>best-N reduction：DOMAIN-SAFE</p><p>S.243 residual packing：OPEN</p><p>Q.12 / Q.1：OPEN</p><p>regularity：OPEN</p><p>无仿真 / NO DGX</p></div></div></header><article>
{report_body()}
<section id="figure"><div class="section-no">F / 期刊主图</div><h2>已付分支删除、两类 residual 与共享 best-N gate</h2><picture><source srcset="/assets/r074s/{FIGURE_ID}.svg" type="image/svg+xml"><img src="/assets/r074s/{FIGURE_ID}.png" alt="R0.74S paid-branch deletion and residual best-N gate"></picture><p><a href="/assets/r074s/{FIGURE_ID}.pdf">矢量 PDF</a> · <a href="/assets/r074s/{FIGURE_ID}.png">600 dpi PNG</a> · <a href="/assets/r074s/{FIGURE_ID}.svg">SVG</a> · <a href="/figures/r074s/{FIGURE_ID}/source-data.csv">source data</a> · <a href="/figures/r074s/{FIGURE_ID}/qa-report.md">视觉 QA</a></p><p class="note">已完成 six-class partition、single-ledger payment 与 domain-safe reduction；S.243 的 PDE packing 仍开放，不是 DNS、奇点或正则性证据。</p></section>
<section id="reproduce"><div class="section-no">R / 冻结证据</div><h2>解析主文、证书与独立审计</h2><p class="files"><a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_problem_freeze.md">问题冻结</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_best_n_last_exit_equivalence.md">Step 9 主文</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_paid_branch_last_exit_residual.md">Step 10 最终解析主文</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_paid_branch_last_exit_primary_audit.md">Step 10 主审计</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_paid_branch_last_exit_independent_audit.md">Step 10 独立审计</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_paid_branch_last_exit_certificate.json">Step 10 机器证书 JSON</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_paid_branch_last_exit_certificate_report.md">Step 10 证书报告</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_claim_state_update.md">主张边界</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_literature_boundary.md">文献边界</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_report-source.md">中文 reader source</a></p><p><a href="/notes/r0-74s.pdf">同步研究笔记 PDF</a> · <a href="/recap-r0-61-r0-74o.html">上一大里程碑 recap（截止 R0.74O，157 节）</a> · <a href="/recap-r0-61-r0-74o.pdf">PDF</a></p><p class="note">Step 10：Python 12/12 exact、10/10 finite、79/79 structural、47/47 mutations；Ruby 9/9 groups、65,681 cases、21/21 contract mutations、13/13 report checks、15/15 audit bindings。有限证书不替代 inherited local-energy/PDE 解析证明。</p></section>
<section id="next"><div class="section-no">NEXT / 下一门槛</div><h2 style="margin:.35rem 0 .15rem;font-size:1.15rem">R0.74T</h2><p style="margin:.15rem 0">分别研究 short non-\\(D\\)、\\(Q\\)-small packing 与 anomalous-defect / high-Rayleigh scalar-excess ancestry，最终用一个共享的固定 \\(N_0\\) exception budget 重组并检验 (S.243)。</p></section></article></main></body></html>'''
...
def update_home() -> None:
    ...
    pairs = (
        ('data-site-version="1.88"', 'data-site-version="1.89"', "home version"),
        ('/i18n-en.js?v=1.88', '/i18n-en.js?v=1.89', "home i18n"),
        ('/site-refresh.js?v=1.88.1', '/site-refresh.js?v=1.89.1', "home refresh"),
        ('<strong>v1.88</strong>网页版本', '<strong>v1.89</strong>网页版本', "home stat version"),
        ('<h3>R0.74S：canonical best-N last-exit 等价与 no-gain</h3>', '<h3>R0.74S：删除已付分支后的 residual best-N gate</h3>', "current route title"),
        ('<p class="tree-current-summary">F-half-exit 精确等于二分之一 signed best-N tail；K-last-exit 只在尖锐 B_Q 内等价。canonical stops 不产生新二次压缩。NOT CLAY。</p>', '<p class="tree-current-summary">六类精确分区把四个已付分支压入一次 Q ledger 与一次 cubic ledger；R_sh 与 R_x 共享一个 residual best-N gate，S.243 仍开放。NOT CLAY。</p>', "current route summary"),
        ('<p class="tree-path">no-exception no-go → signed half-exit → K-last-exit → sharp B_Q equivalence → paid-branch residual tail</p>', '<p class="tree-path">canonical last exit → D-first six-class partition → single-ledger payment → two-mechanism residual gate</p>', "current route path"),
        ('综述 v1.88 · 2026-09-03', '综述 v1.89 · 2026-09-03', "footer"),
    )
    for old, new, label in pairs:
        page = replace_once_or_present(page, old, new, label)
    page, count = re.subn(r'<div class="summary-item"><strong>我目前关注</strong><span>.*?</span></div>', '<div class="summary-item"><strong>我目前关注</strong><span>R0.74S Step 10 已删除四个已付分支，只留下 short non-D、Q-small 与 scalar-excess ancestry 两类 residual mechanism。下一门槛是用一个共享的固定 N0 budget 证明 S.243。</span></div>', page, count=1, flags=re.S)
    if count != 1:
        raise RuntimeError("home focus replacement failed")
    latest = '''<section class="route-overview latest-release-spotlight" id="latest-release" aria-labelledby="latest-release-title"><div class="route-overview-inner"><header class="route-map-header"><div><p class="eyebrow">LATEST RELEASE · R0.74S · 2026-09-03</p><h2 class="route-map-title" id="latest-release-title">R0.74S｜删除已付分支后的 residual best-N gate</h2><p class="route-map-intro">四个已付分支只收一次 Q 与 cubic ledger；两类 residual 共用一个 best-N gate。S.243 OPEN. NOT CLAY.</p></div><nav class="route-map-actions" aria-label="最新发布快捷入口"><a class="route-map-latest" href="/notes/r0-74s.pdf">阅读最新 R0.74S 研究笔记 →</a><a href="/assets/r074s/fig-r074s-ball-clock-debt.pdf">期刊主图</a><a href="/recap-r0-61-r0-74o.html">最新大里程碑 recap（R0.61–R0.74O，157 节）</a><a href="/notes/">221 篇研究笔记总索引</a><a href="#r074s">查看首页 R0.74S 卡片</a></nav></header><div class="route-legend" aria-label="最新发布计数"><span><i class="route-legend-mark kept" aria-hidden="true"></i>R0.70A–R0.74S · 123 节已公开</span><span><i class="route-legend-mark kept" aria-hidden="true"></i>98 节完整封存</span><span><i class="route-legend-mark current" aria-hidden="true"></i>当前端点 R0.74S</span></div></div></section>'''
...
def update_literature() -> None:
    ...
    for old, new, label in (
        ('data-site-version="1.88"', 'data-site-version="1.89"', "lit version"), ('/i18n-en.js?v=1.88', '/i18n-en.js?v=1.89', "lit i18n"), ('文献综述 v1.88 · 2026-09-03', '文献综述 v1.89 · 2026-09-03', "lit footer"),
    ):
        page = replace_once_or_present(page, old, new, label)
    old = '<div class="route-step kept"><header><b>R0.74S</b><strong>canonical best-N last-exit 等价与 no-gain</strong></header><p>S.200–S.207 给出 signed half-exit 精确表示；S.210–S.214 给出 K-last-exit 的尖锐一个 B_Q 比较；S.218 证明 canonical stops 与既有 best-N tail 等价，不产生新二次压缩。<a href="/notes/r0-74s.html">研究笔记</a> <a href="#r074s-boundary">主张边界</a></p></div><div class="route-step pause"><header><b>开放接口 · R0.74T</b><strong>paid-branch residual tail</strong></header><p>删去 Step 7 low-Rayleigh 与 Step 8 I_beta/I_sigma 已付分支后，定义并审计 forced PDE residual tail。</p></div>'
    new = '<div class="route-step kept"><header><b>R0.74S</b><strong>删除已付分支后的 residual best-N gate</strong></header><p>S.223–S.231 给出 D-first 六类分区与 single Q/cubic payments；S.232–S.240 在两类 residual 上证明 T/6&lt;r&lt;T/2，并建立 domain-safe best-N 等价。S.243 的 fixed-N PDE packing 仍开放。<a href="/notes/r0-74s.html">研究笔记</a> <a href="#r074s-boundary">主张边界</a></p></div><div class="route-step pause"><header><b>开放接口 · R0.74T</b><strong>two-mechanism residual packing</strong></header><p>分别研究 R_sh 与 R_x，最终用一个共享的固定 N0 exception budget 重组。</p></div>'
    page = replace_once_or_present(page, old, new, "literature route")
    boundary = '<h3 id="r074s-boundary">R0.74S 的文献与主张边界</h3><p>Step 10 在继承的 completed-clock、last-exit 与 local-energy payments 上做精确分支删除；不声称新颖性、优先权或新的 PDE packing theorem。</p><div class="boundary"><strong>R0.74S Step 10 公开边界</strong><p>PROVED：D-first exact six-class partition；P_beta/P_Q 共用一个 6B_Q；P_sigma/P_LE 共用一个 C5 ledger；R_sh/R_x 上 T/6&lt;r&lt;T/2；domain-safe best-N reduction；plateau 与 small-payment corollary。REFUTED / RULED OUT：额外 low-Rayleigh residual、重复完整 ledger、两份 N-budget、terminal-D 自动 last-exit persistence、仅靠 last-exit algebra 的 fixed-N compression。FINITE：Python 12/12 exact、10/10 finite、79/79 structural、47/47 mutations；Ruby 9/9 groups、65,681 cases、21/21 contract mutations、13/13 report checks、15/15 audit bindings。OPEN：S.243、两类 residual 的 PDE packing、Q.12、Q.1 与正则性。保留此前 no-go。<strong>NOT CLAY.</strong> <a href="/notes/r0-74s.html">阅读完整中文笔记</a>。</p></div>\n'
...
def update_notes_index() -> None:
    ...
    for old, new, label in (
        ('data-site-version="1.88"', 'data-site-version="1.89"', "index version"), ('/i18n-en.js?v=1.88', '/i18n-en.js?v=1.89', "index i18n"), ('/site-refresh.js?v=1.88', '/site-refresh.js?v=1.89', "index refresh"), ('研究笔记总索引 · v1.88 · 2026-09-03', '研究笔记总索引 · v1.89 · 2026-09-03', "index footer"),
    ):
        page = replace_once_or_present(page, old, new, label)
    entry = '''          <li class="note-entry" data-note="r0-74s"><article><div class="entry-copy"><p class="note-code">R0.74S</p><h3>删除已付分支后的 residual best-N gate</h3></div><nav class="entry-files" aria-label="R0.74S files"><a class="file-link html" href="/notes/r0-74s.html" aria-label="Read R0.74S HTML">HTML</a><a class="file-link pdf" href="/notes/r0-74s.pdf" aria-label="Download R0.74S PDF">PDF</a></nav></article></li>\n'''
