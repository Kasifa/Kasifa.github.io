...
1. Test the sharp two-radius family against the actual invariant coefficient
   subspace and symmetrized tree aggregation.
2. Continue to a tagged order-12 residual only if that structural test removes
   the sharp family or supplies an additional cancellation.
3. Implement the Morosi--Pizzocchero control ODE with interval arithmetic only
...
5. Publish a formal remainder note only after Gate C closes or yields a
   precise obstruction.
