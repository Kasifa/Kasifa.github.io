...
const protectedArtifacts = {
  "public/notes/clay-b-signed-mixed-pressure-20260907.html": "f7fc2adf6eb00815022df47eb071eaa443fdaea5c6008a4770bfd2876094707e",
...
test("the HTML-only SourceEnstrophy note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-source-enstrophy-20260907.html");
  for (const marker of [
    "ClayB-SourceEnstrophy-20260907", "CB.24", "严格正时间的梯度恒等式",
    "Gradient identities on strict positive-time intervals", "常系数二次组合的有限类障碍",
    "Finite-class obstruction for constant quadratic combinations", "条件性 4/3 二阶成本发散",
    "Conditional divergence of the 4/3 second-order cost", "ONE-WAY ONLY", "PROVED",
    "CONDITIONAL", "DIRECT DERIVATION", "NO CLOSURE", "LITERATURE", "FINITE CHECKS ONLY", "OPEN", "NOT CLAY",
  ...
  assert.equal(note.includes("/notes/clay-b-source-enstrophy-20260907.pdf"), false);
...
test("homepage keeps R0 solid, Clay-B dashed, sequential, and spotlight-latest", async () => {
  ...
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-source-enstrophy-20260907"));
  ...
  const boundary = home.indexOf('class="tree-row clay-b-public-boundary-row"', cb24);
  assert.ok(r0 < rb && rb < divider && divider < clay && clay < cb22 && cb22 < cb23 && cb23 < cb24 && cb24 < boundary);
  ...
  assert.equal((home.match(/id="clay-b-source-enstrophy"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-signed-mixed-pressure"/g) ?? []).length, 0);
  assert.equal((literature.match(/id="clay-b-source-enstrophy-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-source-enstrophy-20260907"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-source-enstrophy-20260907.pdf"), false);
  assert.equal(site.version, "2.68");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 24);
  assert.equal(site.latestIndependentNote, "ClayB-SourceEnstrophy-20260907");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-source-enstrophy-20260907.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.latestIndependentChapter, "CB.24");
  assert.equal(site.nextIndependentChapter, "CB.25");
  assert.equal(manifest.siteVersion, "2.68");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestPublication.releaseId, "clay-b-source-enstrophy-20260907");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-SignedMixedPressure-20260907");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.24");
});
...
test("prior note, canonical endpoint, recap, and historical PDF remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-source-enstrophy-20260907.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-source-enstrophy-20260907.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-source-enstrophy-20260907.pdf")));
});
