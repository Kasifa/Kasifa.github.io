...
// Render and bind the complete Chinese R0.74M note. This certifies publication
...
const release = "r074m";
const noteSlug = "r0-74m";
const title = "R0.74M｜最后一小段布朗路径，排出了最近内领圈";
...
try {
  ...
  if (structure.title !== title) throw new Error("R0.74M PDF title drift");
  ...
  ) throw new Error("R0.74M render provenance mismatch");

  const binding = {
    schemaVersion: "r074m-synchronized-pdf-binding-v1",
    release: "R0.74M",
