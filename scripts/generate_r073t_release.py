#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Fail-closed R0.73T GitHub Pages release transaction.

The mathematical copy comes from ``r073t_release_content.py``.  This file owns
...
from r073t_release_content import (
...
RELEASE_SOURCE_COMMIT = "ade09232e00905885840727374ced4daff1a9036"

BINDING_ORDER = (
    ("R0.73S published baseline", RELEASE_BASELINE_COMMIT),
    ("R0.73T analytic source", ANALYTIC_SOURCE_COMMIT),
    ("R0.73T finite package", FINITE_PACKAGE_COMMIT),
    ("R0.73T formal figure package", FIGURE_PACKAGE_COMMIT),
    ("R0.73T final reader content", FINAL_CONTENT_COMMIT),
    ("R0.73T release source", RELEASE_SOURCE_COMMIT),
)
...
FINITE_EXACT_ROOTS = ("research/certificates/r073t",)
...
RELEASE_SOURCE_EXACT_PATHS = (
    "scripts/r073t_release_content.py",
    "scripts/generate_r073t_release.py",
    "scripts/add-r073t-translations.mjs",
    "scripts/bind-r073t-pdfs.mjs",
    "scripts/render-note-pdf.mjs",
    "tests/r073t-dynamic-autocorrelation-gate.test.mjs",
...
LATER_STAGE_OUTPUTS = (
    "public/notes/r0-73t.pdf",
    "public/recap-r0-61-r0-73t.pdf",
    "research/r073t_note_pdf_render.json",
    "research/r073t_recap_pdf_render.json",
    "research/r073t_pdf_bindings.json",
    "translations/en.json",
    "public/i18n-en.js",
    "scripts/i18n-snapshots/r073t-missing.json",
)
FIGURE_PUBLIC_OUTPUTS = (
    f"{FIGURE_ARCHIVE_RELATIVE}/manifest.json",
    f"public/{FIGURE_ARCHIVE_RELATIVE}/manifest.json",
    f"public/assets/r073t/{FIGURE_ID}.pdf",
    f"public/assets/r073t/{FIGURE_ID}.svg",
    f"public/assets/r073t/{FIGURE_ID}.png",
)
PUBLICATION_STAGE_ORDER = (
    "freeze-r073s-published-baseline",
    "freeze-r073t-analytic-and-package-sources",
...
def normalized_release_generator(payload: bytes) -> bytes:
    ...
    if count != 1:
        raise RuntimeError("R0.73T normalized release-source pin slot drift")
...
def verify_binding_order() -> None:
    ...
    for (_, older), (_, newer) in zip(BINDING_ORDER, BINDING_ORDER[1:]):
        ...
        if completed.returncode != 0:
            raise RuntimeError("R0.73T binding commits are not in declared ancestry order")
...
def verify_commit_paths(commit: str, paths: tuple[str, ...], label: str) -> None:
    for relative in paths:
        ...
        if relative == "scripts/generate_r073t_release.py":
...
def decode_baseline(relative: str) -> str:
    try:
        ...
        raise RuntimeError(relative + ": frozen R0.73S baseline is not UTF-8") from exc
...
def assert_public_html(value: str, label: str, *, require_boundary: bool = True) -> None:
    ...
    tokens = ["R0.73T", "/i18n-en.js?v=1.60"]
    if require_boundary:
...
def validate_certificate_package() -> dict:
    base = "research/certificates/r073t"
    manifest = strict_json_file(f"{base}/manifest.json", "R0.73T certificate manifest")
    checklist = strict_json_file(
        f"{base}/audit-checklist.json", "R0.73T certificate checklist"
    )
    results = strict_json_file(f"{base}/results.json", "R0.73T certificate results")
    if (
        ...
        or manifest.get("checkInventory") != {"exact": 55, "required": 55}
        ...
        raise RuntimeError("R0.73T certificate is not final-sealed to the analytic commit")
    ...
    if (
        ...
        or len(required_checks) != 55
        or len({row.get("id") for row in required_checks if isinstance(row, dict)}) != 55
        or results.get("schemaVersion") != 1
        or not isinstance(audit, dict)
        or audit.get("required") != 55
        or audit.get("passed") != 55
        or not isinstance(audit.get("results"), list)
        or len(audit["results"]) != 55
        or not all(isinstance(row, dict) and row.get("pass") is True
                   for row in audit["results"])
    ):
        raise RuntimeError("R0.73T certificate exact 55-check audit drifted")
    producer = results.get("producer")
    if (
        results.get("arithmetic")
        != "Python standard-library fractions.Fraction; no floating point"
        or not isinstance(producer, dict)
        or producer.get("standardLibraryOnly") is not True
        or producer.get("gpu") != "not used"
        or producer.get("network") != "not used"
    ):
        raise RuntimeError("R0.73T certificate arithmetic/compute boundary drifted")
    boundary = manifest.get("claimBoundary")
    if not isinstance(boundary, str) or not all(token in boundary for token in (
        "Exact finite Fourier identities", "no generic PDE integration",
        "global regularity", "Clay conclusion",
    )):
        raise RuntimeError("R0.73T certificate claim boundary drifted")
    ...
    if actual != expected:
        raise RuntimeError("R0.73T certificate nine-file inventory drifted")
    recorded = manifest.get("files")
    if not isinstance(recorded, list) or len(recorded) != 7:
        raise RuntimeError("R0.73T certificate manifest-bound inventory drifted")
    by_name = {
        row.get("path"): row for row in recorded
        if isinstance(row, dict) and isinstance(row.get("path"), str)
    }
    if set(by_name) != set(expected) - {"manifest.json", "SHA256SUMS"}:
        raise RuntimeError("R0.73T certificate manifest file paths drifted")
    for name, row in by_name.items():
        payload = current_regular_bytes(f"{base}/{name}")
        if row.get("bytes") != len(payload) or row.get("sha256") != sha256(payload):
            raise RuntimeError("R0.73T certificate manifest is not byte-bound: " + name)
    source_names = {
    ...
    if not isinstance(source_rows, list) or {row.get("path") for row in source_rows} != source_names:
        raise RuntimeError("R0.73T certificate six-source inventory drifted")
    run_package_verifier(
        f"{base}/compute_exact_certificate.py", ["--check-only"],
        "R0.73T exact certificate producer",
    )
    run_package_verifier(
        f"{base}/seal_package.py",
        ["--source-commit", ANALYTIC_SOURCE_COMMIT, "--check-only"],
        "R0.73T certificate provenance seal",
    )
    return manifest
...
def validate_figure_package(certificate_manifest: dict) -> dict:
    ...
    manifest = strict_json_file(f"{base}/manifest.json", "R0.73T figure manifest")
    config = strict_json_file(f"{base}/config.json", "R0.73T figure config")
    contract = strict_json_file(f"{base}/contract.json", "R0.73T figure contract")
    results = strict_json_file(f"{base}/results.json", "R0.73T figure results")
    validation = strict_json_file(f"{base}/validation.json", "R0.73T figure validation")
    if (
        ...
        or manifest.get("validationCheckCount") != 106
        or manifest.get("inventory") != {
            ...
            "sourceCommitBindingCount": 11,
            "sourceFileCount": 10,
        }
        or certificate_manifest.get("sourceCommit") != ANALYTIC_SOURCE_COMMIT
    ):
        raise RuntimeError("R0.73T figure is not source-bound and QA-confirmed")
    if (
        config.get("schemaVersion") != "r073t-dynamic-autocorrelation-figure-config-v1"
        ...
        != "r073t-dynamic-autocorrelation-figure-results-v1"
        or results.get("figureId") != FIGURE_ID
        or results.get("rowCount") != 28
        or results.get("certificateChecks") != 55
        or results.get("allSourceChecksPass") is not True
        or results.get("series") != {
            "analyticChain": 4, "carrier": 8, "pressurePair": 16,
        }
    ):
        raise RuntimeError("R0.73T figure result/configuration drifted")
    checks = validation.get("checks")
    if (
        validation.get("schemaVersion")
        != "r073t-dynamic-autocorrelation-figure-validation-v1"
    ...
    for key in (
        ...
        if boundary.get(key) is not False:
            raise RuntimeError("R0.73T figure overclaims " + key)
    if boundary.get("exactFormulaAndFiniteWitnessOnly") is not True:
        raise RuntimeError("R0.73T figure lost its exact finite evidence class")
    recorded = manifest.get("files")
    if not isinstance(recorded, list) or len(recorded) != 23:
        raise RuntimeError("R0.73T figure sealed inventory drifted")
    by_name = {
        row.get("path"): row for row in recorded
        if isinstance(row, dict) and isinstance(row.get("path"), str)
    }
    if len(by_name) != len(recorded):
        raise RuntimeError("R0.73T figure sealed inventory contains duplicate paths")
    for name, row in by_name.items():
        payload = current_regular_bytes(f"{base}/{name}")
        if row.get("bytes") != len(payload) or row.get("sha256") != sha256(payload):
            raise RuntimeError("R0.73T figure manifest is not byte-bound: " + name)
    expected_hashes = {
    ...
    for name, expected_hash in expected_hashes.items():
        payload = current_regular_bytes(f"{base}/{name}")
        if by_name.get(name, {}).get("sha256") != expected_hash or sha256(payload) != expected_hash:
            raise RuntimeError("R0.73T figure bound hash drifted: " + name)
    dependency_root = os.environ.get(
        "R073T_FIGURE_DEPS", "/Users/kasifa/.cache/codex-runtimes/r073s-figure-python"
    )
    ...
    if not Path(dependency_root).is_dir() or not Path(executable).is_file():
        raise RuntimeError(
            "R0.73T figure verifier runtime missing; set R073T_FIGURE_DEPS and R073T_FIGURE_PYTHON"
        )
    run_package_verifier(
        f"{base}/validate.py",
        ["--deps", dependency_root, "--source-commit", ANALYTIC_SOURCE_COMMIT,
         "--verify-only"],
        "R0.73T figure",
...
def certificate_summary() -> dict[str, object]:
    root = ROOT / "research/certificates/r073t"
...
def build_note(content: ReleaseContent) -> str:
    value = decode_baseline("public/notes/r0-73s.html")
    description = (
        "研究笔记 R0.73T：能量密度自相关的精确演化、一侧动态 AQ 估计、"
        "载频与带符号速度相位的非自治见证，以及压力张量障碍。"
    )
    value = replace_regex(
    ...
    value = replace_regex(
        ...
        f'<meta property="og:image" content="https://kasifa.github.io/assets/r073t/{FIGURE_ID}.png">',
        "note OG image",
    )
    value = replace_regex(
        value, r'<title>.*?</title>',
    ...
    value = replace_once(value, "/i18n-en.js?v=1.59", "/i18n-en.js?v=1.60", "note i18n")
    navigation = "".join(
    ...
    value = replace_regex(
        ...
        f'<div>研究笔记 R0.73T · {html.escape(content.date)}<br>'
        '<a href="/">返回研究主页</a></div></footer>',
        "note footer",
    )
    assert_public_html(value, "R0.73T note")
    if "/note-retro.css" not in value or value.count(FIGURE_ID) < 4:
        raise RuntimeError("R0.73T note lost retro stylesheet or formal figure links")
    return value


def build_recap(content: ReleaseContent) -> str:
    value = decode_baseline("public/recap-r0-61-r0-73s.html")
    value = value.replace("/i18n-en.js?v=1.59", "/i18n-en.js?v=1.60")
    value = value.replace("R0.61–R0.73S", "R0.61–R0.73T")
    value = value.replace("R0.61 到 R0.73S", "R0.61 到 R0.73T")
    value = value.replace("R0.69P–R0.73S", "R0.69P–R0.73T")
    value = value.replace("R0.70A–R0.73S", "R0.70A–R0.73T")
    value = value.replace("回顾截止节点：R0.73S", "回顾截止节点：R0.73T")
    value = value.replace("收录节点：135", "收录节点：136")
    value = value.replace("回顾截止时公开笔记：195", "回顾截止时公开笔记：196")
    value = value.replace("135 节", "136 节").replace("135 个节点", "136 个节点")
    value = value.replace("97 个版本", "98 个版本").replace("97 节", "98 节")
    value = value.replace("73 个满足", "74 个满足").replace("73 节", "74 节")
    value = value.replace("54 个阶段", "55 个阶段")
    value = value.replace("54 个研究阶段", "55 个研究阶段")
    value = value.replace("<strong>135</strong>", "<strong>136</strong>")
    value = value.replace("<strong>97</strong>", "<strong>98</strong>")
    value = value.replace("<strong>73</strong>", "<strong>74</strong>")
    value = replace_regex(
    ...
    value = replace_regex(
        ...
        'R0.73T 共 136 个节点；最新一节分开一侧动态估计、临界 A 预算、'
        '标量非自治见证与压力张量障碍。">',
        "recap description",
    )
    value = replace_regex(
        value, r'<meta property="og:description" content="[^"]*">',
        '<meta property="og:description" content="55 个阶段、136 个节点：从约化递推和环带排除'
        '到动态自相关、精确非自治见证与压力张量障碍。">',
    ...
    node = (
        '<span class="node-ref"><a href="/notes/r0-73t.html">R0.73T</a>'
    ...
    retained = (
        '<li>R0.73T 把 R0.73S 的静态自相关证书接入经典 L4 动力学，得到一侧 '
        '<code>AQ</code> 微分不等式；同时用载频族与六模态符号对精确证明标量自相关非自治。'
        '缺失的 <code>∫A dt</code> 已有经典 LPS 强度，一般压力重建还需要完整二次张量。'
        '这不包含 Navier--Stokes 仿真、奇性或 Clay 结论。</li>'
    )
    value = replace_once(
        value,
    ...
    value = replace_regex(
        ...
        '<h2>动态正结果、经典强度预算与非自治障碍已经分开</h2>'
        f'<p>{html.escape(content.recap_zh)}</p>'
        '<p>不能把 136 个节点或 98 个公开版本解释成 Clay 完成比例。'
        '一侧动态估计成立；但临界 A 积分、带符号通量与张量 heat 闭合保持开放。'
    ...
    value = replace_regex(
        ...
        f'<h2>{html.escape(content.next_release)}：带尺度张量 heat hierarchy</h2>'
    ...
    value = replace_regex(
        ...
        '<a href="/recap-r0-61-r0-73s.html">保留 R0.73S 历史回顾</a> · '
        '<a href="/notes/r0-61.html">从 R0.61 开始逐节阅读</a> · '
        '<a href="/notes/r0-73t.html">打开最新节点 R0.73T</a></p>'
        '<p><a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r073t_report-source.md">查看 canonical report</a> · '
        '<a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r073t_dynamic_autocorrelation_budget.md">查看解析证明</a> · '
        '<a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r073t_primary_literature_audit.md">查看一手文献审计</a> · '
        '<a href="https://github.com/Kasifa/Kasifa.github.io/tree/main/research/certificates/r073t">查看有限证书</a> · '
        f'<a href="/assets/r073t/{FIGURE_ID}.pdf">下载期刊附图</a> · '
        '<a href="/recap-r0-61-r0-73t.pdf">下载同步 PDF</a></p>'
        '<p>经典结论由一手来源承担；本地解析证明承担一侧推论和 no-go；有限诊断只作可复现错误探测。'
        'NOT CLAY。</p></section>',
        "recap reproduction",
    )
    value = value.replace("/recap-r0-61-r0-73s.pdf", "/recap-r0-61-r0-73t.pdf")
    value = value.replace("R0.73S 回顾", "R0.73T 回顾")
    for stale in ("<strong>135</strong>", "<strong>97</strong>", "<strong>73</strong>"):
        if stale in value:
            raise RuntimeError("R0.73T recap retained stale metric " + stale)
    if value.count('<article class="phase">') != 55:
        raise RuntimeError("R0.73T recap must contain exactly 55 phase articles")
    if "54 个阶段" in value or "54 个研究阶段" in value:
        raise RuntimeError("R0.73T recap retained a stale displayed phase count")
    assert_public_html(value, "R0.73T recap")
    return value
...
def latest_spotlight(content: ReleaseContent) -> str:
    return (
        ...
        f'<p class="eyebrow">LATEST RELEASE · R0.73T · {html.escape(content.date)}</p>'
        ...
        '<a class="route-map-latest" href="/notes/r0-73t.pdf">阅读最新 R0.73T 研究笔记 →</a>'
        '<a href="/recap-r0-61-r0-73t.html">136 节累计回顾</a>'
        '<a href="/notes/">196 篇研究笔记总索引</a>'
        '<a href="#r073t">查看首页完整 R0.73T 卡片</a></nav></header>'
        '<div class="route-legend" aria-label="最新发布计数">'
        '<span><i class="route-legend-mark kept" aria-hidden="true"></i>'
        'R0.70A–R0.73T · 98 节已公开</span>'
        '<span><i class="route-legend-mark kept" aria-hidden="true"></i>74 节完整封存</span>'
        '<span><i class="route-legend-mark current" aria-hidden="true"></i>'
        '当前端点 R0.73T</span></div></div></section>'
    )


def update_home(content: ReleaseContent) -> str:
    value = decode_baseline("public/research-review.html")
    value = value.replace('data-site-version="1.59"', 'data-site-version="1.60"')
    value = value.replace("/i18n-en.js?v=1.59", "/i18n-en.js?v=1.60")
    value = value.replace("/site-refresh.js?v=1.59", "/site-refresh.js?v=1.60")
    value = replace_regex(
        value, r'<section class="route-overview latest-release-spotlight".*?</section>',
        latest_spotlight(content), "home latest spotlight",
    )
    marker = '<div class="task-one" id="r073s" data-release="r073s"'
    value = replace_once(value, marker, content.home_card + "\n          " + marker, "home card insert")
    value = value.replace("<strong>195</strong>公开研究笔记", "<strong>196</strong>公开研究笔记")
    value = value.replace("<strong>v1.59</strong>网页版本", "<strong>v1.60</strong>网页版本")
    value = value.replace("<strong>R0.73S</strong>最新研究节点", "<strong>R0.73T</strong>最新研究节点")
    value = value.replace(
        "<strong>quadratic autocorrelation certificate / exact no-go boundary</strong>当前方向",
        "<strong>dynamic autocorrelation / pressure-tensor barrier</strong>当前方向",
    )
    value = value.replace("NEXT · R0.73T", "NEXT · R0.73U")
    value = replace_regex(
        value, r'<h3>R0\.73T 下一接口</h3><p>.*?</p>',
    ...
    value = value.replace("Research topology · R0.1–R0.73S", "Research topology · R0.1–R0.73T")
    value = value.replace(
    ...
    value = value.replace("/recap-r0-61-r0-73s.html", "/recap-r0-61-r0-73t.html")
    value = value.replace("/recap-r0-61-r0-73s.pdf", "/recap-r0-61-r0-73t.pdf")
    value = value.replace("综述 v1.59 ·", "综述 v1.60 ·")
    value = value.replace(
    ...
    value = value.replace(
        ...
        "R0.73T：动态 AQ 估计、精确非自治见证与压力张量障碍已分列",
    )
    value = value.replace('<span>R0.72R–R0.73S：</span>', '<span>R0.72R–R0.73T：</span>')
    value = value.replace(
        "→ shellwise phase certificate → quadratic autocorrelation certificate</p>",
        "→ shellwise phase certificate → quadratic autocorrelation certificate → dynamic autocorrelation</p>",
    )
    current_paragraph = (
        f'<p>{html.escape(content.recap_zh)} '
        '<span>dynamic AQ upper inequality</span>；'
        '<span>carrier-scale non-autonomy</span>；'
        '<span>pressure-tensor barrier</span>。缺失的 A 积分已有经典 LPS 强度。</p>'
    )
    value = replace_once(
    ...
    value = value.replace('<summary>展开 105 篇公开笔记</summary>', '<summary>展开 106 篇公开笔记</summary>')
    value = value.replace('aria-label="R0.69P–R0.73S"', 'aria-label="R0.69P–R0.73T"')
    value = replace_once(
        value,
        '                  <a class="milestone" href="/notes/r0-73s.html">R0.73S</a>',
        '                  <a class="milestone" href="/notes/r0-73s.html">R0.73S</a>\n'
        '                  <a class="milestone" href="/notes/r0-73t.html">R0.73T</a>',
        "home route R0.73T link",
    )
    value = replace_regex(
    ...
    recap_card = (
        ...
        '<p class="eyebrow">累计回顾 R0.61–R0.73T · 2026-08-31</p>'
        '<h3>R0.60 recap 之后的累计回顾收录 136 个节点；全站现有 196 篇公开研究笔记</h3>'
        '<p>累计回顾现分 55 个阶段，完整保留 R0.61–R0.73T；最新节点分开记录动态一侧估计、'
        '临界 A 预算、精确非自治见证、压力张量障碍、有限公式诊断和正式附图。</p>'
        '<p>R0.70A–R0.73T 共 98 个版本已公开；74 个按当前 formal-figure 合同完整封存，'
        '24 个旧版附图档案仍列入回补清单。</p>'
        f'<p><strong>阶段判断：</strong>&nbsp;{html.escape(content.recap_zh)} '
        '任意三维初值全局正则性与 Clay 保持 OPEN。</p>'
        '<p><a href="/recap-r0-61-r0-73t.html"><strong>阅读 R0.60 之后的完整累计回顾 →</strong></a> · '
        '<a href="/recap-r0-61-r0-73t.pdf">下载同步 PDF</a></p></div>'
    )
    value = replace_regex(
    ...
    for stale in (
        "Research topology · R0.1–R0.73S",
        "累计回顾收录 135 个节点；全站现有 195 篇公开研究笔记",
        '<summary>展开 105 篇公开笔记</summary>',
        "<strong>v1.59</strong>网页版本",
        "<strong>R0.73S</strong>最新研究节点",
        'data-site-version="1.59"',
        "/site-refresh.js?v=1.59",
        '<h3>R0.73T 下一接口</h3>',
    ):
        if stale in value:
            raise RuntimeError("R0.73T home retained stale latest marker " + stale)
    assert_public_html(value, "R0.73T home")
    return value
...
def update_literature(content: ReleaseContent) -> str:
    ...
    value = value.replace("/i18n-en.js?v=1.59", "/i18n-en.js?v=1.60")
    value = value.replace("文献综述 v1.59 ·", "文献综述 v1.60 ·")
    marker = '<span class="route-r073s-deck-update">'
    start = value.find(marker)
    end = value.find("</span>", start)
    if start < 0 or end < 0:
        raise RuntimeError("literature R0.73S deck marker missing")
    end += len("</span>")
    value = value[:end] + content.literature_update + value[end:]
    value = value.replace("/recap-r0-61-r0-73s.html", "/recap-r0-61-r0-73t.html")
    value = value.replace("135 节", "136 节")
    value = value.replace("R0.69P–R0.73S", "R0.69P–R0.73T")
    route_step = (
        '<div class="route-step kept"><header><b>R0.73T</b>'
        '<strong>dynamic autocorrelation and the pressure-tensor barrier</strong></header>'
        f'<p>{html.escape(content.recap_zh)} <a href="/notes/r0-73t.html">研究笔记</a> '
        '<a href="/recap-r0-61-r0-73t.html">当前累计回顾</a> '
        '<a href="#r073t-boundary">文献边界</a></p></div>'
        '<div class="route-step pause"><header><b>开放接口 · R0.73U</b>'
        '<strong>tensor heat hierarchy and critical signed flux</strong></header>'
        '<p>下一步检查带尺度张量 heat hierarchy 能否保留压力信息并吸收带符号通量；'
        '任意初值全局正则性与 Clay 保持 OPEN。</p></div>'
    )
    value = replace_regex(
        value,
        r'<div class="route-step pause"><header><b>开放接口 · R0\.73T</b>.*?</div>',
        route_step, "literature current route step",
    )
    literature_boundary = (
        '<h3 id="r073t-boundary">R0.73T 的 L4 压力机制、壳层强制性与 Wiener 邻近文献</h3>'
        '<p><a href="https://doi.org/10.1017/jfm.2020.1033">Tran--Yu--Dritschel 2021</a>'
        '是经典 L4 平衡和压力相关机制的直接公式层碰撞；'
        '<a href="https://doi.org/10.1090/tran/8708">Li--Sire 2023</a> 的 Theorem 4.2 '
        '承担标量频率局部化强制性，本站的向量壳结论来自逐分量适配。</p>'
        '<p><a href="https://doi.org/10.1090/proc/16615">Ambrose--Lopes Filho--Nussenzveig Lopes 2024</a>'
        '控制速度 Fourier/Wiener 量，而不是能量密度状态 C。限定式检索未找到相同打包，'
        '但未检出不是新颖性、优先权或不存在证明。</p>'
        '<div class="boundary"><strong>R0.73T 的主张边界</strong>'
        f'<p>{html.escape(CLOSED_LEDGER)}</p>'
        f'<p>{html.escape(FINITE_LEDGER)}</p>'
        f'<p>{html.escape(OPEN_LEDGER)}</p>'
        '<p>一手来源承担经典碰撞；本地解析证明只承担一侧 AQ 推论与精确 no-go；'
        '有限包只复算公式诊断，不认证连续 PDE。NOT CLAY。</p></div>'
    )
    value = replace_once(
        value, '          <ol class="criteria">',
        "          " + literature_boundary + '\n          <ol class="criteria">',
        "literature R0.73T boundary",
    )
    if "开放接口 · R0.73U" not in value or 'id="r073t-boundary"' not in value:
        raise RuntimeError("R0.73T literature route/boundary was not advanced")
    assert_public_html(value, "R0.73T literature")
    for token in ("VERIFIED_CLASSICAL", "INTERNAL_COROLLARY", "优先权"):
        if token not in value:
            raise RuntimeError("R0.73T literature lost boundary token " + token)
    return value
...
def build_manifest_outputs(content: ReleaseContent) -> dict[Path, bytes]:
    ...
    for key, expected in R073S_BASELINE.items():
        if release.get(key) != expected:
            raise RuntimeError("R0.73S release-manifest baseline drift: " + key)
    release.update({
        **R073T_TARGET,
        "latestReleaseGate": "tests/r073t-dynamic-autocorrelation-gate.test.mjs",
        "latestReleasePublicationTest": "tests/r073t-release.test.mjs",
    })
    ...
    expected_site = {
        ...
        "version": "1.59",
        "latestRelease": "R0.73S",
        "publicHtmlNoteCount": 195,
        "publishedDate": "2026-08-31",
    }
    if site != expected_site:
        raise RuntimeError("R0.73S site-version baseline drift")
    site.update({
    ...
    if state != ("r073s", 97, 73, 24):
        raise RuntimeError("R0.73S formal-archive baseline drift")
    for key in ("publishedReleases", "formalSealedReleases"):
        rows = inventory.get(key)
        if not isinstance(rows, list) or rows[-1:] != ["r073s"] or "r073t" in rows:
            raise RuntimeError("formal archive is not append-only: " + key)
        rows.append("r073t")
    inventory.update({
        "latestPublishedRelease": "r073t",
        "publishedReleaseCount": 98,
        "formalSealedReleaseCount": 74,
...
def build_note_index(content: ReleaseContent, site_payload: bytes) -> str:
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="r073t-index-") as temporary:
            ...
            if len(existing) != 195 or existing[0].slug != "r0-73s":
                raise RuntimeError("R0.73T note-index baseline is not exact")
            ...
            note_index.latest_recap_href = lambda: "/recap-r0-61-r0-73t.html"
    ...
    assert_public_html(value, "R0.73T note index", require_boundary=False)
    if "prefers-color-scheme: dark" not in value:
        raise RuntimeError("R0.73T index lost automatic dark theme")
    return value


def formal_figure_payloads(source: Path) -> dict[str, bytes]:
    source_manifest = strict_json_file(
        f"{FIGURE_SOURCE_RELATIVE}/manifest.json", "R0.73T sealed figure manifest"
    )
    if (
        ...
        or source_manifest.get("allChecksPass") is not True
        ...
        or source_manifest.get("validationCheckCount") != 106
        or source_manifest.get("inventory", {}).get("packageFileCount") != 25
        or source_manifest.get("inventory", {}).get("manifestBoundFileCount") != 23
        or source_manifest.get("inventory", {}).get("sha256SumsLineCount") != 24
        or source_manifest.get("inventory", {}).get("sourceFileCount") != 10
        or source_manifest.get("inventory", {}).get("rawFileCount") != 11
    ):
        raise RuntimeError("R0.73T source figure is not a commit-bound sealed package")
    actual = sorted(
    ...
    if not required.issubset(actual):
        raise RuntimeError(
            "R0.73T formal figure inventory is incomplete: "
            + repr(sorted(required - set(actual)))
        )
    recorded = source_manifest.get("files")
    if not isinstance(recorded, list):
        raise RuntimeError("R0.73T sealed figure file inventory is absent")
    ...
    if set(by_name) != expected_recorded:
        raise RuntimeError("R0.73T sealed figure inventory does not cover package files")
    for name, row in by_name.items():
        payload = current_regular_bytes(f"{FIGURE_SOURCE_RELATIVE}/{name}")
        if row.get("bytes") != len(payload) or row.get("sha256") != sha256(payload):
            raise RuntimeError("R0.73T sealed figure record is not byte-bound: " + name)
    ...
    for suffix in ("pdf", "svg", "png"):
        ...
        payload = current_regular_bytes(f"{FIGURE_SOURCE_RELATIVE}/{name}")
        if by_name.get(name, {}).get("sha256") != sha256(payload):
            raise RuntimeError("R0.73T figure source seal lost " + name)
        row: dict[str, object] = {
    ...
    public_assets = [
        {
            "path": f"public/assets/r073t/{FIGURE_ID}.{suffix}",
    ...
    formal = {
        ...
        "analyticalQuestion": (
            "Which scalar-autocorrelation statement survives the dynamics, and "
            "which missing variables prevent exact closure?"
        ),
        "supportedClaim": (
            "The static AQ certificate yields a one-sided L4 differential "
            "inequality, while exact witnesses expose carrier and signed-phase loss."
        ),
        ...
        "data": [
            record("source-data.csv", "r073t-dynamic-autocorrelation-figure-source-v1"),
            record("results.json", "r073t-dynamic-autocorrelation-figure-results-v1"),
            record("validation.json", "r073t-dynamic-autocorrelation-figure-validation-v1"),
    ...
    if sorted(payloads) != actual:
        raise RuntimeError("R0.73T formal figure staging changed the package inventory")
    return payloads
...
def stage_figure_assets(staged: dict[Path, bytes]) -> None:
    ...
    if not source.is_dir() or source.is_symlink() or not (source / "manifest.json").is_file():
        raise RuntimeError("R0.73T formal figure package is incomplete")
    ...
    for suffix in ("pdf", "svg", "png"):
        staged[PUBLIC / f"assets/r073t/{FIGURE_ID}.{suffix}"] = payloads[f"figure.{suffix}"]


def build_staged(content: ReleaseContent) -> dict[Path, bytes]:
    staged: dict[Path, bytes] = {}
    stage_figure_assets(staged)
    staged[PUBLIC / "notes/r0-73t.html"] = build_note(content).encode("utf-8")
    staged[PUBLIC / "recap-r0-61-r0-73t.html"] = build_recap(content).encode("utf-8")
    ...
    if missing:
        raise RuntimeError("R0.73T staged transaction omitted: " + repr(missing))
...
def commit_transaction(staged: dict[Path, bytes]) -> None:
    ...
    try:
        for index, target in enumerate(ordered):
            ...
            temporary = target.parent / f".{target.name}.r073t-{nonce}-{index}.tmp"
            backup = target.parent / f".{target.name}.r073t-{nonce}-{index}.bak"
...
def source_dry_run() -> dict[str, object]:
    ...
    return {
        ...
        "targetAccounting": R073T_TARGET,
        "baselineAccounting": R073S_BASELINE,
...
def verify_release_inputs() -> ReleaseContent:
    ...
    verify_pinned_paths_exist(RELEASE_BASELINE_COMMIT, BASELINE_EXACT_PATHS, "R0.73S baseline")
    verify_commit_paths(ANALYTIC_SOURCE_COMMIT, ANALYTIC_EXACT_PATHS, "R0.73T analytic source")
    verify_commit_trees(FINITE_PACKAGE_COMMIT, FINITE_EXACT_ROOTS, "R0.73T finite package")
    verify_commit_trees(FIGURE_PACKAGE_COMMIT, FIGURE_EXACT_ROOTS, "R0.73T formal figure package")
    verify_commit_paths(FINAL_CONTENT_COMMIT, FINAL_CONTENT_EXACT_PATHS, "R0.73T final content")
    verify_commit_paths(RELEASE_SOURCE_COMMIT, RELEASE_SOURCE_EXACT_PATHS, "R0.73T release source")
    ...
    if not content.publication_ready:
        raise RuntimeError("R0.73T canonical content is not publication-ready: " + repr(content.readiness_failures))
    if certificate_summary().get("finalSeal") is not True:
        raise RuntimeError("R0.73T finite certificate lacks its final immutable-source seal")
    if figure_summary().get("formal") is not True:
        raise RuntimeError("R0.73T formal figure package is absent or unsealed")
    if not PUBLIC_TRANSACTION_IMPLEMENTED:
        raise RuntimeError("R0.73T public transaction implementation is disabled")
    return content


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Validate or explicitly apply the fail-closed R0.73T release transaction."
    )
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit("R0.73T canonical-source gate failed: " + str(exc)) from exc
