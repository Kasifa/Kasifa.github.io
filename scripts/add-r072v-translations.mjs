...
const root = resolve(process.env.R072V_RELEASE_ROOT ?? resolve(import.meta.dirname, ".."));
const publicDirectory = resolve(root, "public");
const translationPath = resolve(root, "translations/en.json");
const bundlePath = resolve(publicDirectory, "i18n-en.js");
const snapshotPath = resolve(root, "scripts/i18n-snapshots/r072v-missing.json");
...
for (const relative of activePages) {
  ...
  if (!html.includes('/i18n-en.js?v=1.35')) {
    throw new Error(`${relative}: expected i18n cache version v1.35`);
  }
}

const translations = JSON.parse(await readFile(translationPath, "utf8"));
const retained = translations.filter((entry) => !/^r072v\d+$/.test(entry.id));
const retainedByChinese = new Map(retained.map((entry) => [entry.zh, entry]));
if (retainedByChinese.size !== retained.length) {
  throw new Error("Duplicate Chinese keys outside the R0.72V batch");
}
...
if (JSON.stringify(missingFiles) !== JSON.stringify(activePages)) {
  throw new Error(`Unexpected R0.72V missing-string files: ${JSON.stringify(missingFiles)}`);
}

const snapshot = JSON.parse(await readFile(snapshotPath, "utf8"));
if (!Array.isArray(snapshot) || snapshot.length === 0) throw new Error("Empty R0.72V snapshot");
...
if (JSON.stringify(snapshotChinese) !== JSON.stringify(liveChinese)) {
  ...
  throw new Error(`R0.72V missing-string snapshot drift at row ${index + 1}`);
}
const englishByChinese = new Map(snapshot.map((entry) => [entry.zh, entry.en]));
if (englishByChinese.size !== snapshot.length) {
  throw new Error("Duplicate R0.72V snapshot keys");
}
...
if (!longRoute || !previousRoute) {
  throw new Error("Missing cumulative R0.72V literature-route binding");
}
const routeTail = " General Navier–Stokes regularity remains open.";
if (!previousRoute.en.endsWith(routeTail)) {
  throw new Error("Unexpected retained R0.72U literature-route tail");
}
englishByChinese.set(
  longRoute.zh,
  previousRoute.en.slice(0, -routeTail.length) +
    " R0.72V then globalizes coefficient-uniform unit charts through a nonhomogeneous H^-1 direct sum, closing whole-line graph coercivity for the exact cubic linear scalar model; a separately constructed all-L2 energy evolution yields observability for actual solutions and fixed-block contraction. Uniformity as T downarrow 0 is false, while H5/H7/R9 remainder stability, periodic transfer, nonlinear closure, and Clay remain open." +
    routeTail,
);
...
const translatedEntries = missing.map((entry, index) => {
  ...
  if (!en || en === "__DERIVED_FROM_R072U_ROUTE__" || containsChinese(en)) {
    throw new Error(`Invalid R0.72V English row: ${entry.zh}`);
  }
  if (/\b(?:we|our|ours|ourselves|us)\b/i.test(en)) {
    throw new Error(`Plural voice in R0.72V English row: ${entry.zh}`);
  }
  if (JSON.stringify(extractProtectedTokens(en)) !== JSON.stringify(extractProtectedTokens(entry.zh))) {
    throw new Error(`Protected-token mismatch for R0.72V row: ${entry.zh}`);
  }
  if (JSON.stringify(boundaryTokens(en)) !== JSON.stringify(boundaryTokens(entry.zh))) {
    throw new Error(`CLOSED/OPEN/FALSE boundary drift for R0.72V row: ${entry.zh}`);
  }
  return { ...entry, id: `r072v${String(index + 1).padStart(3, "0")}`, en };
});
...
if (checkOnly) {
  if (JSON.stringify(translations) !== JSON.stringify(finalTranslations)) {
    throw new Error("R0.72V translations are stale");
  }
  if (await readFile(bundlePath, "utf8") !== bundleOutput) {
    throw new Error("R0.72V bundle is stale");
  }
} else {
  await Promise.all([
