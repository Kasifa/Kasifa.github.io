...
NOTE_ARTICLE = r'''      <article>
        ...
        <section id="finite"><div class="section-no">09 / Finite diagnostic</div><h2>四组 Fourier cutoff 只做复算和附图，不承担 continuum theorem</h2><div class="equation result">\[\lambda_{0,128}=0.170407976920433,\quad \lambda_{10^{-2},128}=0.156316407014908,\quad \lambda_{10^{-4},128}=0.170261005247709,\quad \lambda_{10^{-6},128}=0.170406506600202,\quad \lambda_{10^{-8},128}=0.170407962217171.\]</div><div class="equation result">\[\|P_{10^{-8},128}-P_{0,128}\|=3.09377\times10^{-6},\qquad \|P_{10^{-8},128}\|=1.68350337.\]</div><p>在 \(N=96,128\) 间，特征值最大差为 \(2.83\times10^{-15}\)；\(N=128\) 最大嵌入残差为 \(6.46\times10^{-15}\)，全 cutoff 最大残差为 \(1.0294\times10^{-6}\)。独立程序逐项重算后误差为零，但两条路径仍使用同一 NumPy/SciPy binary64 体系，不是 interval 或 infinite-tail certificate。独立解析审计另外纠正了 \(X\) 必须按 completion 定义、\(D(H_0)=L^2\) 不能写成 \(H^2\)，并确认 strong resolvent 本身不足以推出投影范数收敛。所有有限数据继续标为 finite diagnostic only。</p></section>
        <section id="literature"><div class="section-no">10 / Literature boundary</div><h2>一般的无黏到黏性不稳定谱持续性已有先例</h2><p><a href="https://doi.org/10.1016/j.anihpc.2007.05.004">Shvydkoy--Friedlander（2008）</a>已证明一般周期情形的谱持续、代数重数和 Riesz 谱子空间结论；<a href="https://www.numdam.org/articles/10.1016/j.anihpc.2007.05.004/">NUMDAM 全文</a>和<a href="https://arxiv.org/abs/math/0509538">arXiv 版本</a>均可核查。Li（2005）给出周期 Kolmogorov-flow 例子；Li--Lin（2011）处理 no-slip channel 的 Orr--Sommerfeld 延拓。本节的贡献是一条 explicit double-harmonic Fourier 行在精确 kinetic space 中的自包含证明，并把投影算子范数论证完整写出；不作一般首创、优先权或严格强化声明。</p></section>
        ...
        <section id="boundary"><div class="section-no">13 / Exact boundary</div><h2>固定谱簇不是完整稳定性理论</h2><p>inviscidRootUnique=OPEN；inviscidEigenvalueSimple=OPEN；quantitativeEigenvalueRate=OPEN；globalRightHalfPlaneNoPollution=OPEN；movingProfileUniformContour=OPEN；uniformComplementaryDichotomy=OPEN；logFastTimeTransfer=OPEN；nonlinearNavierStokes=OPEN；Clay=OPEN。</p></section>
...
HOME_D_CARD = r'''          <div class="task-one" id="r073d" data-release="r073d" style="margin-top:2rem">
            ...
            <p><strong>结论边界：</strong>&nbsp;static persistence、Riesz norm convergence 与 cluster multiplicity 为 CLOSED；simplicity、rate、complement dichotomy、fast time、nonlinearNavierStokes 与 Clay 保持 OPEN。</p>
