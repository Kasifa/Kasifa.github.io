...
const root = resolve(process.env.R073E_RELEASE_ROOT ?? resolve(import.meta.dirname, ".."));
const publicDirectory = resolve(root, "public");
const translationPath = resolve(root, "translations/en.json");
const bundlePath = resolve(publicDirectory, "i18n-en.js");
const snapshotPath = resolve(root, "scripts/i18n-snapshots/r073e-missing.json");
...
for (const relative of activePages) {
  ...
  if (!html.includes('/i18n-en.js?v=1.45')) {
    throw new Error(relative + ": expected i18n cache version v1.45");
  }
}

const translations = JSON.parse(await readFile(translationPath, "utf8"));
const retained = translations.filter((entry) => !/^r073e\d+$/.test(entry.id));
const retainedByChinese = new Map(retained.map((entry) => [entry.zh, entry]));
if (retainedByChinese.size !== retained.length) throw new Error("Duplicate Chinese keys outside the R0.73E batch");
...
if (JSON.stringify(missingFiles) !== JSON.stringify(activePages)) {
  throw new Error("Unexpected R0.73E missing-string files: " + JSON.stringify(missingFiles));
}

if (captureMissing) {
  const skeleton = missing.map(({ zh }) => ({ zh, en: "" }));
  await writeFile(snapshotPath, JSON.stringify(skeleton, null, 2) + "\n");
  console.log(JSON.stringify({ captured: skeleton.length, snapshot: "scripts/i18n-snapshots/r073e-missing.json" }));
  process.exit(0);
}

const snapshot = JSON.parse(await readFile(snapshotPath, "utf8"));
if (!Array.isArray(snapshot) || snapshot.length === 0) throw new Error("Empty R0.73E snapshot");
...
if (JSON.stringify(snapshotChinese) !== JSON.stringify(liveChinese)) {
  ...
  throw new Error("R0.73E missing-string snapshot drift at row " + String(index + 1));
}
const englishByChinese = new Map(snapshot.map((entry) => [entry.zh, entry.en]));
if (englishByChinese.size !== snapshot.length) throw new Error("Duplicate R0.73E snapshot keys");

const boundaryTokens = (value) => value.match(/\b(?:CLOSED|OPEN|FALSE|CONDITIONAL)\b/g) ?? [];
const translatedEntries = missing.map((entry, index) => {
  const en = englishByChinese.get(entry.zh)?.trim();
  if (!en || containsChinese(en)) throw new Error("Invalid R0.73E English row: " + entry.zh);
  if (/\b(?:we|our|ours|ourselves|us)\b/i.test(en)) throw new Error("Plural voice in R0.73E English row: " + entry.zh);
  if (JSON.stringify(extractProtectedTokens(en)) !== JSON.stringify(extractProtectedTokens(entry.zh))) {
    throw new Error("Protected-token mismatch for R0.73E row: " + entry.zh);
  }
  if (JSON.stringify(boundaryTokens(en)) !== JSON.stringify(boundaryTokens(entry.zh))) {
    throw new Error("Claim-state boundary drift for R0.73E row: " + entry.zh);
  }
  return { ...entry, id: "r073e" + String(index + 1).padStart(3, "0"), en };
});
...
if (checkOnly) {
  if (JSON.stringify(translations) !== JSON.stringify(finalTranslations)) throw new Error("R0.73E translations are stale");
  if (await readFile(bundlePath, "utf8") !== bundleOutput) throw new Error("R0.73E bundle is stale");
} else {
  await Promise.all([writeFile(translationPath, jsonOutput), writeFile(bundlePath, bundleOutput)]);
}
console.log(JSON.stringify({ checkOnly, added: translatedEntries.length, total: finalTranslations.length, liveStrings: source.length, missingAfter: stillMissing.length, bundle: "public/i18n-en.js" }));
