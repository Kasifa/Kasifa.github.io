...
const baseUrl = new URL(process.env.R074T_PUBLIC_BASE_URL ?? "https://kasifa.github.io/");
const commit = process.env.R074T_DEPLOYED_COMMIT ?? "unversioned";
const figureId = "fig-r074t-schedule-invariant-dwell-barrier";
const publicFigureRoot = `public/figures/r074t/${figureId}`;
const sha256 = (bytes) => createHash("sha256").update(bytes).digest("hex");

const fixedObjects = [
  ["public/notes/r0-74t.html", "/notes/r0-74t.html"],
  ["public/notes/r0-74t.pdf", "/notes/r0-74t.pdf"],
  ...
  [`public/assets/r074t/${figureId}.pdf`, `/assets/r074t/${figureId}.pdf`],
  [`public/assets/r074t/${figureId}.png`, `/assets/r074t/${figureId}.png`],
  [`public/assets/r074t/${figureId}.svg`, `/assets/r074t/${figureId}.svg`],
];

const figureNames = execFileSync(
  "git",
  ["ls-tree", "-r", "--name-only", "HEAD", `research/figures/r074t/${figureId}`],
...
const objects = [
  ...
  ...figureNames.map((name) => [
    ...
    `/figures/r074t/${figureId}/${encodeURIComponent(name)}`,
  ]),
];
assert.equal(objects.length, 37, "Step 19 online publication inventory");
