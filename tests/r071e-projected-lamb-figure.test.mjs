...
test("archives the formal R0.71E projected-Lamb trace figure", async () => {
  ...
  assert.equal(manifest.release, "R0.71E");
  ...
  assert.match(manifest.claimBoundary, /No bottom-trace integrability/);
  assert.equal(Object.keys(validation.checks).length, 25);
  assert.ok(Object.values(validation.checks).every(Boolean));
  assert.equal(Object.keys(independent.checks).length, 16);
  assert.ok(Object.values(independent.checks).every(Boolean));
  assert.equal(data.trim().split("\n").length - 1, 522);
  assert.match(contract, /bottom face \(s=0\)|bottom trace/i);
  assert.match(caption, /2K\^2/);
  assert.match(caption, /not DNS/);
  assert.match(environment, /DGX: not used/);
});

test("keeps every archived R0.71E figure payload SHA-256 exact", async () => {
...
test("publishes byte-exact web copies of every R0.71E figure format", async () => {
  for (const extension of ["svg", "pdf", "png"]) {
    const [archived, published] = await Promise.all([
      readFile(new URL("figure." + extension, figureRoot)),
      readFile(
        new URL(
          "public/figures/r0-71e-projected-lamb-trace." + extension,
          root,
        ),
      ),
    ]);
