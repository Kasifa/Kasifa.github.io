...
const outputRoot = resolve(process.env.R075O_STEP40_BROWSER_QA_OUTPUT ?? "/tmp/r075o-step40-browser-qa");
const externalBase = process.env.R075O_PUBLIC_BASE_URL;
...
try {
  for (const scenario of scenarios) {
    ...
    const url = new URL("/notes/r0-75o.html", baseUrl);
    url.searchParams.set("lang", scenario.lang);
    url.searchParams.set("qa", "r075o-step40");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        step40MathJaxErrors: document.querySelectorAll("#s-315 mjx-merror, #s-316 mjx-merror, #s-317 mjx-merror, #s-318 mjx-merror, #s-319 mjx-merror, #s-320 mjx-merror").length,
        ...
        figureSections: document.querySelectorAll('#figure, [src*="r075o"], [href*="assets/r075o"]').length,
        constantShear: /CONSTANT SHEAR/.test(visibleText),
        verticalDiffusion: /VERTICAL DIFFUSION/.test(visibleText),
        arbitraryVertical: /ARBITRARY VERTICAL FREQUENCIES/.test(visibleText),
        diagonalFirst: /DIAGONAL REMOVED FIRST/.test(visibleText),
        schurQuarter: /SCHUR 1\/4/.test(visibleText),
        totalFrequencyCap: /TOTAL-FREQUENCY CAP/.test(visibleText),
        realPacket: /REAL PACKET/.test(visibleText),
        shortClock: /K\^2 T >= 1/.test(visibleText),
        packetGain: /K\^-2\/3 GAIN/.test(visibleText),
        strictThreshold: /KAPPA > 98605\/71442/.test(visibleText),
        frozenKappa: /KAPPA = 3\/2 CLOSES/.test(visibleText),
        fullT2Only: /FULL-T2 ATOM ONLY/.test(visibleText),
        versionMPaymentOpen: /VERSION-M PAYMENT OPEN/.test(visibleText),
    ...
    assert.equal(audit.step40MathJaxErrors, 0, `${scenario.id} Step 40 MathJax errors: ${JSON.stringify(audit.mathJaxErrorContexts.filter((entry) => /^s-3(15|16|17|18|19|20)$/.test(entry.section ?? "")))}`);
    ...
    assert.equal(audit.figureSections, 0, `${scenario.id} no R0.75O formal figure`);
    for (const key of [
      "constantShear", "verticalDiffusion", "arbitraryVertical", "diagonalFirst", "schurQuarter",
      "totalFrequencyCap", "realPacket", "shortClock", "packetGain", "strictThreshold",
      "frozenKappa", "fullT2Only", "versionMPaymentOpen", "e24Open",
