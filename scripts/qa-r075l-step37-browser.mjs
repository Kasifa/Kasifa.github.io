...
const outputRoot = resolve(process.env.R075L_STEP37_BROWSER_QA_OUTPUT ?? "/tmp/r075l-step37-browser-qa");
const externalBase = process.env.R075L_PUBLIC_BASE_URL;
...
try {
  for (const scenario of scenarios) {
    ...
    const url = new URL("/notes/r0-75l.html", baseUrl);
    url.searchParams.set("lang", scenario.lang);
    url.searchParams.set("qa", "r075l-step37");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        figureSections: document.querySelectorAll('#figure, [src*="r075l"], [href*="assets/r075l"]').length,
        passiveHarmonic: /EXACT PASSIVE HARMONIC/.test(visibleText),
        physicalSignedFlux: /PHYSICAL SIGNED FLUX/.test(visibleText),
        zeroModeCanceled: /ZERO MODE CANCELED/.test(visibleText),
        onlyTwoK: /ONLY \+\/-2k/.test(visibleText),
        fluxDecay: /FLUX k\^-2/.test(visibleText),
        cubicMass: /CUBIC MASS k\^-2/.test(visibleText),
        diffusiveGain: /GAIN k\^-2\/3/.test(visibleText),
        fullTorusAtom: /FULL-TORUS ATOM/.test(visibleText),
        singleHarmonicOnly: /SINGLE HARMONIC ONLY/.test(visibleText),
        coefficientUnpaid: /\|B\|V_XI UNPAID/.test(visibleText),
    ...
    assert.equal(audit.figureSections, 0, `${scenario.id} no R0.75L formal figure`);
    for (const key of [
      "passiveHarmonic", "physicalSignedFlux", "zeroModeCanceled", "onlyTwoK", "fluxDecay",
      "cubicMass", "diffusiveGain", "fullTorusAtom", "singleHarmonicOnly", "coefficientUnpaid",
      "e24Open", "noveltyBoundary", "clayBoundary", "noFigureBoundary",
