#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Canonical-source reader and UI fragments for the fail-closed R0.73P release.
...
FIGURE_SOURCE_RELATIVE = f"research/figures/r073p/{FIGURE_ID}"
FIGURE_ARCHIVE_RELATIVE = f"figures/r073p/{FIGURE_ID}"
...
REPORT_SOURCE = "research/r073p_report-source.md"
DICTIONARY_SOURCE = "research/r073p_bilingual_dictionary.md"
...
ALLOWED_PUBLICATION_STATES = {
    ...
    "formulaDiagnosticPackage": frozenset(("PRESEAL_PENDING", "PRESEAL_PASS", "CLOSED")),
    "sourceCommitAssigned": frozenset(("FALSE", "TRUE")),
    "finalSeal": frozenset(("FALSE", "TRUE")),
    "formalFigurePackage": frozenset(("PRESEAL_PENDING", "PRESEAL_PASS", "PASS")),
...
class ReleaseContent:
    ...
    def math_next_gate_zh(self) -> str:
        """Keep the mathematical R0.73Q gate, without the pre-deploy handoff."""
        marker = f"数学上的下一发布门是 {self.next_release}："
        if marker not in self.next_gate_zh:
            raise CanonicalSourceError("R0.73P mathematical next-gate marker drift")
    ...
    def note_hero(self) -> str:
        return (
            ...
            '      <div><div class="eyebrow">研究笔记 R0.73P · '
            'CRITICAL STABILITY / FREQUENCY GATE</div>\n'
            f'        <h1>{html.escape(self.public_title_zh)}</h1>\n'
            f'        <p class="lead">{html.escape(self.lead_zh)}</p></div>\n'
            '      <div class="stamp"><span class="state">状态 · R0.73P 完成</span>'
            '<strong>Classical critical corollary + exact frequency transfer</strong>'
            f'<p>版本 v0.73P · {html.escape(self.date)}</p>'
            '<p>critical H1/2 orbit tube：CLASSICAL COROLLARY</p>'
            '<p>band-limited L2 gate：N^-1/2 COROLLARY</p>'
            '<p>delayed one-sided L2→H3：CLOSED</p>'
            '<p>L2-only / early weak interval / Clay：OPEN</p><p>NOT CLAY</p></div>\n'
            '    </div></header>'
        )
    ...
    def note_article(self) -> str:
        ...
        boundary = (
            ...
            '<h2>临界强稳定、带限推论与早期弱缺口严格分列</h2>'
            f'<p>{html.escape(self.closed_ledger)}</p>'
            f'<p>{html.escape(self.finite_ledger)}</p>'
            f'<p>{html.escape(self.open_ledger)}</p>'
            '<p>临界稳定管是已发表鲁棒性定理的经典推论；N^-1/2 只对范数转移锐。'
            '解析附图不证明 PDE 必要阈值；无频率条件的 L2-only 初时刻正则、'
            '早期弱区间与 Clay 仍开放。NOT CLAY。</p></section>'
        )
        figure = (
            '        <section id="figure"><div class="section-no">F / Journal figure</div>'
            '<h2>临界频率门槛、Sobolev 指数区与线性热基准</h2>'
            f'<p><img src="/assets/r073p/{FIGURE_ID}.svg" '
            'alt="R0.73P critical frequency gate and linear heat benchmark"></p>'
            f'<p><a href="/assets/r073p/{FIGURE_ID}.pdf">下载矢量 PDF</a> · '
            f'<a href="/assets/r073p/{FIGURE_ID}.png">下载 600 dpi PNG</a> · '
            f'<a href="/assets/r073p/{FIGURE_ID}.svg">打开 SVG</a></p>'
            '<p>附图只呈现 N^-3 与 N^-1/2 的解析换算、纯模 Sobolev 幂律和'
            '线性热半群基准。它不认证非线性入口、PDE 必要阈值、奇性或 Clay。</p></section>'
        )
        reproduction = (
            ...
            'research/r073p_critical_frequency_proof.md">critical-frequency proof</a> · '
            '<a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/'
            'research/r073p_delayed_synchronization_proof.md">delayed synchronization proof</a> · '
            '<a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/'
            'research/r073p_literature_audit.md">bounded literature audit</a> · '
            '<a href="https://github.com/Kasifa/Kasifa.github.io/tree/main/'
            'research/certificates/r073p">finite diagnostic package</a></p>'
            f'<p><a href="/assets/r073p/{FIGURE_ID}.pdf">journal figure PDF</a> · '
            '<a href="/notes/r0-73p.pdf">synchronized note PDF</a> · '
            '<a href="/recap-r0-61-r0-73p.html">132-node cumulative recap</a> · '
            '<a href="/recap-r0-61-r0-73p.pdf">synchronized recap PDF</a></p>'
            '</section>'
        )
        return (
            "      <article>\n" + body + "\n" + figure + "\n" + boundary
    ...
    def home_card(self) -> str:
        return (
            '          <div class="task-one" id="r073p" data-release="r073p" '
            'style="margin-top:2rem">\n'
            f'            <p class="eyebrow">研究笔记 R0.73P · {html.escape(self.date)}</p>'
            ...
            'NOT CLAY。</p>\n'
            '            <p><a href="/notes/r0-73p.html"><strong>阅读 R0.73P '
            '研究笔记 →</strong></a><br><a href="/notes/r0-73p.pdf">下载同步 PDF</a> · '
            f'<a href="/assets/r073p/{FIGURE_ID}.pdf">下载期刊附图 PDF</a> · '
            '<a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/'
            'research/r073p_critical_frequency_proof.md">查看连续证明</a> · '
            '<a href="/recap-r0-61-r0-73p.html">打开累计回顾</a></p>\n'
    ...
    def latest_spotlight(self) -> str:
        return (
            ...
            f'        <div><p class="eyebrow">LATEST RELEASE · R0.73P · '
            ...
            '<a class="route-map-latest" href="/notes/r0-73p.pdf">阅读最新 R0.73P '
            '研究笔记 →</a><a href="/recap-r0-61-r0-73p.html">132 节累计回顾</a>'
            '<a href="/notes/">192 篇研究笔记总索引</a>'
            '<a href="#r073p">查看首页完整 R0.73P 卡片</a></nav>\n'
            '      </header><div class="route-legend" aria-label="最新发布计数">'
            '<span><i class="route-legend-mark kept" aria-hidden="true"></i>'
            'R0.70A–R0.73P · 94 节已公开</span>'
            '<span><i class="route-legend-mark kept" aria-hidden="true"></i>'
            '70 节完整封存</span><span><i class="route-legend-mark current" '
            'aria-hidden="true"></i>当前端点 R0.73P</span></div></div>\n'
            '    </section>'
        )
    ...
    def recap_phase(self) -> str:
        return (
            f'            <article class="phase"><h3>R0.73P · '
            ...
            '<div class="links"><a href="/notes/r0-73p.html">R0.73P</a>'
            f'<a href="/assets/r073p/{FIGURE_ID}.pdf">R0.73P 附图</a>'
            '<a href="https://github.com/Kasifa/Kasifa.github.io/tree/main/'
            'research/certificates/r073p">R0.73P 有限诊断包</a></div></article>'
        )

    @property
    def literature_update(self) -> str:
        return (
            '<span class="route-r073p-deck-update">'
...
def _parse_sections(report: str) -> tuple[ReportSection, ...]:
    ...
    if not matches:
        raise CanonicalSourceError("R0.73P report source has no level-two sections")
...
def _build_public_report(*, forced_ready: bool) -> str:
    readiness = (
        "临界连续证明已经通过解析复核；有限公式诊断和正式附图仍按独立发布门管理。"
        if forced_ready else
        "临界连续证明仍在复核；发布生成器保持关闭。"
    )
    return r"""# R0.73P | Critical stability, the N^{-1/2} frequency gate, and the early-time regularity gap

## 1. 直接结论

固定一条先验全局周期强轨道，弱比较解满足全时段相对 (L^2) 衰减；
足够小的 (H^{1/2}) 初始差进入全局临界稳定管。对 Fourier 支撑
(|k|le N) 的扰动，这给出充分门槛
[
 |w_0|_2<R_{1/2}[u]N^{-1/2}.
]
该指数只对 Sobolev 范数换算锐利，不是 Navier--Stokes 动力学必要阈值。

## 2. 归一化与临界半径

在 ([0,2pi]^3) 上使用归一化 Haar 测度 ((2pi)^{-3}dx)，令
(|z|_s=|A^{s/2}z|_2)。已发表的周期临界鲁棒性定理在
(alpha=1/2) 时，与有限轨道作用量
[
 mathcal A_c[u]=int_0^infty |u(t)|_1^4,dt
]
组合，给出对每个起始时刻一致的正半径 (R_{1/2}[u])。

## 3. 带限换算与锐性边界

Parseval 恒等式给出
[
 |w_0|_{1/2}le N^{1/2}|w_0|_2,qquad
 |w_0|_3le N^3|w_0|_2.
]
单 Fourier 模能饱和这些幂次，因此 (N^{-1/2}) 是范数转移的精确尺度。
单模可以全局光滑，不能据此推出更大数据发生失稳、奇性或爆破。
...
[
 |w_0|_{1/2}le
 |w_0|_2^{1-1/(2s)}|w_0|_s^{1/(2s)}.
]
另一条可审计入口是
[
 N|w_0|_2^2+|Q_{>N}w_0|_{1/2}^2<R_{1/2}[u]^2.
]
这些条件都含有频率或高阶信息，不是统一的 (L^2)-only 阈值。

## 5. 延迟同步

能量球内的每条 Leray--Hopf 选择在共同上界时间 (T_3(M)) 后进入小
(H^3) 球。相对固定全局强参考轨道，可得到一侧的延迟
(L^2	o H^3) Lipschitz 同步。这不构造任意两条弱分支之间的半群。

## 6. 早期弱区间

最终正则不能向后传播。无频率条件的初时刻强正则、早期弱区间中的唯一性以及
任意三维光滑初值的全局正则性仍然开放。

## 7. 有限诊断边界

正式附图只比较 (N^{-3}) 与 (N^{-1/2}) 的解析幂律、纯模 Sobolev
缩放和线性热半群基准。它不包含非线性 Duhamel 控制，不证明 PDE 阈值必要性，
也不认证奇性或 Clay。
...
`oneSidedDelayedL2ToH3Synchronization=CLOSED_AFTER_AUDIT`；
`uniformL2OnlyStrongThreshold=OPEN_COLLISION_SENSITIVE`；
`earlyWeakIntervalRegularity=OPEN`；`clayConclusion=OPEN`。NOT CLAY。
...
def load_release_content(root: Path | None = None) -> ReleaseContent:
    ...
    analytic_states = {
        key: _invariant_state(combined, key)
    ...
    release_title_match = re.search(
        r"(?m)^\*\*Release title:\*\*\s*\*?(.+?)\*?\s*$", dictionary
    )
    release_title_en = (
        release_title_match.group(1).strip("* ")
        if release_title_match else
    ...
    next_release = (
        ...
        if next_release_match else "R0.73Q"
    )
    ...
    fallback_lead = (
        "固定全局强轨道周围，弱比较解满足 L2 相对衰减；临界 H1/2 小球给出全局强正则，"
        "并把带限数据的充分 L2 门槛从 N^-3 改进到 N^-1/2。"
    )
    fallback_home = (
        "这一步分开弱 L2 相对能量、临界 H1/2 强正则和延迟 H3 同步。"
        "没有频率或高阶包络时，初时刻强正则仍然开放。"
    )
    fallback_recap = (
        "R0.73P 关闭了固定全局强轨道周围的临界稳定与带限输入接口，"
        "并把不能跨越的部分定位到早期弱区间。"
    )
    fallback_literature = (
        "Burczak--Zajaczkowski 与 Marin-Rubio--Robinson--Sadowski 提供临界鲁棒性，"
        "Hoang--Martinez 提供最终 Gevrey 正则；Mucha 2001 的完整量词仍待核对。"
    )
    fallback_next = (
        "数学上的下一发布门是 R0.73Q：检查相对热流、频率包络和临界 Besov 拓扑，"
        "并先完成相关临界空间路线的碰撞审计。"
    )
    ...
    opened = (
        "uniformL2OnlyStrongThreshold=OPEN_COLLISION_SENSITIVE；"
        "earlyWeakIntervalRegularity=OPEN；clayConclusion=OPEN"
    )
    source_sha256 = {
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Read and validate canonical R0.73P release content without writing."
    )
