#!/usr/bin/env python3
"""Create final-size, grayscale, and PDF-raster QA surfaces for R0.72M."""
...
def main() -> None:
    ...
    subprocess.run([
        pdftoppm, "-png", "-f", "1", "-l", "1", "-singlefile", "-r", str(qa_dpi),
        str(ROOT / "figure.pdf"), str(ROOT / "qa-pdf"),
    ], check=True)
    ...
    print(f"R0.72M QA surfaces: {size[0]}x{size[1]} px at {qa_dpi} dpi")
