...
NOTE_HERO = r'''    <header class="hero"><div class="hero-inner">
      <div>
        <div class="eyebrow">研究笔记 R0.72Q · ARBITRARY PHASES · EXACT CAUSTIC</div>
        <h1>固定 \(M\) 的任意相位 shape gate 已闭合；<br>结论仍止于 \(Q_2\le1/2\) 的主谐波锥</h1>
        <p class="lead">对固定有限 \(M\)，任意相对相位与 \(Q_2=\sum_{m=2}^M m^2b_m\le1/2\)，归一化 profile 恰有两个临界点。实际 Coble shear 在 \(0\le y\le1\) 上具有 \((r,\mathfrak C_0,\mathfrak C_1)=(\pi/12,81,36)\) 的统一 shape contract；归一化 \(F\) 保留更尖锐的 away 常数 \(12\)。1:2 退化集由精确 nephroid 参数式给出，phase-uniform 安全圆盘半径恰为 \(1/4\)。</p>
      </div>
      <div class="stamp"><span class="state">状态 · R0.72Q 任意相位固定 M 正类完成</span><strong>phase-robust fixed-M shape gate: CLOSED</strong><p>版本 v0.72Q · 2026-08-28</p><p>fixed \(M\), arbitrary relative phases: CLOSED</p><p>\(Q_2\le1/2\), two critical points: CLOSED</p><p>physical \((r,C_0,C_1)=(\pi/12,81,36)\): CLOSED</p><p>exact 1:2 caustic and disk radius \(1/4\): CLOSED</p><p>growing \(M\) / phases varying in time: OPEN</p><p>general 3D / Clay problem: OPEN</p></div>
    </div></header>'''
...
NOTE_ARTICLE = r'''      <article>
        <section id="result"><div class="section-no">00 / Direct decision</div><h2>相位限制被移除，但有限 carrier ceiling 与二阶矩小量仍是定理条件</h2>
          <div class="verdict-grid">
            <div class="verdict-card true"><strong>THEOREM · ARBITRARY PHASES</strong><p>固定 \(M&lt;\infty\)，所有 relative phases 任意，且 \(Q_2\le1/2\)。</p></div>
        ...
        <section id="normalization"><div class="section-no">01 / Phase quotient</div><h2>全局平移只固定第一谐波，其余相位无需对齐</h2>
          ...
          F_y(\phi)=\cos\phi+\sum_{m=2}^{M}\operatorname{Re}\!\left(\beta_m(y)e^{im\phi}\right),
          \qquad Q_j=\sup_y\sum_{m=2}^{M}m^j|\beta_m(y)|.
          \]</div>
          <p>从 \(Q_2\le1/2\) 逐项得到 \(Q_1\le1/4\) 与 \(Q_0\le1/8\)；推导没有使用同相或反相结构。</p>
        </section>
        ...
        <section id="fixedm"><div class="section-no">04 / Fixed-M ledger</div><h2>前三阶导数与 slow-time 门槛明确依赖固定 \(M\)</h2>
          ...
          \qquad \eta\le\left(1+\frac M2\right)^{-4}.
          \]</div>
          <p>因此常数可对每个固定 \(M\) 的 compact shape class 统一，但没有得到 \(M\to\infty\) 的一致定理。</p>
        </section>

        <section id="ed"><div class="section-no">05 / Full-superposition ED</div><h2>R0.72P 的完整传播结论扩展到声明的任意相位锥</h2>
          <p>Coble–He 的 profile-by-profile 定理结合上述固定邻域、cutoffs 与 shape bounds，给固定 \(M\) 类的 proof-level uniform corollary；紧 \(\eta\) 区间仍由精确 \(L^2\) 收缩补齐。</p>
          <div class="equation result">\[
          E(y)\le C_{\rm ED}e^{-c_{\rm ED}\sqrt\varepsilon y}E(0),
          \qquad \int_0^1E(y)\,dy\le C_{\rm ED}\varepsilon^{-1/2}E(0).
          \]</div>
          <p>常数只依赖固定 \(M\) 与声明的 upper shape class，不依赖相位、\(R\)、\(\varepsilon\) 或初值。</p>
        </section>
        ...
        <section id="certificate"><div class="section-no">08 / Independent exact audit</div><h2>Python Fraction 与 JavaScript BigInt 双路只核验有限代数骨架</h2>
          <p>两路独立重建 jet budget、radical comparisons、slow threshold 与 caustic ledger；comparator 要求 canonical payload 精确相等。正式 hash builder 拒绝临时 crosscheck、脏 source lineage、缺件、多件与 symlink。</p>
        </section>

        <section id="literature"><div class="section-no">09 / Literature boundary</div><h2>半群输入来自一手文献，任意相位 family uniformity 是本站的 proof-level 抽取</h2>
          <p><a href="https://doi.org/10.4310/CMS.2024.v22.n6.a10">Coble–He</a> 提供时变非退化 shear 的 enhanced-dissipation 框架；<a href="https://arxiv.org/abs/2104.05123">Voorhaar</a> 提供 caustic 的 Laurent-polynomial 语境。两者都不逐字陈述本站固定 \(M\)、\(Q_2\le1/2\)、任意相位与 1:2 实系数 nephroid 的组合定理。</p>
        </section>

        <section id="figure"><div class="section-no">10 / Journal figure</div><h2>正式附图区分任意相位安全锥、物理 shape 常数与精确 caustic</h2>
          <p><img src="/assets/r072q/fig-r072q-phase-robust-shape.svg" alt="R0.72Q arbitrary-phase fixed-M shape gate and exact 1:2 caustic"></p>
        ...
        <section id="value"><div class="section-no">11 / Research value</div><h2>这是从特殊两载波相位线到相位鲁棒有限模式锥的实质扩张</h2>
          <p>R0.72Q 移除了 R0.72P 的 real-collinear-phase 限制，并把临界点计数、uniform shape 与适用边界变成可审计的系数空间几何。它提高了机制类结果的稳健性，但仍不是一般三维稳定阈值。</p>
        </section>
...
HOME_Q_CARD = r'''          <div class="task-one" id="r072q" data-release="r072q" style="margin-top:2rem">
            ...
            <h3>固定 \(M\) 的任意相位 Morse shape gate 已经闭合</h3>
            <p>在 \(Q_2\le1/2\) 下，所有 relative phases 都允许；临界点恰有两个，实际 Coble shear 可取 \((r,C_0,C_1)=(\pi/12,81,36)\)。</p>
            <p>1:2 退化集是精确 nephroid \(z(\phi)=\frac18e^{-3i\phi}-\frac38e^{-i\phi}\)，phase-uniform 安全圆盘半径恰为 \(1/4\)。</p>
            <p><strong>结论边界：</strong>&nbsp;固定 \(M\) 与 jet dominance 不能删除；caustic 只标记 Morse applicability，不是 ED 失败。</p>
...
def build_note() -> None:
    ...
    replacements = (
        (r'<meta name="description" content=".*?">', '<meta name="description" content="研究笔记 R0.72Q：固定 M、任意相位与 Q2≤1/2 下的两临界点 shape gate，以及精确 1:2 caustic。">'),
        (r'<meta property="og:title" content=".*?">', '<meta property="og:title" content="R0.72Q｜任意相位 shape gate 与精确 caustic">'),
        (r'<meta property="og:description" content=".*?">', '<meta property="og:description" content="固定 M 任意相位的统一 Morse shape contract 与 phase-uniform 半径 1/4。">'),
        ...
        (r'<title>.*?</title>', '<title>R0.72Q｜任意相位 shape gate 与精确 caustic</title>'),
    )
...
def build_recap() -> None:
    ...
    html = section(html, r'<meta name="description" content=".*?">', '<meta name="description" content="R0.60 之后的研究回顾：完整覆盖 R0.61 到 R0.72Q 的 107 个节点；最新一节闭合固定 M 任意相位 shape gate。">', "Q recap description")
    html = section(html, r'<meta property="og:description" content=".*?">', '<meta property="og:description" content="二十八个阶段、107 个节点：从约化递推到固定 M 任意相位 shape gate。">', "Q recap og description")
    ...
    phase = r'''            <article class="phase"><h3>R0.72L–R0.72Q · strong-coupling、物理回填与相位鲁棒 shape gate</h3>
              <p>R0.72L–N 保留 actual ledger 并排除声明一载波上的 action-poor route；R0.72O 完成物理回填，R0.72P 在 fixed real-collinear-phase 1:2 正类上关闭完整传播门。</p>
              <p>R0.72Q 再把相位限制移除：对固定 \(M\)、任意相位与 \(Q_2\le1/2\)，恰有两个临界点，实际 Coble shear 可取 \((\pi/12,81,36)\)；1:2 caustic 给出尖锐 phase-uniform 半径 \(1/4\)。增长 \(M\) 与一般 carrier 集仍开放。</p>
    ...
    retained = r'''            <li>R0.72Q 的 fixed-\(M\) arbitrary-phase theorem：\(Q_2\le1/2\) 保证恰有两个临界点，物理 shear 的正式 shape 常数为 \((\pi/12,81,36)\)；1:2 caustic 给出尖锐 phase-uniform disk 半径 \(1/4\)。</li>
'''
    html = once(html, "          </ul>\n          <p>这些结果可以分别整理成", retained + "          </ul>\n          <p>这些结果可以分别整理成", "Q recap retained")
    html = section(html, r'        <section id="value">.*?</section>', r'''        <section id="value"><div class="section-no">04 / 目前的判断</div><h2>相位鲁棒的有限模式锥已闭合，一般 superposition 仍远未完成</h2><p>截至 R0.72Q，没有一般三维 continuation criterion，也没有证明有限时破裂或全局光滑性；不能把 107 个节点或 69 个公开版本解释成 Clay 问题完成比例。</p><p>新的严格增量是 fixed-\(M\)、arbitrary-phase、\(Q_2\le1/2\) 的 uniform shape gate，以及精确 1:2 caustic。</p></section>''', "Q recap value")
    ...
    html = section(html, r'        <section id="claims">.*?</section>', r'''        <section id="claims"><div class="section-no">06 / 说明边界</div><h2>公开、完整封存与问题解决继续分开计数</h2><p>R0.70A–R0.72Q 的 69 节已公开；45 节按当前 formal-figure 合同完整封存；24 节旧档仍待回补。</p><p>R0.72Q 只覆盖固定 \(M\)、任意相位、\(Q_2\le1/2\) 的 dominant-first-harmonic cone。增长 \(M\)、一般 carrier 集和 Clay 正式问题保持开放。</p></section>''', "Q recap claims")
...
def update_home() -> None:
    ...
    html = section(html, r'<div class="summary-item"><strong>我目前关注</strong><span>.*?</span></div>', '<div class="summary-item"><strong>我目前关注</strong><span>R0.72Q 已闭合 fixed-M arbitrary-phase、Q2≤1/2 的 shape gate；下一关是离开 dominant-first-harmonic cone。</span></div>', "Q home focus")
    ...
    recap = r'''          <div class="task-one" id="post-r060-recap" style="margin-top:2rem">
            ...
            <p>累计回顾保持二十八个问题阶段，并给出 R0.61–R0.72Q 的完整逐节点索引。R0.72Q 对固定 \(M\)、任意相位与 \(Q_2\le1/2\) 证明 uniform shape gate，并给出精确 1:2 caustic。</p>
...
def update_literature() -> None:
    ...
    new_steps = r'''<div class="route-step closed"><header><b>R0.72Q</b><strong>fixed-M arbitrary-phase shape gate and exact 1:2 caustic</strong></header><p>固定 \(M\)、任意相位与 \(Q_2\le1/2\) 下，恰有两个临界点，实际 shear 的 shape 常数为 \((\pi/12,81,36)\)；1:2 caustic 给出尖锐半径 \(1/4\)。<a href="/notes/r0-72q.html">研究笔记</a> <a href="/recap-r0-61-r0-72q.html">当前累计回顾</a> <a href="#r072q-boundary">方法边界</a></p></div>
    ...
    boundary = r'''
          ...
          <h3 id="r072q-boundary">R0.72Q 的任意相位 shape theorem 与 caustic 文献边界</h3>
          <p><a href="https://doi.org/10.4310/CMS.2024.v22.n6.a10">Coble–He, Theorem 1.2 and Appendix A</a> 为单个非退化时变 shear 提供 modewise enhanced dissipation。R0.72Q 通过固定 critical boxes、统一 Hessian margin 与 fixed-\(M\) derivative ledger，从 proof 中抽取声明 family 的一致常数；这不是原论文逐字陈述的 arbitrary-phase family theorem。</p>
          ...
          <div class="boundary"><strong>R0.72Q 的主张边界</strong><p>正结果只覆盖固定 \(M\)、dominant first harmonic 与 \(Q_2\le1/2\)。正式 physical shape 常数为 \((\pi/12,81,36)\)，归一化 \(F\) 的 away 常数 \(12\) 不可直接替代。增长 \(M\)、任意 time-dependent phases、无 jet dominance、跨越 caustic 的 profile、fixed-\(R\) arbitrary coupling 与一般三维问题仍开放；限定检索不构成新颖性或优先权证明。</p></div>'''
