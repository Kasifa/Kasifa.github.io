#!/usr/bin/env python3
"""Build the R0.72S archive manifest and SHA-256 ledger."""
...
FIGURE_COMMAND = (
    "python3 figures/r072s-heat-collisions/"
    "fig-r072s-heat-collisions/plot.py with the six frozen lineage arguments"
)
PACKAGE_SOURCES = (
    "README.md",
    "caption.md",
...
def require_formal_runtime_gate(
    ...
    if (
        ...
        raise RuntimeError("temporary, dirty, untracked, or source-mismatched R0.72S certificates are forbidden")
...
def main(status: str) -> None:
    ...
    if not visual:
        raise RuntimeError("R072S_VISUAL_QA_INSPECTED=true is required")
    ...
    if not source_paths or source_paths[0] != "research/r072s_report-source.md":
    ...
    if status == "formal":
        source_commit = os.environ.get("R072S_SOURCE_COMMIT", "").strip().lower()
        certificate_commit = os.environ.get("R072S_CERTIFICATE_COMMIT", "").strip().lower()
        dirty = os.environ.get("R072S_DIRTY_AT_CERTIFIED_RUN", "").strip().lower()
        ...
        if source_commit != runtime_source_commit:
            raise RuntimeError("R072S_SOURCE_COMMIT does not match the formal runtime source commit")
        if results.get("repositoryCommitAtBuild") != certificate_commit:
            raise RuntimeError(
                "results.repositoryCommitAtBuild must equal R072S_CERTIFICATE_COMMIT"
            )
    ...
    data_schemas = {
        ...
        "data.csv": "panel, route, series, kind, incidence or centered-time coordinates, angle parameters, optional auxiliary fields, source, pointer, claim status, and note",
    ...
    payload = {
        "schemaVersion": "1.1",
        "figureId": "fig-r072s-heat-collisions",
        "release": "R0.72S",
        "status": status,
        ...
        "computation": {
            ...
            "configuration": (
                "the projected A4/A5 incidence spine, the exact unique A2 heat collision with 4/3/2 distinct critical points, and the exact real-even A3 heat collision with 4/2/2 distinct critical points; the canonical R0.72S certificate directory is independently closed by its flat SHA256SUMS"
            ),
            ...
            "incidencePreimageA2ThroughA5Closed": True,
            "restrictedMiniversalityModuloConstantsClosed": True,
            "uniqueA2HeatCollisionClosed": True,
            "realEvenA3HeatCollisionClosed": True,
            "completeFourDimensionalCausticImageClassified": False,
            "causticCrossingEnhancedDissipationClosed": False,
            "arbitraryTimeDependentPhasesClosed": False,
            "generalThirdCarrierFamilyClosed": False,
        ...
        "qa": {
            ...
            "higherStratumSpineInspected": visual,
            "a2ExactAndLeadingBranchesInspected": visual,
            "a3ExactLeadingAndPersistentBranchesInspected": visual,
            "distinctAndMultiplicityCountsInspected": visual,
