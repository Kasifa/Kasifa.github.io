...
const protectedArtifacts = {
  ...
  "public/notes/clay-b-pressure-geometry-20260906.html": "ed1357f67d38bdfab1ee78efed811cd605b3f20b308cfa4b7574f005e698775f",
};
...
test("the HTML-only PressureQuotient note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-pressure-quotient-20260906.html");
  for (const marker of [
    "ClayB-PressureQuotient-20260906",
    "CB.8",
    "压力投影：抵消成立，一个特定瞬时残差预算失败",
    "one specific instantaneous residual budget fails",
    "∫R²/H dt",
    "fixed total energy",
    "F=0 plateau",
    "does not show that W is large",
    "pressure-moderator",
    "FINITE: NONE",
    "OPEN",
    "NOT CLAY",
  ...
  assert.equal(note.includes("/notes/clay-b-pressure-quotient-20260906.pdf"), false);
  assert.equal(note.includes("R0.76M"), false);
});

test("the homepage keeps the solid R0 sequence separate from the dashed CB.1–CB.8 lane", async () => {
  ...
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-pressure-quotient-20260906"));
  ...
  const positions = independentChapters.map(([chapter, slug]) => {
    ...
    assert.ok(home.slice(row, row + 1300).includes(chapter), `${chapter} route label`);
    return row;
  });
  assert.deepEqual(positions, [...positions].sort((a, b) => a - b));
  assert.equal((home.match(/class="route-overview independent-release-spotlight"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-pressure-quotient"/g) ?? []).length, 1);
  assert.equal((literature.match(/id="clay-b-pressure-quotient-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-pressure-quotient-20260906"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-pressure-quotient-20260906.pdf"), false);
  assert.equal(site.version, "2.52");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 8);
  assert.equal(site.latestIndependentNote, "ClayB-PressureQuotient-20260906");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-pressure-quotient-20260906.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.independentChapterScheme, "CB.n");
  assert.equal(site.latestIndependentChapter, "CB.8");
  assert.equal(site.nextIndependentChapter, "CB.9");
  assert.equal(manifest.siteVersion, "2.52");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestCompletedStep, 63);
  assert.equal(manifest.latestPublication.releaseId, "clay-b-pressure-quotient-20260906");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-PressureGeometry-20260906");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.8");
});
...
test("historical milestones, endpoint, prior note, and retained PDFs remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-pressure-quotient-20260906.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-pressure-quotient-20260906.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-pressure-quotient-20260906.pdf")));
});
