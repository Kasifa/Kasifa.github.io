...
const root = resolve(process.env.R073L_RELEASE_ROOT ?? resolve(import.meta.dirname, ".."));
const publicDirectory = resolve(root, "public");
const translationPath = resolve(root, "translations/en.json");
const bundlePath = resolve(publicDirectory, "i18n-en.js");
const snapshotPath = resolve(root, "scripts/i18n-snapshots/r073l-missing.json");
const usage = "usage: add-r073l-translations.mjs (--apply | --check-only | --capture-missing)";
...
if (unknownArguments.length) {
  throw new Error("Unknown R0.73L translation argument: " + unknownArguments[0] + "\n" + usage);
}
...
function assertInsideRoot(canonicalPath, label) {
  ...
  if (
    ...
    throw new Error(label + " escaped R073L_RELEASE_ROOT: " + canonicalPath);
  }
}
...
async function writeTranslationTransaction(writes) {
  if (writes.length !== 3 || new Set(writes.map(({ path }) => path)).size !== 3) {
    throw new Error("R0.73L translation transaction requires exactly three distinct files");
  }
  ...
  for (const [index, { path, payload }] of writes.entries()) {
    ...
    entries.push({
      ...
      temporary: resolve(dirname(path), `.${path.split("/").at(-1)}.r073l-${nonce}-${index}.tmp`),
      backup: resolve(dirname(path), `.${path.split("/").at(-1)}.r073l-${nonce}-${index}.bak`),
      backedUp: false,
      installed: false,
    });
  }
  ...
  try {
    ...
    if (rollbackErrors.length) {
      throw new AggregateError([error, ...rollbackErrors], "R0.73L translation rollback failed");
    }
    throw error;
  } finally {
...
async function writeSingleAtomic(path, payload) {
  ...
  const nonce = `${process.pid}-${Date.now()}-${process.hrtime.bigint()}`;
  const temporary = resolve(dirname(path), `.${path.split("/").at(-1)}.r073l-${nonce}.tmp`);
  try {
...
if (absentPages.length) {
  throw new Error(
    "R0.73L HTML must be generated before translation snapshot/application: " +
...
for (const relativePage of activePages) {
  ...
  if (!html.includes("/i18n-en.js?v=1.52")) {
    throw new Error(relativePage + ": expected i18n cache version v1.52");
  }
...
const retained = translations.filter((entry) => !/^r073l\d+$/.test(entry.id));
const retainedByChinese = new Map(retained.map((entry) => [entry.zh, entry]));
if (retainedByChinese.size !== retained.length) {
  throw new Error("Duplicate Chinese keys outside the R0.73L batch");
}
...
if (JSON.stringify(missingFiles) !== JSON.stringify(activePages)) {
  throw new Error("Unexpected R0.73L missing-string files: " + JSON.stringify(missingFiles));
}
...
if (captureMissing) {
  ...
  console.log(JSON.stringify({
    ...
    snapshot: "scripts/i18n-snapshots/r073l-missing.json",
  }));
  process.exit(0);
}
...
if (!Array.isArray(snapshot) || snapshot.length === 0) {
  throw new Error("Empty R0.73L snapshot");
}
...
if (JSON.stringify(snapshotChinese) !== JSON.stringify(liveChinese)) {
  ...
  throw new Error("R0.73L missing-string snapshot drift at row " + String(index + 1));
}
const englishByChinese = new Map(snapshot.map((entry) => [entry.zh, entry.en]));
if (englishByChinese.size !== snapshot.length) {
  throw new Error("Duplicate R0.73L snapshot keys");
}

const boundaryTokens = (value) =>
  value.match(/NOT CLAY|\b(?:FALSE_AS_INFERENCE|CONDITIONAL|INCONCLUSIVE|NOT_RUN|FAILED|CLOSED|OPEN|PASS|FALSE|TRUE)\b/g) ?? [];
...
const translatedEntries = missing.map((entry, index) => {
  ...
  if (!en || containsChinese(en)) {
    throw new Error("Invalid R0.73L English row: " + entry.zh);
  }
  if (/\b(?:we|our|ours|ourselves|us)\b/i.test(en)) {
    throw new Error("Plural voice in R0.73L English row: " + entry.zh);
  }
  if (JSON.stringify(extractProtectedTokens(en)) !== JSON.stringify(extractProtectedTokens(entry.zh))) {
    throw new Error("Protected-token mismatch for R0.73L row: " + entry.zh);
  }
  if (JSON.stringify(accountingTokens(en)) !== JSON.stringify(accountingTokens(entry.zh))) {
    throw new Error("Version/count-token mismatch for R0.73L row: " + entry.zh);
  }
  if (JSON.stringify(boundaryTokens(en)) !== JSON.stringify(boundaryTokens(entry.zh))) {
    throw new Error("Claim-state boundary drift for R0.73L row: " + entry.zh);
  }
  if (JSON.stringify(machineLedgerTokens(en)) !== JSON.stringify(machineLedgerTokens(entry.zh))) {
    throw new Error("Machine-ledger sequence drift for R0.73L row: " + entry.zh);
  }
  return { ...entry, id: "r073l" + String(index + 1).padStart(3, "0"), en };
});
...
if (checkOnly) {
  if (await readFile(snapshotPath, "utf8") !== snapshotOutput) {
    throw new Error("R0.73L snapshot is stale");
  }
  if (JSON.stringify(translations) !== JSON.stringify(finalTranslations)) {
    throw new Error("R0.73L translations are stale");
  }
  if (await readFile(bundlePath, "utf8") !== bundleOutput) {
    throw new Error("R0.73L bundle is stale");
  }
} else if (apply) {
