...
// Cryptographically bind the synchronized R0.73W PDFs to their HTML sources
...
const usage =
  "usage: bind-r073w-pdfs.mjs (--apply | --check-only | " +
...
const outputPath = resolve(root, "research/r073w_pdf_bindings.json");
...
const releaseTitle = "R0.73W | Signed subfilter production: heat-plane characteristics, the energy-class boundary, and exact counterexamples";
const publicTitle = "R0.73W｜带符号亚滤波 production：heat-plane 特征线、能量类边界与精确反例";
const documentTitle = "R0.73W｜Signed subfilter production: heat-plane characteristics, the energy-class boundary, and exact counterexamples";
const recapTitle = "R0.61–R0.73W｜R0.60 之后的研究回顾";
...
async function atomicWrite(path, payload) {
  ...
  const temporary = resolve(
    dirname(path),
    `.${basename(path)}.r073w-${process.pid}-${Date.now()}-${process.hrtime.bigint()}.tmp`,
  );
  try {
...
if (/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/.test(report + dictionary)) {
  throw new Error("R0.73W canonical report or dictionary contains control characters");
}
if (!report.startsWith("# " + releaseTitle + "\n") ||
    !report.includes("**Public title (zh):** " + publicTitle)) {
  throw new Error("R0.73W canonical report title ledger drift");
}
const boundaryCorpus = (report + "\n" + dictionary)
  .replace(/^>\s?/gm, "")
  .replace(/\s+/g, " ");
for (const phrase of [
  "The forced-diffusion equation and exact scale integral for the Gaussian subfilter stress are established in the literature; this release does not claim novelty for them.",
  "The heat-plane characteristic identity exactly controls the signed spatial mean; it does not control \\(|\\Pi_s|\\) or a pointwise sign.",
  "The absolute energy-class estimate loses \\(s^{-1/4}\\) as \\(s\\downarrow0\\), but this loss is integrable over heat scale.",
  "The exact Fourier witness produces both signs of the mean production, so no one-sided sign law holds for all smooth divergence-free data.",
  "Amplitude scaling excludes the stated same-time amplitude-independent quadratic absorption; it does not exclude nonlinear, time-integrated, or localized estimates.",
  "The public primary witness has rank-three Fourier support, but it remains only a counterexample to a universal sign statement, not generic turbulence, a singularity, or a blow-up candidate.",
  "The finite certificate is exact Fourier algebra, not a Navier--Stokes time simulation.",
  "The bounded search did not locate an identical package; non-detection is not proof of novelty, priority, non-existence, or first authorship.",
  ...
  if (!boundaryCorpus.includes(phrase)) {
    throw new Error("R0.73W canonical sources missing boundary phrase: " + phrase);
  }
}
for (const token of [
...
if (
  ...
  throw new Error("R0.73W HTML/accounting apply must precede PDF binding");
}

const certificateManifest = JSON.parse((await regularBytes(
  "research/certificates/r073w/manifest.json",
)).toString("utf8"));
const figureValidation = JSON.parse((await regularBytes(
  `research/figures/r073w/${figureId}/validation.json`,
)).toString("utf8"));
const figureResults = JSON.parse((await regularBytes(
  `research/figures/r073w/${figureId}/results.json`,
)).toString("utf8"));
...
if (
  ...
  certificateManifest.checkInventory?.exactPerPath !== certificateChecks ||
  certificateChecks !== 56 ||
  figureValidation.schemaVersion !== "r073w-signed-production-validation-v1" ||
  ...
  throw new Error("R0.73W sealed certificate/figure accounting drifted");
}

const documents = [
  {
    kind: "research-note",
    html: "public/notes/r0-73w.html",
    pdf: "public/notes/r0-73w.pdf",
    provenance: "research/r073w_note_pdf_render.json",
    route: "/notes/r0-73w.html",
    title: documentTitle,
    htmlMarkers: [publicTitle, "heatPlaneCharacteristicIdentity=INTERNAL_EXACT_AUDITED", figureId],
  },
  {
    kind: "cumulative-recap",
    html: "public/recap-r0-61-r0-73w.html",
    pdf: "public/recap-r0-61-r0-73w.pdf",
    provenance: "research/r073w_recap_pdf_render.json",
    route: "/recap-r0-61-r0-73w.html",
    title: recapTitle,
    htmlMarkers: ["R0.73W", "139", "localizedScaleCriticalControl=OPEN"],
  },
];
...
const manifest = {
  schemaVersion: "r073w-synchronized-pdf-bindings-v1",
  release: "R0.73W",
  canonicalTitleSource: {
    path: "research/r073w_report-source.md",
    sha256: sha256(Buffer.from(report)),
    releaseTitle,
    publicChineseTitle: publicTitle,
  },
  canonicalBoundarySource: {
    path: "research/r073w_bilingual_dictionary.md",
  ...
  claimBoundary: {
    ...
    finiteFormulaDiagnosticChecksPerPath: certificateChecks,
...
if (apply) {
  ...
  if (!current.equals(payload)) throw new Error("R0.73W PDF binding manifest is stale");
}
...
console.log(JSON.stringify({
  ...
  output: "research/r073w_pdf_bindings.json",
