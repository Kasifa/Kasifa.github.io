...
const baseUrl = new URL(process.env.R074X_PUBLIC_BASE_URL ?? "https://kasifa.github.io/");
const commit = process.env.R074X_DEPLOYED_COMMIT ?? "unversioned";
const figureId = "fig-r074x-three-packet-payment-gate";
const publicFigureRoot = `public/figures/r074x/${figureId}`;
const sha256 = (bytes) => createHash("sha256").update(bytes).digest("hex");

const fixedObjects = [
  ["public/notes/r0-74x.html", "/notes/r0-74x.html"],
  ["public/notes/r0-74x.pdf", "/notes/r0-74x.pdf"],
  ...
  [`public/assets/r074x/${figureId}.pdf`, `/assets/r074x/${figureId}.pdf`],
  [`public/assets/r074x/${figureId}.png`, `/assets/r074x/${figureId}.png`],
  [`public/assets/r074x/${figureId}.svg`, `/assets/r074x/${figureId}.svg`],
];

const figureNames = execFileSync(
  "git",
  ["ls-tree", "-r", "--name-only", "HEAD", `research/figures/r074x/${figureId}`],
...
const objects = [
  ...
  ...figureNames.map((name) => [
    ...
    `/figures/r074x/${figureId}/${encodeURIComponent(name)}`,
  ]),
];
assert.equal(objects.length, 37, "Step 23 online publication inventory");
