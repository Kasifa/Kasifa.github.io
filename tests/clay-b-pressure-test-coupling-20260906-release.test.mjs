...
const protectedArtifacts = {
  ...
  "public/notes/clay-b-bad-time-net-work-20260906.html": "a41b14e826d99c9da979b1e0cc976036cd0444b585872618b1686306b29ba726",
};
const independentChapters = [
  ...
  ["CB.11", "clay-b-pressure-test-coupling-20260906"],
];

test("the HTML-only PressureTestCoupling note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-pressure-test-coupling-20260906.html");
  for (const marker of ["ClayB-PressureTestCoupling-20260906", "CB.11", "压力不能与它的测试因子分开估计", "Pressure cannot be estimated apart from its test factor", "W_ang≤Cχ(M²g²+g⁴)", "only sufficient", "STATIC OBSTRUCTION", "CONDITIONAL", "OPEN", "NOT CLAY"]) assert.ok(note.includes(marker), marker);
  ...
  assert.equal(note.includes("/notes/clay-b-pressure-test-coupling-20260906.pdf"), false);
  assert.equal(note.includes("R0.76M"), false);
});

test("the homepage keeps the solid R0 sequence separate from the dashed CB.1–CB.11 lane", async () => {
  ...
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-pressure-test-coupling-20260906"));
  ...
  assert.equal((home.match(/id="clay-b-pressure-test-coupling"/g) ?? []).length, 1);
  assert.equal((literature.match(/id="clay-b-pressure-test-coupling-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-pressure-test-coupling-20260906"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-pressure-test-coupling-20260906.pdf"), false);
  assert.equal(site.version, "2.55");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 11);
  assert.equal(site.latestIndependentNote, "ClayB-PressureTestCoupling-20260906");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-pressure-test-coupling-20260906.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.latestIndependentChapter, "CB.11");
  assert.equal(site.nextIndependentChapter, "CB.12");
  assert.equal(manifest.siteVersion, "2.55");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestPublication.releaseId, "clay-b-pressure-test-coupling-20260906");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-BadTimeNetWork-20260906");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.11");
});
...
test("historical milestones, endpoint, prior note, and retained PDFs remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-pressure-test-coupling-20260906.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-pressure-test-coupling-20260906.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-pressure-test-coupling-20260906.pdf")));
});
