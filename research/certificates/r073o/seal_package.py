#!/usr/bin/env python3
"""Apply or verify the two-stage R0.73O diagnostic provenance seal."""
...
SOURCE_FILES = {
    ...
    "config.json",
    "requirements.txt",
    "compute_spectrum_diagnostic.py",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source-commit",
        default="",
        help="immutable 40-hex R0.73O certificate-source commit for final sealing",
    )
...
def main() -> None:
    ...
    if args.verify_only:
        ...
        if manifest.get("schemaVersion") != "r073o-kolmogorov-spectrum-manifest-v1":
        ...
        if assigned is True:
            ...
            if (
                ...
                or manifest.get("finalSeal") is not False
            ):
                raise RuntimeError("uncommitted manifest status drift")
    ...
    manifest = {
        "schemaVersion": "r073o-kolmogorov-spectrum-manifest-v1",
        "release": "R0.73O",
        ...
        "sourceCommitAssignedMeaning": (
            ...
            if final
            else "The pre-seal state binds source bytes by SHA-256; a parent release may assign an immutable source commit later."
        ),
        "inventory": {
            "sourceFileCount": 9,
            "generatedFileCount": 10,
    ...
    if final:
        ...
        manifest["finalSealPendingReason"] = (
            "No immutable R0.73O certificate-source commit was provided."
        )
