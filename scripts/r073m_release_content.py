...
NOTE_ARTICLE = r'''      <article>
        ...
        <section id="endpoint"><div class="section-no">05 / Fixed-distance endpoint</div><h2>选定谐波的线性主项压过立方修正与四阶余项</h2><p>对 \(0<\rho\le\rho_0\)，有效振幅 \(\delta_\Lambda\) 保持统一小，奇偶谐波结构排除二次项直接返回 \(K_z=\pm1\)。于是终点 selected-pair 范数至少为 \((c_L/2)\rho\)。</p></section>
