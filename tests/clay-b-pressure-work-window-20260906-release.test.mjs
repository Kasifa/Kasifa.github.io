...
const protectedArtifacts = {
  ...
  "public/notes/clay-b-pressure-quotient-20260906.html": "c3bef222c5b358721c1cdd7e978524192a930839ed0db14d662566cc3b5b977e",
};
const independentChapters = [
  ...
  ["CB.8", "clay-b-pressure-quotient-20260906"],
  ["CB.9", "clay-b-pressure-work-window-20260906"],
];

test("the HTML-only PressureWorkWindow note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-pressure-work-window-20260906.html");
  for (const marker of [
    "ClayB-PressureWorkWindow-20260906", "CB.9",
    "固定总能量不能给出这条 L³ 增长预算",
    "Fixed total energy cannot provide this L³ growth budget",
    "genuine signed pressure work", "tε=τ₀ε^(5/2)", "O(√ε)",
    "leading factor exactly one", "FINITE: NONE", "OPEN", "NOT CLAY",
  ...
  assert.equal(note.includes("/notes/clay-b-pressure-work-window-20260906.pdf"), false);
  assert.equal(note.includes("R0.76M"), false);
});

test("the homepage keeps the solid R0 sequence separate from the dashed CB.1–CB.9 lane", async () => {
  const [home, literature, index, site, manifest, noteFiles] = await Promise.all([
    ...
    text("public/site-version.json").then(JSON.parse), text("research/release-manifest.json").then(JSON.parse),
    readdir(resolve(root, "public/notes")),
  ]);
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-pressure-work-window-20260906"));
  ...
  const positions = independentChapters.map(([chapter, slug]) => {
    ...
    assert.ok(home.slice(row, row + 1400).includes(chapter), `${chapter} route label`);
    return row;
  });
  assert.deepEqual(positions, [...positions].sort((a, b) => a - b));
  assert.equal((home.match(/class="route-overview independent-release-spotlight"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-pressure-work-window"/g) ?? []).length, 1);
  assert.equal((literature.match(/id="clay-b-pressure-work-window-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-pressure-work-window-20260906"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-pressure-work-window-20260906.pdf"), false);
  assert.equal(site.version, "2.53");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 9);
  assert.equal(site.latestIndependentNote, "ClayB-PressureWorkWindow-20260906");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-pressure-work-window-20260906.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.latestIndependentChapter, "CB.9");
  assert.equal(site.nextIndependentChapter, "CB.10");
  assert.equal(manifest.siteVersion, "2.53");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestCompletedStep, 63);
  assert.equal(manifest.latestPublication.releaseId, "clay-b-pressure-work-window-20260906");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-PressureQuotient-20260906");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.9");
});
...
test("historical milestones, endpoint, prior note, and retained PDFs remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-pressure-work-window-20260906.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-pressure-work-window-20260906.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-pressure-work-window-20260906.pdf")));
});
