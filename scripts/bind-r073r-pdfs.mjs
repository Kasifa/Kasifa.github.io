...
// Cryptographically bind the two synchronized R0.73R PDFs to their HTML
...
const usage = "usage: bind-r073r-pdfs.mjs (--apply | --check-only)";
...
const outputPath = resolve(root, "research/r073r_pdf_bindings.json");
...
function canonicalTitles(dictionary) {
  if (/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/.test(dictionary)) {
    throw new Error("R0.73R bilingual dictionary contains control characters");
  }
  ...
  if (releaseTitle !== "R0.73R | A shellwise phase certificate for the critical heat trace" ||
      publicChinese !== "R0.73R｜能谱看不见的相位：临界热流迹的壳层证书") {
    throw new Error("R0.73R canonical title ledger drift");
  }
  for (const token of [
  ...
  return {
    ...
    recap: "R0.61–R0.73R｜R0.60 之后的研究回顾",
  };
}
...
if (
  ...
  throw new Error("R0.73R HTML/accounting apply must precede PDF binding");
}
const documents = [
  {
    kind: "research-note",
    html: "public/notes/r0-73r.html",
    pdf: "public/notes/r0-73r.pdf",
    title: titles.note,
  },
  {
    kind: "cumulative-recap",
    html: "public/recap-r0-61-r0-73r.html",
    pdf: "public/recap-r0-61-r0-73r.pdf",
    title: titles.recap,
  },
];
...
const manifest = {
  schemaVersion: "r073r-synchronized-pdf-bindings-v1",
  release: "R0.73R",
  canonicalTitleSource: {
    path: "research/r073r_bilingual_dictionary.md",
...
if (apply) {
  ...
  if (!current.equals(payload)) throw new Error("R0.73R PDF binding manifest is stale");
}
...
console.log(JSON.stringify({
  ...
  output: "research/r073r_pdf_bindings.json",
}));
