...
const outputRoot = resolve(process.env.R075X_STEP49_BROWSER_QA_OUTPUT ?? "/tmp/r075x-step49-browser-qa");
const externalBase = process.env.R075X_PUBLIC_BASE_URL;
...
const target = {
  id: "note",
  path: "/notes/r0-75x.html",
  h1Zh: /固定有限谐波族的低载频 signed-flux 付款/,
  h1En: /Low-carrier signed-flux payment for a fixed finite harmonic family/i,
};
const results = [];
  ...
  for (const scenario of scenarios) {
    {
      ...
      url.searchParams.set("qa", "r075x-step49");
      ...
      const audit = await page.evaluate(() => {
        ...
        return {
          ...
          step49MathJaxErrors: document.querySelectorAll("#s-384 mjx-merror, #s-385 mjx-merror, #s-386 mjx-merror, #s-387 mjx-merror, #s-388 mjx-merror, #s-389 mjx-merror, #s-390 mjx-merror, #s-391 mjx-merror, #s-392 mjx-merror").length,
          ...
          figureSections: document.querySelectorAll('#figure, [src*="assets/r075x"], [href*="assets/r075x"]').length,
          fixedFinite: /FIXED FINITE q|fixed finite/i.test(visibleText),
          lowCarrier: /LOW CARRIER ONLY|low-carrier|低载频/i.test(visibleText),
          noUniformQ: /NO UNIFORM q GROWTH|no uniform q-growth|not uniform as q grows/i.test(visibleText),
          highCarrierOpen: /HIGH CARRIER 3\+ OPEN|high-carrier[^\n]{0,80}(?:3\+|three|three or more)[^\n]{0,80}(?:OPEN|open)/i.test(visibleText),
          noGapDivision: /NO GAP DIVISOR|no division by[^\n]{0,120}(?:frequency|gap)/i.test(visibleText),
          noVelocityDivision: /NO V DIVISION|no division by[^\n]{0,80}(?:velocity|v)/i.test(visibleText),
      ...
      assert.equal(audit.step49MathJaxErrors, 0, `${id} Step 49 MathJax errors`);
      ...
      assert.equal(audit.figureSections, 0, `${id} no R0.75X formal figure`);
      for (const key of ["fixedFinite", "lowCarrier", "noUniformQ", "highCarrierOpen", "noGapDivision", "noVelocityDivision", "noveltyBoundary", "clayBoundary"])
