...
const outputRoot = resolve(process.env.R076E_STEP56_BROWSER_QA_OUTPUT ?? "/tmp/r076e-step56-browser-qa");
...
const target = {
  id: "note",
  path: "/notes/r0-76e.html",
  h1Zh: /精确剪切的线性模态熵窗口/,
  h1En: /Linear modal-entropy window for exact shears/i,
};
const results = [];
...
try {
  for (const scenario of scenarios) {
    ...
    url.searchParams.set("qa", "r076e-step56");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        step56MathJaxErrors: document.querySelectorAll("#s-442 mjx-merror, #s-443 mjx-merror, #s-444 mjx-merror, #s-445 mjx-merror, #s-446 mjx-merror, #s-447 mjx-merror, #s-448 mjx-merror, #s-449 mjx-merror").length,
        ...
        figureSections: document.querySelectorAll('#figure, [src*="assets/r076e"], [href*="assets/r076e"]').length,
        exactShear: /EXACT REAL CONSTANT SHEAR|exact real constant-shear|精确剪切/i.test(visibleText),
        modalEntropy: /EXP\(C Q\) LOSS|exp\(C_\* q\)|linear modal-entropy|线性模态熵/i.test(visibleText),
        growingWindow: /Q\(L\) = o\(L\^2\)|q\(L\)=o\(L\^2\)|增长窗口/i.test(visibleText),
        alphaPlusQ: /ALPHA\+Q DERIVATIVE|alpha\+q|空间导数/i.test(visibleText),
        lastUnitEndpoint: /LAST-UNIT ENDPOINT|last-unit endpoint/i.test(visibleText),
        delayedSplit: /DELAYED STABLE HEAT CLOCK|delayed stable heat clock|delayed stable-tail split/i.test(visibleText),
        earlyHolder: /EARLY HOLDER PAYMENT|early Holder/i.test(visibleText),
        monotoneTail: /MONOTONE TAIL ONLY|monotone tail|tail becomes monotone/i.test(visibleText),
        ...
        externalInputs: /Turan--Nazarov/i.test(visibleText) && /Erdelyi/i.test(visibleText) && /external\s*[:：]|external inputs|外部输入|imports the same two approximation-theory inputs/i.test(visibleText),
        localDeductions: /delayed split[^\n]{0,240}early Holder[^\n]{0,240}(?:scale conversion|physical conversion)/i.test(visibleText),
    ...
    assert.equal(audit.step56MathJaxErrors, 0, `${id} Step 56 MathJax errors`);
    ...
    assert.equal(audit.figureSections, 0, `${id} no R0.76E formal figure`);
    for (const key of ["exactShear", "modalEntropy", "growingWindow", "lastUnitEndpoint", "delayedSplit", "earlyHolder", "monotoneTail", "weightedPower", "terminalPower", "completeSquare", "externalInputs", "finiteBoundary", "arbitraryPacketsOpen", "versionM", "noveltyBoundary", "clayBoundary"])
