...
def main() -> None:
    ...
    with Image.open(ROOT / "figure.png") as source:
        ...
        if abs(width - expected_width) > 3 or abs(height - expected_height) > 3:
            raise RuntimeError(f"unexpected archival PNG size {source.size}; expected approximately {(expected_width, expected_height)}")
    ...
    with Image.open(ROOT / "qa-pdf.png") as rendered:
        ...
        if abs(pdf_width - qa_width) > 4 or abs(pdf_height - qa_height) > 4:
            raise RuntimeError(f"PDF preview size {rendered.size} is inconsistent with declared final size {(qa_width, qa_height)} at {qa_dpi} dpi")
    ...
    (ROOT / "qa-report.md").write_text(
        "# R0.71Y operator-sampling figure QA\n\n"
        ...
        f"- Panel A exact lattice factor, upper bound, units, and tail-fit label inspection: {manual}\n"
        f"- Panel B three theorem-envelope laws and exact curve coincidence inspection: {manual}\n"
        f"- Panel C log10 inverse lower bound, direct labels, and non-IFT wording inspection: {manual}\n"
        f"- analytic/certificate-envelope and not-simulated wording inspection: {manual}\n"
        ...
        "- selected-root, unit-phase, and full-enstrophy-floor footer visible: PASS\n"
        "- no-growing-root-construction and not-DNS footer visible: PASS\n"
