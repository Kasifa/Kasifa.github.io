...
const root = resolve(process.env.R072X_RELEASE_ROOT ?? resolve(import.meta.dirname, ".."));
const publicDirectory = resolve(root, "public");
const translationPath = resolve(root, "translations/en.json");
const bundlePath = resolve(publicDirectory, "i18n-en.js");
const snapshotPath = resolve(root, "scripts/i18n-snapshots/r072x-missing.json");
...
for (const relative of activePages) {
  ...
  if (!html.includes('/i18n-en.js?v=1.37')) {
    throw new Error(`${relative}: expected i18n cache version v1.37`);
  }
}

const translations = JSON.parse(await readFile(translationPath, "utf8"));
const retained = translations.filter((entry) => !/^r072x\d+$/.test(entry.id));
const retainedByChinese = new Map(retained.map((entry) => [entry.zh, entry]));
if (retainedByChinese.size !== retained.length) {
  throw new Error("Duplicate Chinese keys outside the R0.72X batch");
}
...
if (JSON.stringify(missingFiles) !== JSON.stringify(activePages)) {
  throw new Error(`Unexpected R0.72X missing-string files: ${JSON.stringify(missingFiles)}`);
}

const snapshot = JSON.parse(await readFile(snapshotPath, "utf8"));
if (!Array.isArray(snapshot) || snapshot.length === 0) {
  throw new Error("Empty R0.72X snapshot");
}
...
if (JSON.stringify(snapshotChinese) !== JSON.stringify(liveChinese)) {
  ...
  throw new Error(`R0.72X missing-string snapshot drift at row ${index + 1}`);
}
const englishByChinese = new Map(snapshot.map((entry) => [entry.zh, entry.en]));
if (englishByChinese.size !== snapshot.length) {
  throw new Error("Duplicate R0.72X snapshot keys");
}
...
if (!longRoute || !previousRoute) {
  throw new Error("Missing cumulative R0.72X literature-route binding");
}
const routeMarker = " The global caustic image is not completed";
if (!previousRoute.en.includes(routeMarker)) {
  throw new Error("Unexpected retained R0.72W literature-route shape");
}
englishByChinese.set(
  longRoute.zh,
  previousRoute.en.slice(0, previousRoute.en.indexOf(routeMarker)) +
    " The global caustic image is not completed, and general three-dimensional Navier--Stokes regularity is not proved. R0.72T fixes the exact A2 spacetime germ and its unique scaling. R0.72U closes center-uniform fixed-chart graph coercivity. R0.72V closes exact cubic scalar whole-line graph coercivity and fixed-block contraction. R0.72W retains the full analytic sine tail and closes the exact periodic scalar collision block. R0.72X extends the graph constant to every block center in a fixed physical-time compact set, yielding the all-start exact-path semigroup, integrated A2 scale, uniform Bloch twists, and the strong-row direct sum; these Bloch-uniform conclusions belong only to the exact A2 path. The fixed-margin Coble--He input joins the exact A2 cocycle into A1--A2--A1 propagation only for the periodic representative beta=0; no Bloch-uniform A1 extension is inferred. Shrinking-interface fixed-shape A1 hypotheses, the prefactor-one all-gap exponential, and strict contraction of every physical row are FALSE. Forced H^-1 transfer, the complete linearized shear subsystem, nonlinear Navier--Stokes, and Clay remain OPEN.",
);
...
const translatedEntries = missing.map((entry, index) => {
  ...
  if (!en || en === "__DERIVED_FROM_R072W_ROUTE__" || containsChinese(en)) {
    throw new Error(`Invalid R0.72X English row: ${entry.zh}`);
  }
  if (/\b(?:we|our|ours|ourselves|us)\b/i.test(en)) {
    throw new Error(`Plural voice in R0.72X English row: ${entry.zh}`);
  }
  if (JSON.stringify(extractProtectedTokens(en)) !== JSON.stringify(extractProtectedTokens(entry.zh))) {
    throw new Error(`Protected-token mismatch for R0.72X row: ${entry.zh}`);
  }
  if (JSON.stringify(boundaryTokens(en)) !== JSON.stringify(boundaryTokens(entry.zh))) {
    throw new Error(`CLOSED/OPEN/FALSE boundary drift for R0.72X row: ${entry.zh}`);
  }
  return { ...entry, id: `r072x${String(index + 1).padStart(3, "0")}`, en };
});
...
if (checkOnly) {
  if (JSON.stringify(translations) !== JSON.stringify(finalTranslations)) {
    throw new Error("R0.72X translations are stale");
  }
  if (await readFile(bundlePath, "utf8") !== bundleOutput) {
    throw new Error("R0.72X bundle is stale");
  }
} else {
  await Promise.all([
