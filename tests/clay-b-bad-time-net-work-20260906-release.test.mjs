...
const protectedArtifacts = {
  ...
  "public/notes/clay-b-pressure-work-window-20260906.html": "603347ab6c389be37f70cc046f11337d96b3af3dd2f26ebd24994c211405952d",
};
...
test("the HTML-only BadTimeNetWork note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-bad-time-net-work-20260906.html");
  for (const marker of ["ClayB-BadTimeNetWork-20260906", "CB.10", "坏时间净压力工作", "Bad-time net pressure work", "liminf 𝓑_J/Hχ(t) ≥ 1", "necessary lower bound", "not an upper bound", "NECESSARY CONDITION", "OPEN", "NOT CLAY"]) assert.ok(note.includes(marker), marker);
  ...
  assert.equal(note.includes("/notes/clay-b-bad-time-net-work-20260906.pdf"), false);
  assert.equal(note.includes("R0.76M"), false);
});

test("the homepage keeps the solid R0 sequence separate from the dashed CB.1–CB.10 lane", async () => {
  ...
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-bad-time-net-work-20260906"));
  ...
  const positions = independentChapters.map(([chapter, slug]) => {
    ...
    assert.ok(home.slice(row, row + 1450).includes(chapter), `${chapter} route label`);
    return row;
  });
  assert.deepEqual(positions, [...positions].sort((a, b) => a - b));
  assert.equal((home.match(/class="route-overview independent-release-spotlight"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-bad-time-net-work"/g) ?? []).length, 1);
  assert.equal((literature.match(/id="clay-b-bad-time-net-work-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-bad-time-net-work-20260906"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-bad-time-net-work-20260906.pdf"), false);
  assert.equal(site.version, "2.54");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 10);
  assert.equal(site.latestIndependentNote, "ClayB-BadTimeNetWork-20260906");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-bad-time-net-work-20260906.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.latestIndependentChapter, "CB.10");
  assert.equal(site.nextIndependentChapter, "CB.11");
  assert.equal(manifest.siteVersion, "2.54");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestPublication.releaseId, "clay-b-bad-time-net-work-20260906");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-PressureWorkWindow-20260906");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.10");
});
...
test("historical milestones, endpoint, prior note, and retained PDFs remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-bad-time-net-work-20260906.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-bad-time-net-work-20260906.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-bad-time-net-work-20260906.pdf")));
});
