...
const outputRoot = resolve(process.env.CLAY_B_TWO_SCALE_BROWSER_QA_OUTPUT ?? "/tmp/clay-b-two-scale-browser-qa");
const externalBase = process.env.CLAY_B_TWO_SCALE_PUBLIC_BASE_URL;
...
const targets = [
  {
    id: "clay-b-note",
    path: "/notes/clay-b-two-scale-20260905.html",
    minMath: 14,
    h1Zh: /两尺度差能量：瞬时吸收的限制与完整支付/,
    h1En: /Two-scale difference energy: limits of instantaneous absorption and the complete payment/i,
  },
  {
    id: "r076l-preserved",
    path: "/notes/r0-76l.html",
    minMath: 160,
...
try {
  for (const target of targets) for (const scenario of scenarios) {
    ...
    url.searchParams.set("qa", "clay-b-two-scale-20260905");
    ...
    const audit = await page.evaluate((targetId) => {
      ...
      return {
        ...
        mathErrors: targetId === "clay-b-note"
          ? document.querySelectorAll("mjx-merror").length
          : document.querySelectorAll(
            "#s-497 mjx-merror,#s-498 mjx-merror,#s-499 mjx-merror,#s-500 mjx-merror,#s-501 mjx-merror,#s-502 mjx-merror,#s-503 mjx-merror,#s-504 mjx-merror,#s-505 mjx-merror,#s-506 mjx-merror,#s-507 mjx-merror",
          ).length,
        rawInline: (text.match(/\\\(/g) ?? []).length,
        rawDisplay: (text.match(/\\\[/g) ?? []).length,
        chinese: (text.match(/[\u3400-\u9fff\uf900-\ufaff]/gu) ?? []).length,
        chineseSamples: [...body.querySelectorAll("*")]
          .filter((node) => node.children.length === 0 && /[\u3400-\u9fff\uf900-\ufaff]/u.test(node.textContent ?? ""))
          .slice(0, 10).map((node) => node.textContent.trim().slice(0, 200)),
        imageCount: document.images.length,
        laterLeak: /R0\.76M/i.test(text),
        localProof: /PROVED LOCALLY/i.test(text),
        finite: /FINITE COMPUTATION/i.test(text),
        noFigure: /NO FIGURE|scientific figure: not applicable|正式图件：不适用/i.test(text),
        noSimulation: /NO SIMULATION|Simulation: not run|仿真：未运行/i.test(text),
        uniformAbsorption: /uniform instantaneous absorption|统一瞬时吸收/i.test(text),
        paidEstimate: /complete payment|complete paid|完整支付/i.test(text),
        inverseSquare: /inverse-square|R⁻²/i.test(text),
        initialDependence: /initial-data|initial data|初值依赖/i.test(text),
        completeCancellation: /complete-time cancellation|complete time interval|完整时间抵消/i.test(text),
        contractG: /contract G|合同 G/i.test(text),
        open: /\bOPEN\b/i.test(text),
        clay: /NOT CLAY/i.test(text),
        isClayB: targetId === "clay-b-note",
      };
    }, target.id);
    ...
    assert.equal(audit.mathErrors, 0, `${id} MathJax errors`);
    assert.equal(audit.rawInline, 0, `${id} raw inline TeX`);
    assert.equal(audit.rawDisplay, 0, `${id} raw display TeX`);
    assert.equal(audit.laterLeak, false, `${id} later release leak`);
    assert.equal(audit.sitePaper, scenario.colorScheme === "dark" ? "#181714" : "#f3ecd8", `${id} colour scheme`);
    if (target.id === "clay-b-note") {
      assert.equal(audit.imageCount, 0, `${id} images`);
      for (const key of [
        "localProof", "finite", "noFigure", "noSimulation", "uniformAbsorption", "paidEstimate",
        "inverseSquare", "initialDependence", "completeCancellation", "contractG", "open", "clay",
      ]) assert.equal(audit[key], true, `${id} ${key}`);
    } else {
      assert.equal(audit.imageCount, 1, `${id} preserved L image`);
    }
    if (scenario.lang === "en") {
      assert.equal(audit.chinese, 0, `${id} untranslated Chinese: ${JSON.stringify(audit.chineseSamples)}`);
