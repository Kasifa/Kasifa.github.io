...
const outputRoot = resolve(process.env.R075J_STEP35_BROWSER_QA_OUTPUT ?? "/tmp/r075j-step35-browser-qa");
const externalBase = process.env.R075J_PUBLIC_BASE_URL;
...
try {
  for (const scenario of scenarios) {
    ...
    const url = new URL("/notes/r0-75j.html", baseUrl);
    url.searchParams.set("lang", scenario.lang);
    url.searchParams.set("qa", "r075j-step35");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        figureSections: document.querySelectorAll('#figure, [src*="r075j"], [href*="assets/r075j"]').length,
        meanZeroSource: /MEAN-ZERO SOURCE/.test(visibleText),
        exactAdjointSignChanging: /EXACT ADJOINT SIGN-CHANGING/.test(visibleText),
        dualityExact: /DUALITY EXACT/.test(visibleText),
        negativeWeightedDissipation: /NEGATIVE WEIGHTED DISSIPATION/.test(visibleText),
        constantShiftCanceled: /CONSTANT SHIFT CANCELED/.test(visibleText),
        surchargeCd: /SURCHARGE CD/.test(visibleText),
        positiveMajorantViable: /POSITIVE MAJORANT VIABLE/.test(visibleText),
        initialRowUnpaid: /INITIAL ROW UNPAID/.test(visibleText),
        notBlanketNoGo: /NOT BLANKET NO-GO/.test(visibleText),
    ...
    assert.equal(audit.figureSections, 0, `${scenario.id} no R0.75J formal figure`);
    assert.equal(audit.meanZeroSource, true, `${scenario.id} mean-zero source boundary`);
    assert.equal(audit.exactAdjointSignChanging, true, `${scenario.id} sign-changing exact adjoint boundary`);
    assert.equal(audit.dualityExact, true, `${scenario.id} exact duality boundary`);
    assert.equal(audit.negativeWeightedDissipation, true, `${scenario.id} unfavorable dissipation boundary`);
    assert.equal(audit.constantShiftCanceled, true, `${scenario.id} constant-shift cancellation`);
    assert.equal(audit.surchargeCd, true, `${scenario.id} CD surcharge`);
    assert.equal(audit.positiveMajorantViable, true, `${scenario.id} positive-majorant viability`);
    assert.equal(audit.initialRowUnpaid, true, `${scenario.id} unpaid initial row`);
    assert.equal(audit.notBlanketNoGo, true, `${scenario.id} non-blanket no-go boundary`);
