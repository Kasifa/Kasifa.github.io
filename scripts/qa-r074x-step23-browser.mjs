...
const outputRoot = resolve(process.env.R074X_STEP22_BROWSER_QA_OUTPUT ?? "/tmp/r074x-step23-browser-qa");
...
try {
  ...
  for (const scenario of scenarios) {
    ...
    const response = await page.goto(
      `http://127.0.0.1:${address.port}/notes/r0-74x.html?lang=${scenario.lang}`,
    ...
    const audit = await page.evaluate((expectedFigureId) => {
      ...
      return {
        ...
        twoCoordinateLabels: [...document.querySelectorAll(".label")].filter((node) => node.textContent.includes("TWO COORDINATES")).length,
        actualGateNotProved: /actual.{0,80}(NOT PROVED|not proved)/i.test(visibleText),
        cubicPaymentNoGo: /CUBIC PAYMENT NO-GO|NO-GO BY CUBIC PAYMENT/i.test(visibleText),
    ...
    assert.match(audit.figureCurrentSource, /r074x\/fig-r074x-three-packet-payment-gate\.svg/);
    assert.equal(audit.twoCoordinateLabels, 1, `${scenario.id} two-coordinate prominence`);
    assert.equal(audit.actualGateNotProved, true, `${scenario.id} actual-gate boundary`);
    assert.equal(audit.cubicPaymentNoGo, true, `${scenario.id} cubic-payment no-go`);
    ...
    if (scenario.lang === "en") {
      ...
      assert.match(audit.h1, /Three-packet fixed-deletion endpoint obstruction/);
      assert.equal(audit.switcher, "中文");
      assert.ok(audit.chinesePdfLabels >= 1, `${scenario.id} Chinese-PDF label`);
    } else {
      assert.match(audit.h1, /三 packet fixed-deletion endpoint obstruction/);
