...
// R0.73W translation stage. English copy is accepted only from the local
...
const usage = "usage: add-r073w-translations.mjs (--apply | --check-only | --capture-missing)";
...
const root = resolve(process.env.R073W_RELEASE_ROOT ?? resolve(import.meta.dirname, ".."));
const publicDirectory = resolve(root, "public");
const translationPath = resolve(root, "translations/en.json");
const bundlePath = resolve(publicDirectory, "i18n-en.js");
const snapshotPath = resolve(root, "scripts/i18n-snapshots/r073w-missing.json");
const dictionaryPath = resolve(root, "research/r073w_bilingual_dictionary.md");
const reportPath = resolve(root, "research/r073w_report-source.md");
...
// in this executable. Populate it only after the R0.73W HTML transaction has
// materialized the complete missing-string set.
const activePages = [
  "literature-review.html",
  "notes/index.html",
  "notes/r0-73w.html",
  "recap-r0-61-r0-73w.html",
...
function assertInsideRoot(path, label) {
  ...
  if (!offset || offset === ".." || offset.startsWith("../") || offset.startsWith("..\\")) {
    throw new Error(label + " escaped R073W_RELEASE_ROOT");
  }
  return offset;
}
...
async function safeAtomicWrite(path, payload) {
  ...
  const temporary = resolve(
    dirname(path),
    `.${path.split("/").at(-1)}.r073w-${process.pid}-${Date.now()}-${process.hrtime.bigint()}.tmp`,
  );
  try {
...
async function writeTransaction(writes) {
  if (writes.length !== 3 || new Set(writes.map(({ path }) => path)).size !== 3) {
    throw new Error("R0.73W translation transaction requires three distinct files");
  }
  const nonce = `${process.pid}-${Date.now()}-${process.hrtime.bigint()}`;
  const rows = writes.map(({ path, payload }, index) => ({
    path,
    payload,
    temporary: resolve(dirname(path), `.${path.split("/").at(-1)}.r073w-${nonce}-${index}.tmp`),
    backup: resolve(dirname(path), `.${path.split("/").at(-1)}.r073w-${nonce}-${index}.bak`),
    backedUp: false,
    installed: false,
  }));
  try {
...
if (action !== "--capture-missing") {
  snapshotInfo = await lstatOrMissing(snapshotPath, "R0.73W translation snapshot");
  if (!snapshotInfo) {
    throw new Error(
      "R0.73W translation snapshot is absent; wait for staged v1.63 HTML, then run --capture-missing before --apply or --check-only",
    );
  }
  if (snapshotInfo.isSymbolicLink() || !snapshotInfo.isFile()) {
    throw new Error("R0.73W translation snapshot must be a regular nonsymlink file");
  }
}

const dictionary = await regularText(dictionaryPath, "R0.73W bilingual dictionary");
const report = await regularText(reportPath, "R0.73W report source");
const releaseTitle = metadataField(dictionary, "Release title");
const publicTitle = metadataField(dictionary, "Public title \\(zh\\)");
if (releaseTitle !== "R0.73W | Signed subfilter production: heat-plane characteristics, the energy-class boundary, and exact counterexamples" ||
    publicTitle !== "R0.73W｜带符号亚滤波 production：heat-plane 特征线、能量类边界与精确反例") {
  throw new Error("R0.73W bilingual dictionary canonical title ledger is absent or drifted");
}
for (const token of requiredDictionaryTokens) {
  if (!dictionary.includes(token)) throw new Error("R0.73W dictionary missing " + token);
}
for (const token of requiredReportTokens) {
  if (!report.includes(token)) throw new Error("R0.73W report missing " + token);
}
for (const token of forbiddenClaimTokens) {
  if (dictionary.includes(token) || report.includes(token)) {
    throw new Error("R0.73W canonical source contains forbidden claim " + token);
  }
}
for (const stale of [
  ...
  if (dictionary.includes(stale) || report.includes(stale)) {
    throw new Error("R0.73W canonical source still carries " + stale);
  }
}
...
if (
  releaseManifest.latestCompletedRelease !== "r073w" ||
  releaseManifest.siteVersion !== "1.63" ||
  releaseManifest.publicHtmlNoteCount !== 199 ||
  releaseManifest.postR060RecapNodeCount !== 139 ||
  releaseManifest.nextRelease !== "r073x" ||
  releaseManifest.postR070APublishedReleaseCount !== 101 ||
  releaseManifest.postR070AFormalSealedReleaseCount !== 77 ||
  releaseManifest.legacyFormalFigureBacklogCount !== 24 ||
  siteVersion.latestRelease !== "R0.73W" ||
  siteVersion.version !== "1.63" ||
  siteVersion.publicHtmlNoteCount !== 199
) {
  throw new Error("R0.73W HTML/accounting apply must precede translation work");
}
...
for (const relativePage of activePages) {
  ...
  if (!html.includes("/i18n-en.js?v=1.63")) {
    throw new Error(relativePage + ": expected i18n cache version v1.63");
  }
  if (!html.includes("R0.73W")) throw new Error(relativePage + ": R0.73W marker absent");
...
const boundaryTokens = (value) =>
  value.match(/NOT CLAY|\b(?:VERIFIED_CLASSICAL_REDERIVED|VERIFIED_CLASSICAL_RECONSTRUCTION|VERIFIED_CLASSICAL_WITH_ADAPTATION|VERIFIED_CLASSICAL|INTERNAL_UNCONDITIONAL_AUDITED|INTERNAL_CRITICAL_AUDITED|INTERNAL_EXACT_AUDITED|SEALED_COMMIT_BOUND|INTERNAL_EXACT_SCALING|INTERNAL_COROLLARY|INTERNAL_CONDITIONAL|INTERNAL_EXACT|NOT_PROVED|CLOSED_EXACT|FALSE_AS_INFERENCE|FORBIDDEN|REQUIRED|NOT_RUN|PENDING|CLOSED|OPEN|PASS|FALSE|TRUE)\b/g) ?? [];
...
if (JSON.stringify(releaseBatchFiles) !== JSON.stringify(activePages)) {
  throw new Error("unexpected R0.73W release-batch files: " + JSON.stringify(releaseBatchFiles));
}
...
function capturedReasonCodes(entry) {
  ...
  if (releaseId.test(existing.id)) {
    return ["R073W_RELEASE_BATCH", ...forcedCodes, ...issueCodes];
  }
...
if (action === "--capture-missing") {
  ...
  console.log(JSON.stringify({
    ...
    snapshot: "scripts/i18n-snapshots/r073w-missing.json",
...
if (!snapshotInfo) throw new Error("R0.73W translation snapshot state was not established");

const snapshot = JSON.parse(await regularText(snapshotPath, "R0.73W translation snapshot"));
if (!Array.isArray(snapshot) || snapshot.length === 0) throw new Error("empty R0.73W snapshot");
if (snapshot.some((entry) => !entry || typeof entry !== "object" || Array.isArray(entry) ||
  ...
  throw new Error("R0.73W accepts only local-direct-reviewed translation rows");
}
const snapshotByChinese = new Map();
for (const entry of snapshot) {
  if (snapshotByChinese.has(entry.zh)) throw new Error("duplicate R0.73W snapshot key");
  if (!sourceByChinese.has(entry.zh)) throw new Error("R0.73W snapshot contains a non-live key: " + entry.zh);
  if (new Set(entry.reasonCodes).size !== entry.reasonCodes.length) {
    throw new Error("R0.73W snapshot contains duplicate reason codes: " + entry.zh);
  }
  if (new Set(entry.reviewedIssues).size !== entry.reviewedIssues.length) {
    throw new Error("R0.73W snapshot contains duplicate reviewed issues: " + entry.zh);
  }
  if (entry.captureBindingSha256 !== captureBindingSha256(entry)) {
    throw new Error("R0.73W snapshot capture binding drift: " + entry.zh);
  }
  snapshotByChinese.set(entry.zh, entry);
}
for (const entry of releaseBatch) {
  if (!snapshotByChinese.has(entry.zh)) {
    throw new Error("R0.73W snapshot is missing a release-batch row: " + entry.zh);
  }
}
for (const entry of currentLegacyRepairs) {
  if (!snapshotByChinese.has(entry.zh)) {
    ...
    throw new Error(
      "R0.73W snapshot is missing a reviewable historical row " + entry.zh + ": " +
      (issues.length
        ? issues.map(({ code, message }) => `${code} (${message})`).join(", ")
        : "HUMAN_SEMANTIC_REVIEW"),
    );
...
for (const entry of snapshot) {
  if (entry.reviewedIssues.some((code) => !issueReasonCodes.has(code))) {
    throw new Error("R0.73W snapshot contains an unknown reviewed issue: " + entry.zh);
  }
  const existing = existingByChinese.get(entry.zh);
  if (entry.sourceIdAtCapture !== null) {
    if (!existing || existing.id !== entry.sourceIdAtCapture) {
      throw new Error("R0.73W snapshot source-id binding drift: " + entry.zh);
    }
    if (entry.capturedEnglishSha256 === null) {
      throw new Error("R0.73W existing-row capture hash is absent: " + entry.zh);
    }
  } else if (existing && !releaseId.test(existing.id)) {
    throw new Error("R0.73W new-row source-id lifecycle drift: " + entry.zh);
  } else if (entry.capturedEnglishSha256 !== null) {
    throw new Error("R0.73W new-row capture hash must be null: " + entry.zh);
  }
  ...
  const expectedReasonCodes = entry.sourceIdAtCapture === null
    ...
    : releaseId.test(entry.sourceIdAtCapture)
      ? ["R073W_RELEASE_BATCH", ...forcedCodes, ...entry.reviewedIssues]
      : [...forcedCodes, ...entry.reviewedIssues];
  if (JSON.stringify(entry.reasonCodes) !== JSON.stringify(expectedReasonCodes)) {
    throw new Error("R0.73W snapshot reasonCodes binding drift: " + entry.zh);
  }
  ...
  if (existing && entry.capturedEnglishSha256 !== null) {
    ...
    if (currentEnglishSha256 === entry.capturedEnglishSha256) {
      if (JSON.stringify(entry.reviewedIssues) !== JSON.stringify(currentIssueCodes)) {
        throw new Error("R0.73W snapshot captured-issue binding drift: " + entry.zh);
      }
    } else if (currentEnglishSha256 !== sha256Text(englishIssues(entry.zh, entry.en).en)) {
      throw new Error(
        "R0.73W existing English is neither captured nor reviewed snapshot English: " + entry.zh,
      );
    }
  }
  ...
  if (!existing || releaseId.test(existing.id) ||
    ...
    throw new Error("R0.73W snapshot has an unproved historical repair row: " + entry.zh);
  }
}
...
if (JSON.stringify(snapshot.map(({ zh }) => zh)) !== JSON.stringify(reviewBatch.map(({ zh }) => zh))) {
  throw new Error("R0.73W review snapshot order or membership drift");
}
...
for (const entry of snapshot) {
  ...
  if (entry.resolution === "semantic-equivalent-approved") {
    ...
    if (review.issues.length) {
      throw new Error(
        "R0.73W review snapshot requires corrected English for " + entry.zh + ": " +
        review.issues.map(({ code, message }) => `${code} (${message})`).join(", "),
      );
    }
    reviewedEnglishByChinese.set(entry.zh, review.en);
...
function allocateReleaseId() {
  while (true) {
    const candidate = "r073w" + String(nextReleaseId).padStart(3, "0");
    nextReleaseId += 1;
...
if (action === "--check-only") {
  if (await regularText(snapshotPath, "R0.73W translation snapshot") !== snapshotOutput) {
    throw new Error("R0.73W translation snapshot is stale");
  }
  if (JSON.stringify(translations) !== JSON.stringify(finalTranslations)) {
    throw new Error("R0.73W translations are stale");
  }
  if (await regularText(bundlePath, "public/i18n-en.js") !== bundleOutput) {
    throw new Error("R0.73W translation bundle is stale");
  }
} else {
