...
const protectedArtifacts = {
  "public/notes/clay-b-recent-source-screen-20260906.html": "61e2ef7f0e3d725ca2e085136d6f72adf5ac818e20636ebd7a2865ffa2516d25",
...
test("the HTML-only PressureMechanismScreen note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-pressure-mechanism-screen-20260906.html");
  for (const marker of [
    "ClayB-PressureMechanismScreen-20260906", "CB.14", "空间恒等式可以周期化", "The spatial identity transfers to the torus",
    "平滑周期修正已付", "The smooth periodic correction is paid", "基本能量给时间可积压力势", "Basic energy makes the pressure potential time-integrable",
    "它明确不是 NS 解", "it is explicitly not an NS solution", "不声称 4/3 是最优时间指数", "does not claim that 4/3 is the optimal time exponent",
    "EXACT SPATIAL IDENTITY", "ENERGY-CLASS OBSTRUCTION", "OPEN", "NOT CLAY",
  ...
  assert.equal(note.includes("/notes/clay-b-pressure-mechanism-screen-20260906.pdf"), false);
  assert.equal(note.includes("R0.76M"), false);
});

test("homepage keeps the solid R0 sequence separate from the dashed CB.1–CB.14 lane", async () => {
  ...
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-pressure-mechanism-screen-20260906"));
  ...
  const positions = independentChapters.map(([chapter, slug]) => {
    ...
    assert.ok(home.slice(row, row + 1800).includes(chapter), `${chapter} route label`);
    return row;
  });
  assert.deepEqual(positions, [...positions].sort((a, b) => a - b));
  assert.equal((home.match(/class="route-overview independent-release-spotlight"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-pressure-mechanism-screen"/g) ?? []).length, 1);
  assert.equal((literature.match(/id="clay-b-pressure-mechanism-screen-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-pressure-mechanism-screen-20260906"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-pressure-mechanism-screen-20260906.pdf"), false);
  assert.equal(site.version, "2.58");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 14);
  assert.equal(site.latestIndependentNote, "ClayB-PressureMechanismScreen-20260906");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-pressure-mechanism-screen-20260906.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.latestIndependentChapter, "CB.14");
  assert.equal(site.nextIndependentChapter, "CB.15");
  assert.equal(manifest.siteVersion, "2.58");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestPublication.releaseId, "clay-b-pressure-mechanism-screen-20260906");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-RecentSourceScreen-20260906");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.14");
});
...
test("historical milestones, endpoint, prior note, and retained PDFs remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-pressure-mechanism-screen-20260906.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-pressure-mechanism-screen-20260906.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-pressure-mechanism-screen-20260906.pdf")));
});
