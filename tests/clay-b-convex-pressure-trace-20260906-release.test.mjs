...
const protectedArtifacts = {
  "public/notes/clay-b-adjoint-weak-trace-screen-20260906.html": "37ccc97089ba6bddfc6727c993dbff5fefcfdb8c0b7f61c46665b4317514b912",
...
const independentChapters = [
  ...
  ["CB.21", "clay-b-convex-pressure-trace-20260906"],
];

test("the HTML-only ConvexPressureTrace note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-convex-pressure-trace-20260906.html");
  for (const marker of [
    "ClayB-ConvexPressureTrace-20260906", "CB.21", "压力梯度具有足够的时间可积性",
    "The pressure gradient has sufficient time integrability", "所有 1≤q<2 的强零初迹",
    "Strong-zero initial trace holds for every 1≤q<2", "普遍消压的局部测试分类",
    "Classifying local tests that universally cancel pressure", "PROVED IN STATED SCOPE",
    "DIRECT DERIVATION", "CONDITIONAL ENDPOINT", "LITERATURE", "FINITE CHECKS ONLY", "OPEN", "NOT CLAY",
  ...
  assert.equal(note.includes("/notes/clay-b-convex-pressure-trace-20260906.pdf"), false);
  assert.equal(note.includes("R0.76M"), false);
});

test("homepage keeps the solid R0 sequence separate from the dashed CB.1–CB.21 lane", async () => {
  ...
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-convex-pressure-trace-20260906"));
  ...
  assert.equal((home.match(/id="clay-b-convex-pressure-trace"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-adjoint-weak-trace-screen"/g) ?? []).length, 0);
  assert.equal((literature.match(/id="clay-b-convex-pressure-trace-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-convex-pressure-trace-20260906"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-convex-pressure-trace-20260906.pdf"), false);
  assert.equal(site.version, "2.65");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 21);
  assert.equal(site.latestIndependentNote, "ClayB-ConvexPressureTrace-20260906");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-convex-pressure-trace-20260906.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.latestIndependentChapter, "CB.21");
  assert.equal(site.nextIndependentChapter, "CB.22");
  assert.equal(manifest.siteVersion, "2.65");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestPublication.releaseId, "clay-b-convex-pressure-trace-20260906");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-AdjointWeakTraceScreen-20260906");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.21");
});
...
test("prior note, canonical endpoint, recap, and historical PDF remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-convex-pressure-trace-20260906.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-convex-pressure-trace-20260906.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-convex-pressure-trace-20260906.pdf")));
});
