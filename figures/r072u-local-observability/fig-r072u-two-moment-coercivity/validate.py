#!/usr/bin/env python3
"""Strict fail-closed validation of a rendered R0.72U figure package."""
...
CERTIFICATE_ROOT = REPOSITORY / "research/certificates/r072u"
...
def validate_certificate_commit(manifest: dict) -> None:
    ...
    for name in (
        ...
        relative = f"research/certificates/r072u/{name}"
...
def validate_claim_boundaries(*boundaries: dict) -> None:
    required_false = {
...
def validate_ledger() -> None:
    ...
    if set(actual) != SOURCE_FILES | GENERATED - {"SHA256SUMS"}:
        raise RuntimeError("formal figure package contains an unexpected or missing file")
...
def main() -> None:
    ...
    certificate = load(CERTIFICATE_ROOT / "certificate.json")
    if (
        manifest.get("status") != "formal"
        or manifest.get("release") != "R0.72U"
        or manifest.get("figureId") != FIGURE_ID
        or contract.get("figureId") != FIGURE_ID
        or contract.get("stage") != "source-only"
    ):
        raise RuntimeError("formal R0.72U identity or source contract drift")
    ...
    validate_certificate_commit(manifest)
    validate_claim_boundaries(
        manifest.get("claimBoundary"), contract.get("claimBoundary"),
        certificate.get("claimBoundary"),
    )

    rows = list(csv.DictReader((ROOT / "data.csv").open(encoding="utf-8")))
    if len(rows) != 2406 or {row.get("panel") for row in rows} != {"A", "B", "C"}:
        raise RuntimeError("analytic sample ledger drift")
    allowed = {"analytic presentation sample", "exact inviscid calibration sample"}
    ...
    if (
        ...
        or "whole-line block contraction" not in results.get("claimsNotMade", [])
    ):
        raise RuntimeError("result claim boundary drift")
    for record in manifest.get("figure", {}).get("outputs", []):
        path = ROOT / record.get("path", "")
        if not path.is_file() or path.is_symlink() or digest(path) != record.get("sha256"):
            raise RuntimeError(f"figure output drift: {path.name}")
    publication = manifest.get("publication", {})
    if (
        publication.get("directory") != "public/assets/r072u"
    ...
    if set(GENERATED) - {path.name for path in ROOT.iterdir() if path.is_file()}:
    ...
    if generic.returncode:
        raise RuntimeError(
            f"generic archive validation failed:\n{generic.stdout}\n{generic.stderr}"
        )
    print("R0.72U strict formal figure validation: passed")
