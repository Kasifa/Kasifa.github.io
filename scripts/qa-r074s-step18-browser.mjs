...
const outputRoot = resolve(process.env.R074S_STEP18_BROWSER_QA_OUTPUT ?? "/tmp/r074s-step18-browser-qa");
...
try {
  ...
  for (const scenario of scenarios) {
    ...
    const response = await page.goto(
      `http://127.0.0.1:${address.port}/notes/r0-74s.html?lang=${scenario.lang}`,
    ...
    const audit = await page.evaluate(() => {
      ...
      const figure = document.querySelector('#figure img[src*="fixed-deletion-quantifier-gap"]');
      ...
      return {
        ...
        figureComplete: Boolean(figure?.complete && figure.naturalWidth > 0),
    ...
    assert.ok(audit.mathJaxContainers > 250, `${scenario.id} MathJax coverage`);
    ...
    assert.ok(audit.images.every((image) => image.complete && image.width > 0), `${scenario.id} image load`);
    assert.equal(audit.figureComplete, true, `${scenario.id} formal figure`);
