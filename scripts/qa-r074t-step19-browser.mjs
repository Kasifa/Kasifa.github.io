...
const outputRoot = resolve(process.env.R074T_STEP19_BROWSER_QA_OUTPUT ?? "/tmp/r074t-step19-browser-qa");
...
try {
  ...
  for (const scenario of scenarios) {
    ...
    const response = await page.goto(
      `http://127.0.0.1:${address.port}/notes/r0-74t.html?lang=${scenario.lang}`,
    ...
    const audit = await page.evaluate(() => {
      ...
      const figure = document.querySelector('#figure img[src*="schedule-invariant-dwell-barrier"]');
