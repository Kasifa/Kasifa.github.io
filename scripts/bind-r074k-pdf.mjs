...
// Render and bind the complete Chinese R0.74K note. This certifies publication
...
const release = "r074k";
const noteSlug = "r0-74k";
const title = "R0.74K｜自由热指数为何只卡在最近内领圈";
...
try {
  ...
  if (structure.title !== title) throw new Error("R0.74K PDF title drift");
  ...
  ) throw new Error("R0.74K render provenance mismatch");

  const binding = {
    schemaVersion: "r074k-synchronized-pdf-binding-v1",
    release: "R0.74K",
