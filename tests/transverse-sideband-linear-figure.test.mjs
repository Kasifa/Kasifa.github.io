...
const packageRoot = new URL(
  "../figures/r069c-transverse-linear/fig-r069c-transverse-linear/",
  import.meta.url,
);
...
test("archives the formal R0.69C transverse-linear figure", async () => {
  const [manifestText, symbols, decay, validationText, svg, png, pdf] =
    await Promise.all([
      readFile(new URL("manifest.json", packageRoot), "utf8"),
      readFile(new URL("symbol-curves.csv", packageRoot), "utf8"),
      readFile(new URL("decay-envelopes.csv", packageRoot), "utf8"),
  ...
  assert.equal(manifest.figureId, "fig-r069c-transverse-linear");
  ...
  assert.match(manifest.supportedClaim, /converges to free heat/);
  assert.match(manifest.claimBoundary, /not a nonlinear/);
  assert.equal(symbols.trim().split("\n").length, 4 * 161 + 1);
  assert.equal(decay.trim().split("\n").length, 52);
...
test("publishes byte-exact mirrors of the R0.69C figure", async () => {
  for (const extension of ["svg", "png", "pdf"]) {
    const [archived, published] = await Promise.all([
      ...
      readFile(
        new URL("figures/r0-69c-transverse-linear." + extension, publicRoot),
      ),
    ]);
