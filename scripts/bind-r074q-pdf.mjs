...
// Render and cryptographically bind the complete Chinese R0.74Q reader PDF.
...
try {
  const htmlRelative = "public/notes/r0-74q.html";
  const pdfRelative = "public/notes/r0-74q.pdf";
  const provenanceRelative = "research/r074q_note_pdf_render.json";
  const bindingRelative = "research/r074q_pdf_bindings.json";
  const url = `http://127.0.0.1:${address.port}/notes/r0-74q.html?lang=zh`;
  ...
  const title = "R0.74Q｜许多壳层同时亮起，为什么仍然压不低支付？";
  ...
  const binding = {
    schemaVersion: "r074q-note-synchronized-pdf-binding-v1",
    release: "R0.74Q",
    kind: "note",
    ...
    claimBoundary: {
      ...
      evidenceClassesSeparated: ["PROVED", "INHERITED", "FINITE", "CONDITIONAL", "LITERATURE BOUNDARY", "OPEN", "NOT CLAY"],
      terminalEffectiveShellReduction: "PROVED",
      commonShearFiniteExactNse: "PROVED_PRIOR_STRUCTURE",
      frozenAngleCommonCalibrationObstruction: "PROVED_FOR_STATED_INHERITED_GEOMETRY",
      relaxedGrowingFiniteMultipacketGeometry: "PROVED",
      equalTargetAllLobeDominance: "PROVED",
      targetClockLowerBound: "PROVED",
      completeMatchedSquareFunctionUpperBound: "OPEN",
      trueVelocityCubicPaymentObstruction: "PROVED",
      signedCumulativeFluxLowerBound: "OPEN",
      fixedScaleInequality: "OPEN",
      formalFigure: "NOT_APPLICABLE",
  ...
  process.stdout.write(`${JSON.stringify({ applied: true, release: "R0.74Q", pageCount: structure.pageCount, sha256: sha256(pdf), bytes: pdf.length }, null, 2)}\n`);
} finally {
