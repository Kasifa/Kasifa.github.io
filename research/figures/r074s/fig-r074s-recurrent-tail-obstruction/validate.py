#!/usr/bin/env python3
"""Validate and two-stage seal the 25-file R0.74S figure archive."""
...
FIGURE_SCHEMA_VERSION = "r074s-recurrent-tail-obstruction-manifest-v1"
PENDING_FIGURE_SOURCE_COMMIT = "PENDING_FIGURE_SOURCE_COMMIT"
HEX40 = re.compile(r"[0-9a-f]{40}")
FORBIDDEN_BYTES = (b"/Users/" + b"kasifa", b"/private/" + b"tmp/" + b"r074s-")
...
def load_plot() -> Any:
    spec = importlib.util.spec_from_file_location("r074s_recurrent_tail_plot_validation", HERE / "plot.py")
...
def capture_baseline(path: Path) -> None:
    ...
    payload = {
        ...
        "schema": "r074s-recurrent-tail-deterministic-baseline-v1",
    }
...
def validate_content(repository: Path, confirm_visual_qa: bool) -> tuple[list[dict[str, object]], dict[str, Any]]:
    ...
    add(checks, "no-source-raw-symlinks", all(not (HERE / name).is_symlink() for name in SOURCE_FILES + RAW_FILES))
    add(checks, "figure-id-consistency", config["figureId"] == contract["figureId"] == FIGURE_ID)
    add(checks, "explicit-claim-boundary", all((
        contract["claimBoundary"]["analyticExactField"] is True,
        ...
        contract["claimBoundary"]["openS472Proved"] is False,
    )))
    ...
    add(checks, "frozen-core-ancestor-and-blobs", len(source_blobs) == 3,
    ...
    add(checks, "results-source-binding", results.get("sourceBinding", {}).get("commit") == config["sourceBinding"]["commit"])
    ...
    add(checks, "period-quadrature", expected_audit["periodQuadratureDifference"] <= expected_audit["tolerances"]["periodQuadratureDifference"])
    add(checks, "rk4-closure", expected_audit["closureErrorL2"] <= expected_audit["tolerances"]["closureErrorL2"])
    add(checks, "streamline-level-invariant", max(expected_audit["maximumRk4LevelResidual"], expected_audit["maximumAnalyticLevelResidual"]) <= expected_audit["tolerances"]["levelResidual"])
    add(checks, "g-range", max(expected_audit["gMinimumError"], expected_audit["gMaximumError"]) <= expected_audit["tolerances"]["gExtremaError"])
    add(checks, "q-equals-g-prime", expected_audit["maximumCentralDerivativeError"] <= expected_audit["tolerances"]["centralDerivativeError"])
    add(checks, "one-cycle-variation", expected_audit["onePeriodVariationError"] <= expected_audit["tolerances"]["variationError"])
    add(checks, "signed-cycle-return", expected_audit["signedOnePeriodDrift"] <= expected_audit["tolerances"]["endpointDrift"])
    add(checks, "cumulative-variation-monotone", expected_audit["minimumCumulativeVariationIncrement"] >= -expected_audit["tolerances"]["monotonicitySlack"])
    add(checks, "amplitude-log-slopes", max(expected_audit["maximumQuadraticLogSlopeError"], expected_audit["maximumCubicLogSlopeError"]) <= 5.0e-13)
    ...
    add(checks, "pdf-scope-labels", "NOT DNS" in extracted and "NOT CLAY" in extracted)
    ...
    add(checks, "svg-scope-labels", "NOT DNS" in svg_text and "NOT CLAY" in svg_text)
    ...
    add(checks, "progress-monitor", len(progress) >= 6 and all({"utc", "pid", "elapsedSeconds", "event"}.issubset(row) for row in progress), events=[row["event"] for row in progress])
    add(checks, "resource-monitor", len(resources) == 1 and {"utc", "pid", "wallSeconds", "cpuSeconds", "maximumResidentSetSizeRaw"}.issubset(resources[0]))
    add(checks, "environment-observation",
        environment["python"] == config["runtime"]["python"]
        and environment["matplotlibConfigPolicy"] in {
            "explicit external environment directory", "system temporary directory removed after render"
        })
    add(checks, "machine-specific-literals-absent",
        not scan_machine_literals(set(SOURCE_FILES + RAW_FILES)),
        failures=scan_machine_literals(set(SOURCE_FILES + RAW_FILES)))
...
def run_negative_tests(plot: Any, config: dict[str, Any]) -> dict[str, object]:
    ...
    drifted["runtime"]["python"] = "0.0.0"
    runtime_failed = False
    try:
        plot.live_runtime_versions(drifted)
    except RuntimeError:
        runtime_failed = True
...
def make_qa_report(checks: list[dict[str, object]], context: dict[str, Any],
    ...
    return f"""# R0.74S recurrent-tail formal figure QA report
...
- orbit period: `{audit['period']:.15f}`
- period quadrature discrepancy: `{audit['periodQuadratureDifference']:.3e}`
- RK4 closure error: `{audit['closureErrorL2']:.3e}`
- maximum streamline-level residual: `{audit['maximumRk4LevelResidual']:.3e}`
- one-period absolute variation: `{audit['onePeriodAbsoluteVariation']:.15f}`
- signed one-period drift: `{audit['signedOnePeriodDrift']:.3e}`
...
def build_manifest(checks: list[dict[str, object]], context: dict[str, Any],
    ...
    return {
        ...
        "computation": {
            ...
            "precision": "analytic formulas with deterministic IEEE-754 binary64 quadrature and RK4 rendering",
            "solver": "none for PDE; fixed-step RK4 only for exact orbit time parametrization",
...
def write_seal(repository: Path, baseline: Path | None, confirm_visual_qa: bool,
    ...
    validation = {
        ...
        "pid": os.getpid(),
        "schemaVersion": "r074s-recurrent-tail-validation-v1",
    ...
    with tempfile.TemporaryDirectory(prefix="r074s-figure-seal-") as temporary:
        ...
        checksum_rows = [
            f"{sha256(HERE / name)}  {name}" for name in sorted(EXPECTED_FILES - {"SHA256SUMS"})
        ]
...
def main() -> None:
    ...
    if args.capture_deterministic_core:
        ...
        write_seal(
            repository,
            args.determinism_baseline,
            True,
            args.figure_source_commit or None,
        )
