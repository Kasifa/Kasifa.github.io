...
const outputRoot = resolve(process.env.R075Q_STEP42_BROWSER_QA_OUTPUT ?? "/tmp/r075q-step42-browser-qa");
const externalBase = process.env.R075Q_PUBLIC_BASE_URL;
...
try {
  for (const scenario of scenarios) {
    ...
    const url = new URL("/notes/r0-75q.html", baseUrl);
    url.searchParams.set("lang", scenario.lang);
    url.searchParams.set("qa", "r075q-step42");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        step42MathJaxErrors: document.querySelectorAll("#s-328 mjx-merror, #s-329 mjx-merror, #s-330 mjx-merror, #s-331 mjx-merror, #s-332 mjx-merror, #s-333 mjx-merror, #s-334 mjx-merror").length,
        ...
        figureSections: document.querySelectorAll('#figure, [src*="r075q"], [href*="assets/r075q"]').length,
        constantShear: /CONSTANT SHEAR/.test(visibleText),
        oneRealHarmonic: /ONE REAL HARMONIC/.test(visibleText),
        spatiallySpread: /SPATIALLY SPREAD/.test(visibleText),
        exactZeroRow: /EXACT ZERO ROW/.test(visibleText),
        radialDerivativeL1: /RADIAL DERIVATIVE L1/.test(visibleText),
        phaseUniformPeriods: /PHASE-UNIFORM PERIODS/.test(visibleText),
        rectangularSubcollar: /RECTANGULAR SUBCOLLAR/.test(visibleText),
        collarCubic: /3D COLLAR CUBIC/.test(visibleText),
        packetGain: /K\^-2\/3 GAIN/.test(visibleText),
        noEntranceConcentration: /NO ENTRANCE CONCENTRATION/.test(visibleText),
    ...
    assert.equal(audit.step42MathJaxErrors, 0, `${scenario.id} Step 42 MathJax errors: ${JSON.stringify(audit.mathJaxErrorContexts.filter((entry) => /^s-3(?:2[8-9]|3[0-4])$/.test(entry.section ?? "")))}`);
    ...
    assert.equal(audit.figureSections, 0, `${scenario.id} no R0.75Q formal figure`);
    for (const key of [
      "constantShear", "oneRealHarmonic", "spatiallySpread", "exactZeroRow", "radialDerivativeL1",
      "phaseUniformPeriods", "rectangularSubcollar", "collarCubic", "packetGain", "noEntranceConcentration",
      "actualComponentOnly", "projectionExcluded", "multimodeOpen", "e24Open",
