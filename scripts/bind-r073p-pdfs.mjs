...
// Cryptographically bind the two synchronized R0.73P PDFs to their HTML
...
const usage = "usage: bind-r073p-pdfs.mjs (--apply | --check-only)";
...
const outputPath = resolve(root, "research/r073p_pdf_bindings.json");
...
function canonicalTitles(dictionary) {
  if (/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/.test(dictionary)) {
    throw new Error("R0.73P bilingual dictionary contains control characters");
  }
  const releaseTitle = dictionary.match(/^\*\*Release title:\*\*\s*\*?(.+?)\*?\s*$/m)?.[1]
    ?.replace(/^\*|\*$/g, "").trim();
  const publicChinese = dictionary.match(
    /^\*\*Public title \(zh\):\*\*\s*\*?(.+?)\*?\s*$/m,
  )?.[1]?.replace(/^\*|\*$/g, "").trim();
  if (releaseTitle !== "R0.73P | Critical stability, the N^{-1/2} frequency gate, and the early-time regularity gap" ||
      publicChinese !== "R0.73P｜临界稳定、N^{-1/2} 频率门槛与早期正则缺口") {
    throw new Error("R0.73P canonical title ledger drift");
  }
  for (const token of [
  ...
  return {
    note: releaseTitle.replace(" | ", "｜"),
    publicChinese,
    recap: "R0.61–R0.73P｜R0.60 之后的研究回顾",
  };
}
...
if (
  ...
  throw new Error("R0.73P HTML/accounting apply must precede PDF binding");
}
const documents = [
  {
    kind: "research-note",
    html: "public/notes/r0-73p.html",
    pdf: "public/notes/r0-73p.pdf",
    title: titles.note,
  },
  {
    kind: "cumulative-recap",
    html: "public/recap-r0-61-r0-73p.html",
    pdf: "public/recap-r0-61-r0-73p.pdf",
    title: titles.recap,
  },
];
...
const manifest = {
  schemaVersion: "r073p-synchronized-pdf-bindings-v1",
  release: "R0.73P",
  canonicalTitleSource: {
    path: "research/r073p_bilingual_dictionary.md",
...
if (apply) {
  ...
  if (!current.equals(payload)) throw new Error("R0.73P PDF binding manifest is stale");
}
...
console.log(JSON.stringify({
  ...
  output: "research/r073p_pdf_bindings.json",
}));
