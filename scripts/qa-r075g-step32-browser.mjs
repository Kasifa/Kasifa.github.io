...
const outputRoot = resolve(process.env.R075G_STEP32_BROWSER_QA_OUTPUT ?? "/tmp/r075g-step32-browser-qa");
const externalBase = process.env.R075G_PUBLIC_BASE_URL;
...
try {
  for (const scenario of scenarios) {
    ...
    const url = new URL("/notes/r0-75g.html", baseUrl);
    url.searchParams.set("lang", scenario.lang);
    url.searchParams.set("qa", "r075g-step32");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        figureSections: document.querySelectorAll('#figure, [src*="r075g"], [href*="assets/r075g"]').length,
        exactThreshold: /EXACT THRESHOLD PROVED/.test(visibleText),
        strictThreshold: /STRICT INEQUALITY REQUIRED/.test(visibleText),
        oneThirdSufficient: /ONE THIRD CONDITIONALLY SUFFICIENT/.test(visibleText),
        oneQuarterInsufficient: /ONE QUARTER INSUFFICIENT BY THIS ROUTE/.test(visibleText),
        amplitudeGainImpossible: /AMPLITUDE GAIN IMPOSSIBLE/.test(visibleText),
        residenceThreshold: /RESIDENCE BETA STAR 27163\/35721/.test(visibleText),
        positiveGainOpen: /POSITIVE GAIN OPEN/.test(visibleText),
    ...
    assert.equal(audit.figureSections, 0, `${scenario.id} no R0.75G formal figure`);
    assert.equal(audit.exactThreshold, true, `${scenario.id} exact-threshold boundary`);
    assert.equal(audit.strictThreshold, true, `${scenario.id} strict-threshold boundary`);
    assert.equal(audit.oneThirdSufficient, true, `${scenario.id} one-third boundary`);
    assert.equal(audit.oneQuarterInsufficient, true, `${scenario.id} one-quarter boundary`);
    assert.equal(audit.amplitudeGainImpossible, true, `${scenario.id} amplitude boundary`);
    assert.equal(audit.residenceThreshold, true, `${scenario.id} residence threshold`);
    assert.equal(audit.positiveGainOpen, true, `${scenario.id} positive-gain boundary`);
