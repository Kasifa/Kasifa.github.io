...
const outputRoot = resolve(process.env.R075N_STEP39_BROWSER_QA_OUTPUT ?? "/tmp/r075n-step39-browser-qa");
const externalBase = process.env.R075N_PUBLIC_BASE_URL;
...
try {
  for (const scenario of scenarios) {
    ...
    const url = new URL("/notes/r0-75n.html", baseUrl);
    url.searchParams.set("lang", scenario.lang);
    url.searchParams.set("qa", "r075n-step39");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        step39MathJaxErrors: document.querySelectorAll("#s-308 mjx-merror, #s-309 mjx-merror, #s-310 mjx-merror, #s-311 mjx-merror, #s-312 mjx-merror, #s-313 mjx-merror, #s-314 mjx-merror").length,
        ...
        figureSections: document.querySelectorAll('#figure, [src*="r075n"], [href*="assets/r075n"]').length,
        canonicalCollar: /CANONICAL RADIAL COLLAR/.test(visibleText),
        selectableCutoff: /SELECTABLE CUTOFF/.test(visibleText),
        x1AveragedRow: /X1-AVERAGED ROW/.test(visibleText),
        sumSupOrder: /SUM-SUP ORDER/.test(visibleText),
        zeroMode: /D0 = 0/.test(visibleText),
        noRNegativeLoss: /NO R\^-1 LOSS/.test(visibleText),
        wienerRow: /WIENER ROW O\(L\)/.test(visibleText),
        fullAverage: /FULL AVERAGE O\(L\^2 R\)/.test(visibleText),
        tangencyIncluded: /TANGENCY INCLUDED/.test(visibleText),
        twoIbp: /TWO IBP/.test(visibleText),
        highFrequency: /K >= R\^-3\/2/.test(visibleText),
        coefficientOnly: /COEFFICIENT THEOREM ONLY/.test(visibleText),
        dynamicalOpen: /DYNAMICAL FLUX OPEN/.test(visibleText),
    ...
    assert.equal(audit.step39MathJaxErrors, 0, `${scenario.id} Step 39 MathJax errors: ${JSON.stringify(audit.mathJaxErrorContexts.filter((entry) => /^s-3(08|09|10|11|12|13|14)$/.test(entry.section ?? "")))}`);
    ...
    assert.equal(audit.figureSections, 0, `${scenario.id} no R0.75N formal figure`);
    for (const key of [
      "canonicalCollar", "selectableCutoff", "x1AveragedRow", "sumSupOrder", "zeroMode",
      "noRNegativeLoss", "wienerRow", "fullAverage", "tangencyIncluded", "twoIbp",
      "highFrequency", "coefficientOnly", "dynamicalOpen", "e24Open",
