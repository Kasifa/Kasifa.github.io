...
RELEASE_SOURCE_COMMIT = "3ec784ef6162179909505d1885d609d5214dbe94"
...
<style>:root{{--paper:#f3ecd8;--ink:#26231d;--rule:#8b2f2b;--muted:#625d52}}*{{box-sizing:border-box}}body{{margin:0;background:var(--paper);color:var(--ink);font:17px/1.75 Georgia,"Noto Serif SC",serif}}.top{{border-bottom:3px double var(--ink);padding:12px 5vw;display:flex;justify-content:space-between}}main{{width:min(920px,90vw);margin:auto}}.hero{{padding:64px 0 32px;border-bottom:1px solid var(--ink)}}h1{{font-size:clamp(2rem,6vw,4.3rem);line-height:1.05}}h2{{margin-top:3rem;color:var(--rule)}}.stamp,.section-no{{font:700 12px/1.4 ui-monospace,monospace;letter-spacing:.08em;text-transform:uppercase}}article{{padding:20px 0 80px}}.equation{{overflow:auto;background:#fff8e8;padding:14px;border-left:4px solid var(--rule)}}pre.report-ledger,pre.report-ledger code{{max-width:100%;white-space:pre-wrap;overflow-wrap:anywhere;word-break:break-word}}pre.report-ledger{{overflow-x:auto}}a{{color:#702824}}img{{max-width:100%;height:auto}}@media(max-width:600px){{body{{font-size:15px}}.equation{{contain:inline-size}}.equation mjx-container[display="true"]{{display:block!important;width:100%!important;min-width:0!important;max-width:100%;overflow-x:auto;overflow-y:hidden}}}}</style></head>
