#!/usr/bin/env python3
"""Create final-size, grayscale, and PDF-raster QA surfaces for R0.72L."""
...
def main() -> None:
    ...
    subprocess.run(
        [pdftoppm, "-png", "-f", "1", "-l", "1", "-singlefile", "-r", str(qa_dpi), str(pdf_path), str(ROOT / "qa-pdf")],
        check=True,
    )
    with Image.open(ROOT / "qa-pdf.png") as image:
        pdf_raster = image.convert("RGB")
    ...
    print(f"R0.72L QA surfaces: {size[0]}x{size[1]} px at {qa_dpi} dpi")
