...
// R0.73V translation stage. English copy is accepted only from the local
...
const usage = "usage: add-r073v-translations.mjs (--apply | --check-only | --capture-missing)";
...
const snapshotPath = resolve(root, "scripts/i18n-snapshots/r073v-missing.json");
...
// in this executable. Populate it only after the R0.73V HTML transaction has
// materialized the complete missing-string set.
const activePages = [
  "literature-review.html",
  "notes/index.html",
  "notes/r0-73v.html",
  "recap-r0-61-r0-73v.html",
...
async function safeAtomicWrite(path, payload) {
  ...
  const temporary = resolve(
    dirname(path),
    `.${path.split("/").at(-1)}.r073v-${process.pid}-${Date.now()}-${process.hrtime.bigint()}.tmp`,
  );
  try {
...
async function writeTransaction(writes) {
  if (writes.length !== 3 || new Set(writes.map(({ path }) => path)).size !== 3) {
    throw new Error("R0.73V translation transaction requires three distinct files");
  }
  const nonce = `${process.pid}-${Date.now()}-${process.hrtime.bigint()}`;
  const rows = writes.map(({ path, payload }, index) => ({
    path,
    payload,
    temporary: resolve(dirname(path), `.${path.split("/").at(-1)}.r073v-${nonce}-${index}.tmp`),
    backup: resolve(dirname(path), `.${path.split("/").at(-1)}.r073v-${nonce}-${index}.bak`),
    backedUp: false,
    installed: false,
  }));
  try {
...
const dictionary = await regularText(dictionaryPath, "R0.73V bilingual dictionary");
const report = await regularText(reportPath, "R0.73V report source");
const releaseTitle = metadataField(dictionary, "Release title");
const publicTitle = metadataField(dictionary, "Public title \\(zh\\)");
if (releaseTitle !== "R0.73V | A pressure-aware signed third-order heat lift: exact scale generation and the 3→4 physical-time boundary" ||
    publicTitle !== "R0.73V｜压力感知的有符号三阶热提升：精确尺度生成律与 3→4 物理时间边界") {
  throw new Error("R0.73V bilingual dictionary canonical title ledger is absent or drifted");
}
for (const token of requiredDictionaryTokens) {
  if (!dictionary.includes(token)) throw new Error("R0.73V dictionary missing " + token);
}
...
for (const token of forbiddenClaimTokens) {
  if (dictionary.includes(token) || report.includes(token)) {
    throw new Error("R0.73V canonical source contains forbidden claim " + token);
  }
}
for (const stale of [
  ...
  if (dictionary.includes(stale) || report.includes(stale)) {
    throw new Error("R0.73V canonical source still carries " + stale);
  }
}
...
if (
  ...
  throw new Error("R0.73V HTML/accounting apply must precede translation work");
}
...
for (const relativePage of activePages) {
  ...
  if (!html.includes("/i18n-en.js?v=1.62")) {
    throw new Error(relativePage + ": expected i18n cache version v1.62");
  }
  if (!html.includes("R0.73V")) throw new Error(relativePage + ": R0.73V marker absent");
...
if (JSON.stringify(releaseBatchFiles) !== JSON.stringify(activePages)) {
  throw new Error("unexpected R0.73V release-batch files: " + JSON.stringify(releaseBatchFiles));
}
...
function capturedReasonCodes(entry) {
  ...
  if (releaseId.test(existing.id)) {
    return ["R073V_RELEASE_BATCH", ...forcedCodes, ...issueCodes];
  }
...
if (action === "--capture-missing") {
  ...
  console.log(JSON.stringify({
    ...
    snapshot: "scripts/i18n-snapshots/r073v-missing.json",
...
const snapshot = JSON.parse(await regularText(snapshotPath, "R0.73V translation snapshot"));
if (!Array.isArray(snapshot) || snapshot.length === 0) throw new Error("empty R0.73V snapshot");
if (snapshot.some((entry) => !entry || typeof entry !== "object" || Array.isArray(entry) ||
  ...
  throw new Error("R0.73V accepts only local-direct-reviewed translation rows");
}
const snapshotByChinese = new Map();
for (const entry of snapshot) {
  if (snapshotByChinese.has(entry.zh)) throw new Error("duplicate R0.73V snapshot key");
  if (!sourceByChinese.has(entry.zh)) throw new Error("R0.73V snapshot contains a non-live key: " + entry.zh);
  if (new Set(entry.reasonCodes).size !== entry.reasonCodes.length) {
    throw new Error("R0.73V snapshot contains duplicate reason codes: " + entry.zh);
  }
  if (new Set(entry.reviewedIssues).size !== entry.reviewedIssues.length) {
    throw new Error("R0.73V snapshot contains duplicate reviewed issues: " + entry.zh);
  }
  if (entry.captureBindingSha256 !== captureBindingSha256(entry)) {
    throw new Error("R0.73V snapshot capture binding drift: " + entry.zh);
  }
  snapshotByChinese.set(entry.zh, entry);
}
for (const entry of releaseBatch) {
  if (!snapshotByChinese.has(entry.zh)) {
    throw new Error("R0.73V snapshot is missing a release-batch row: " + entry.zh);
  }
}
for (const entry of currentLegacyRepairs) {
  if (!snapshotByChinese.has(entry.zh)) {
    ...
    throw new Error(
      "R0.73V snapshot is missing a reviewable historical row " + entry.zh + ": " +
      (issues.length
        ? issues.map(({ code, message }) => `${code} (${message})`).join(", ")
        : "HUMAN_SEMANTIC_REVIEW"),
    );
...
for (const entry of snapshot) {
  if (entry.reviewedIssues.some((code) => !issueReasonCodes.has(code))) {
    throw new Error("R0.73V snapshot contains an unknown reviewed issue: " + entry.zh);
  }
  const existing = existingByChinese.get(entry.zh);
  if (entry.sourceIdAtCapture !== null) {
    if (!existing || existing.id !== entry.sourceIdAtCapture) {
      throw new Error("R0.73V snapshot source-id binding drift: " + entry.zh);
    }
    if (entry.capturedEnglishSha256 === null) {
      throw new Error("R0.73V existing-row capture hash is absent: " + entry.zh);
    }
  } else if (existing && !releaseId.test(existing.id)) {
    throw new Error("R0.73V new-row source-id lifecycle drift: " + entry.zh);
  } else if (entry.capturedEnglishSha256 !== null) {
    throw new Error("R0.73V new-row capture hash must be null: " + entry.zh);
  }
  ...
  const expectedReasonCodes = entry.sourceIdAtCapture === null
    ...
    : releaseId.test(entry.sourceIdAtCapture)
      ? ["R073V_RELEASE_BATCH", ...forcedCodes, ...entry.reviewedIssues]
      : [...forcedCodes, ...entry.reviewedIssues];
  if (JSON.stringify(entry.reasonCodes) !== JSON.stringify(expectedReasonCodes)) {
    throw new Error("R0.73V snapshot reasonCodes binding drift: " + entry.zh);
  }
  ...
  if (existing && entry.capturedEnglishSha256 !== null) {
    ...
    if (currentEnglishSha256 === entry.capturedEnglishSha256) {
      if (JSON.stringify(entry.reviewedIssues) !== JSON.stringify(currentIssueCodes)) {
        throw new Error("R0.73V snapshot captured-issue binding drift: " + entry.zh);
      }
    } else if (currentEnglishSha256 !== sha256Text(englishIssues(entry.zh, entry.en).en)) {
      throw new Error(
        "R0.73V existing English is neither captured nor reviewed snapshot English: " + entry.zh,
      );
    }
  }
  ...
  if (!existing || releaseId.test(existing.id) ||
    ...
    throw new Error("R0.73V snapshot has an unproved historical repair row: " + entry.zh);
  }
}
...
if (JSON.stringify(snapshot.map(({ zh }) => zh)) !== JSON.stringify(reviewBatch.map(({ zh }) => zh))) {
  throw new Error("R0.73V review snapshot order or membership drift");
}
...
for (const entry of snapshot) {
  ...
  if (entry.resolution === "semantic-equivalent-approved") {
    ...
    if (review.issues.length) {
      throw new Error(
        "R0.73V review snapshot requires corrected English for " + entry.zh + ": " +
        review.issues.map(({ code, message }) => `${code} (${message})`).join(", "),
      );
    }
    reviewedEnglishByChinese.set(entry.zh, review.en);
...
function allocateReleaseId() {
  while (true) {
    const candidate = "r073v" + String(nextReleaseId).padStart(3, "0");
    nextReleaseId += 1;
...
if (action === "--check-only") {
  if (await regularText(snapshotPath, "R0.73V translation snapshot") !== snapshotOutput) {
    throw new Error("R0.73V translation snapshot is stale");
  }
  if (JSON.stringify(translations) !== JSON.stringify(finalTranslations)) {
    throw new Error("R0.73V translations are stale");
  }
  if (await regularText(bundlePath, "public/i18n-en.js") !== bundleOutput) {
    throw new Error("R0.73V translation bundle is stale");
  }
} else {
