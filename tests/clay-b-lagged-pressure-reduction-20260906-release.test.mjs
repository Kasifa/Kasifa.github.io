...
const protectedArtifacts = {
  "public/notes/clay-b-pressure-test-coupling-20260906.html": "6aa67a32f5d3450030cca3dc60a645df8fbcc7099c702abd1a60aafcb4fe5a4e",
...
test("the HTML-only LaggedPressureReduction note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-lagged-pressure-reduction-20260906.html");
  for (const marker of [
    "ClayB-LaggedPressureReduction-20260906", "CB.12", "旧热背景可以移走", "The old heat background can be removed",
    "没有证明旧压力功自身是 o(H_t)", "does not prove that old pressure work itself is o(H_t)",
    "不是必要性、最优性", "neither necessary nor optimal", "不是 p(R) 的上界", "not an upper bound for p(R)",
    "CONDITIONAL REDUCTION", "SUFFICIENT SCALE ONLY", "OPEN", "NOT CLAY",
  ...
  assert.equal(note.includes("/notes/clay-b-lagged-pressure-reduction-20260906.pdf"), false);
  assert.equal(note.includes("R0.76M"), false);
});

test("homepage keeps the solid R0 sequence separate from the dashed CB.1–CB.12 lane", async () => {
  ...
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-lagged-pressure-reduction-20260906"));
  ...
  const positions = independentChapters.map(([chapter, slug]) => {
    ...
    assert.ok(home.slice(row, row + 1600).includes(chapter), `${chapter} route label`);
    return row;
  });
  assert.deepEqual(positions, [...positions].sort((a, b) => a - b));
  assert.equal((home.match(/class="route-overview independent-release-spotlight"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-lagged-pressure-reduction"/g) ?? []).length, 1);
  assert.equal((literature.match(/id="clay-b-lagged-pressure-reduction-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-lagged-pressure-reduction-20260906"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-lagged-pressure-reduction-20260906.pdf"), false);
  assert.equal(site.version, "2.56");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 12);
  assert.equal(site.latestIndependentNote, "ClayB-LaggedPressureReduction-20260906");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-lagged-pressure-reduction-20260906.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.latestIndependentChapter, "CB.12");
  assert.equal(site.nextIndependentChapter, "CB.13");
  assert.equal(manifest.siteVersion, "2.56");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestPublication.releaseId, "clay-b-lagged-pressure-reduction-20260906");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-PressureTestCoupling-20260906");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.12");
});
...
test("historical milestones, endpoint, prior note, and retained PDFs remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-lagged-pressure-reduction-20260906.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-lagged-pressure-reduction-20260906.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-lagged-pressure-reduction-20260906.pdf")));
});
