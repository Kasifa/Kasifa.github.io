...
const protectedArtifacts = {
  "public/notes/clay-b-fixed-history-screen-20260906.html": "80878b4674b93254c1ba1869800cfb7630a84605228b03f061d5db7cad4eb66e",
...
const independentChapters = [
  ...
  ["CB.17", "clay-b-euler-compactness-screen-20260906"],
];

test("the HTML-only EulerCompactnessScreen note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-euler-compactness-screen-20260906.html");
  for (const marker of [
    "ClayB-EulerCompactnessScreen-20260906", "CB.17", "压力输入可以删去，刚性出口仍未成立", "Pressure input can be removed; the rigidity exit is still unavailable",
    "缩放预检只界定可用窗口", "scaling precheck only identifies admissible windows", "宽 Euler 类非零", "broad Euler class is nonzero",
    "强 L^(11/3) 端点无原子", "Strong L^(11/3) is atom-free", "CONDITIONAL COMPACTNESS", "LITERATURE OBSTRUCTION", "CONDITIONAL ATOM", "KNOWN-METHOD ENDPOINT", "OPEN", "NOT CLAY",
  ...
  assert.equal(note.includes("/notes/clay-b-euler-compactness-screen-20260906.pdf"), false);
  assert.equal(note.includes("R0.76M"), false);
});

test("homepage keeps the solid R0 sequence separate from the dashed CB.1–CB.17 lane", async () => {
  ...
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-euler-compactness-screen-20260906"));
  ...
  const positions = independentChapters.map(([chapter, slug]) => {
    ...
    assert.ok(home.slice(row, row + 2400).includes(chapter), `${chapter} route label`);
    return row;
  });
  assert.deepEqual(positions, [...positions].sort((a, b) => a - b));
  assert.equal((home.match(/class="route-overview independent-release-spotlight"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-euler-compactness-screen"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-fixed-history-screen"/g) ?? []).length, 0);
  assert.equal((literature.match(/id="clay-b-euler-compactness-screen-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-euler-compactness-screen-20260906"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-euler-compactness-screen-20260906.pdf"), false);
  assert.equal(site.version, "2.61");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 17);
  assert.equal(site.latestIndependentNote, "ClayB-EulerCompactnessScreen-20260906");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-euler-compactness-screen-20260906.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.latestIndependentChapter, "CB.17");
  assert.equal(site.nextIndependentChapter, "CB.18");
  assert.equal(manifest.siteVersion, "2.61");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestPublication.releaseId, "clay-b-euler-compactness-screen-20260906");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-FixedHistoryScreen-20260906");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.17");
});
...
test("historical milestones, endpoint, prior note, and retained PDFs remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-euler-compactness-screen-20260906.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-euler-compactness-screen-20260906.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-euler-compactness-screen-20260906.pdf")));
});
