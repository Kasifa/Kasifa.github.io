...
test("publishes R0.71Y as v1.11 with synchronized current counts, route, and recap", async () => {
  ...
  assert.match(literature, /<header><b>R0\.71Y<\/b>/);
  ...
  for (const [page, minimum] of [
    ...
    assert.ok(page.includes('src="/i18n-en.js?v=1.11"'));
    assert.doesNotMatch(
      page,
...
test("states the selected-root theorem, R0.71X correction, and open boundaries precisely", async () => {
  ...
  assert.match(home, /route-closing theorem|路线关闭定理/is);
  assert.match(recap, /不是 complete all-root no-go/is);
  assert.match(literature, /没有得到 all-root count/is);
