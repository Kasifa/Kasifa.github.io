...
const outputPath = resolve(root, "research/r073k_pdf_bindings.json");
const rendererPath = resolve(root, "scripts/render-note-pdf.mjs");
const documents = [
  {
    kind: "research-note",
    html: "public/notes/r0-73k.html",
    pdf: "public/notes/r0-73k.pdf",
    title: "R0.73K｜Parameter-uniform viscous rank-one branch",
  },
  {
    kind: "cumulative-recap",
    html: "public/recap-r0-61-r0-73k.html",
    pdf: "public/recap-r0-61-r0-73k.pdf",
    title: "R0.61–R0.73K｜R0.60 之后的研究回顾",
  },
];

const usage = "usage: bind-r073k-pdfs.mjs (--apply | --check-only)";
...
async function atomicWrite(path, payload) {
  const temporary = resolve(
    dirname(path),
    `.${path.split("/").at(-1)}.r073k-${process.pid}-${Date.now()}-${process.hrtime.bigint()}.tmp`,
  );
  try {
...
const manifest = {
  schemaVersion: "r073k-synchronized-pdf-bindings-v1",
  release: "R0.73K",
  renderer: {
...
if (apply) {
  ...
  const current = await regularBytes("research/r073k_pdf_bindings.json");
  if (!current.equals(payload)) throw new Error("R0.73K PDF binding manifest is stale");
}
...
console.log(JSON.stringify({
  ...
  output: "research/r073k_pdf_bindings.json",
}));
