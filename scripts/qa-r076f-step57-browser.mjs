...
const outputRoot = resolve(process.env.R076F_STEP57_BROWSER_QA_OUTPUT ?? "/tmp/r076f-step57-browser-qa");
const externalBase = process.env.R076F_PUBLIC_BASE_URL;
...
const target = {
  id: "note",
  path: "/notes/r0-76f.html",
  h1Zh: /继承空间观测的指数下界/,
  h1En: /Exponential lower bound for the inherited spatial observation/i,
};
const results = [];
...
try {
  for (const scenario of scenarios) {
    ...
    url.searchParams.set("qa", "r076f-step57");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        step57MathJaxErrors: document.querySelectorAll("#s-450 mjx-merror, #s-451 mjx-merror, #s-452 mjx-merror, #s-453 mjx-merror, #s-454 mjx-merror, #s-455 mjx-merror").length,
        ...
        figureSections: document.querySelectorAll('#figure, [src*="assets/r076f"], [href*="assets/r076f"]').length,
        exactShear: /EXACT REAL DYADIC SHEAR|exact real dyadic|exact smooth shear|精确剪切/i.test(visibleText),
        exponentialLowerBound: /2\^\(Q-1\) LOWER BOUND|2\^\(q-1\)|exponential lower bound|指数下界/i.test(visibleText),
        exponentialOrder: /EXP\(THETA\(Q\)\) SHARPNESS|exp\(Theta\(q\)\)|exponential order|指数阶/i.test(visibleText),
        binomialConstruction: /BINOMIAL CONSTRUCTION|binomial amplitudes|binomial coefficients/i.test(visibleText),
        dyadicBand: /POSITIVE FREQUENCIES|positive-frequency dyadic|dyadic band/i.test(visibleText),
        fixedGeometry: /FIXED I AND J|I=\[-1\/2,1\/2\]|fixed I\/J/i.test(visibleText),
        bZero: /B = 0|B=0/i.test(visibleText),
        spatialOnly: /SPATIAL ROW ONLY|spatial observation row|空间观测行/i.test(visibleText),
        noCompleteFluxLower: /NO FULL-FLUX LOWER BOUND|no complete collar-flux lower bound|不是完整 collar-flux/i.test(visibleText),
    ...
    assert.equal(audit.step57MathJaxErrors, 0, `${id} Step 57 MathJax errors`);
    ...
    assert.equal(audit.figureSections, 0, `${id} no R0.76F formal figure`);
    for (const key of ["exactShear", "exponentialLowerBound", "exponentialOrder", "binomialConstruction", "dyadicBand", "fixedGeometry", "bZero", "spatialOnly", "noCompleteFluxLower", "finiteBoundary", "arbitraryPacketsOpen", "noveltyBoundary", "clayBoundary"])
