...
<title>{TITLE}</title><meta name="description" content="一侧 ball-clock 归约、终端 Abel 恒等式与剩余的 ℓ¹ 债务">
...
<body><nav class="top"><a href="/research-review.html">研究首页</a><span>R0.74S · 2026-09-02</span></nav><main><header class="hero"><div class="hero-inner"><div><div class="section-no">研究笔记 R0.74S · 完整中文版本</div><h1>{TITLE}</h1><p>一侧 ball completion 给出精确 stopped clocks 与 Abel 恒等式，但只留下 \\(\\ell^1\\) 账本。抽象时钟塔证明单靠正性、线性与 tower identity 不能得到匹配 \\(\\ell^2\\) 压缩。<strong>PROVED ABSTRACT NO-GO ONLY. NOT PDE/NSE. NOT CLAY.</strong></p><div class="labels"><span class="label">PROVED</span><span class="label">INHERITED</span><span class="label">FINITE</span><span class="label">ABSTRACT NO-GO</span><span class="label">OPEN</span><span class="label">NOT PDE/NSE</span><span class="label">NOT CLAY</span></div></div><div class="stamp"><strong>状态 · R0.74S</strong><p>一侧 ball cutoff：PROVED</p><p>三条 stopped orientation：PROVED</p><p>terminal Abel identity：PROVED</p><p>ℓ¹/ℓ² obstruction：PROVED ABSTRACT NO-GO</p><p>PDE/NSE counterexample：NOT CLAIMED</p><p>cross-channel sign：OPEN</p><p>Q.1 / regularity：OPEN</p><p>无仿真 / NO DGX</p></div></div></header><article>
{report_body()}
<section id="figure"><div class="section-no">F / 期刊主图</div><h2>一侧球时钟、精确 Abel 恒等式与剩余债务</h2><picture><source srcset="/assets/r074s/{FIGURE_ID}.svg" type="image/svg+xml"><img src="/assets/r074s/{FIGURE_ID}.png" alt="R0.74S one-sided ball completion and l1 debt"></picture><p><a href="/assets/r074s/{FIGURE_ID}.pdf">矢量 PDF</a> · <a href="/assets/r074s/{FIGURE_ID}.png">600 dpi PNG</a> · <a href="/assets/r074s/{FIGURE_ID}.svg">SVG</a> · <a href="/figures/r074s/{FIGURE_ID}/source-data.csv">source data</a> · <a href="/figures/r074s/{FIGURE_ID}/qa-report.md">视觉 QA</a></p><p class="note">确定性解析图，不是 DNS、仿真、PDE/NSE 反例、奇点或正则性证据。</p></section>
<section id="reproduce"><div class="section-no">R / 冻结证据</div><h2>解析主文、证书与独立审计</h2><p class="files"><a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_problem_freeze.md">问题冻结</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_one_sided_ball_clock_no_gain.md">最终解析主文</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_one_sided_ball_clock_primary_audit.md">主审计</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_one_sided_ball_clock_independent_audit.md">独立审计</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_claim_state_update.md">主张边界</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_literature_boundary.md">文献与优先权边界</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_report-source.md">完整中文 reader source</a></p><p><a href="/notes/r0-74s.pdf">同步研究笔记 PDF</a> · <a href="/recap-r0-61-r0-74o.html">上一大里程碑 recap（截止 R0.74O，157 节）</a> · <a href="/recap-r0-61-r0-74o.pdf">PDF</a></p><p class="note">5/5 exact、7/7 finite、55/55 structural、4/4 negative mutation；有限证书不替代解析证明。</p></section>
<section id="next"><div class="section-no">NEXT / 下一门槛</div><h2>R0.74T</h2><p>不再增加同类 positive completion；下一步只检验跨通道动力学符号关系，或证明 stopped block genealogy 的统一有限复杂度。</p></section></article></main></body></html>'''
...
def update_home() -> None:
    ...
    pairs = (
        ...
        ('<h3>R0.74R：终端窗口收缩与任意时钟提取门</h3>', '<h3>R0.74S：一侧 ball-clock 与 ℓ¹/ℓ² no-go</h3>', "current route title"),
        ('<p class="tree-current-summary">同一精确 2D3C 解的自由被动振幅排除冻结标量支付的普适平方根对数端点；增广可观测量与任意流端点仍开放。</p>', '<p class="tree-current-summary">一侧球完成与终端 Abel 恒等式只给出 ℓ¹；抽象时钟塔排除仅靠正性、线性与 tower identity 获得 ℓ² 压缩。不是 PDE/NSE 反例，NOT CLAY。</p>', "current route summary"),
        ('<p class="tree-path">第五支付壳 → 匹配完整支付 → 单一坏内领圈 → 真实包桥边界</p>', '<p class="tree-path">窗口质量收缩 → 任意时钟三分法 → 一侧 ball completion → terminal Abel identity → 抽象 ℓ¹/ℓ² no-go</p>', "current route path"),
    ...
    old_next = '<div class="tree-row"><article class="tree-node next"><div class="tree-node-head"><span class="route-range">NEXT · R0.74S</span><span class="tree-state current">下一检查点</span></div><h3>R0.74S 下一接口</h3><p>检验一侧 ball completion 后剩余的 ℓ¹ debt；若路线失败，只记录 abstract no-go，不外推成 PDE/NSE 反例。</p></article></div>'
    new_next = '<div class="tree-row"><article class="tree-node next"><div class="tree-node-head"><span class="route-range">NEXT · R0.74T</span><span class="tree-state current">下一检查点</span></div><h3>R0.74T 下一接口</h3><p>检验跨通道动力学符号关系，或证明 stopped block genealogy 的统一有限复杂度；不再叠加同类正时钟。</p></article></div>'
    page = replace_once_or_present(page, old_next, new_next, "next route")
    page = replace_once_or_present(page, 'terminal-window convex packing / arbitrary-clock triage / open extraction gate</p>', 'terminal-window convex packing / arbitrary-clock triage / open extraction gate → one-sided ball-clock Abel identity / abstract ℓ¹-to-ℓ² no-go</p>', "path tail")
    card = '''          <div class="task-one" id="r074s" data-release="r074s" style="margin-top:2rem"><p class="eyebrow">研究笔记 R0.74S · 2026-09-02</p><h3>R0.74S｜一侧球时钟与 ℓ¹ 债务</h3><p>精确 Abel 恒等式只给出 ℓ¹；抽象时钟塔排除单靠正性与线性获得 ℓ² 压缩。不是 PDE/NSE 反例，NOT CLAY。</p><p><a href="/notes/r0-74s.html"><strong>阅读完整中文笔记 →</strong></a> · <a href="/notes/r0-74s.pdf">PDF</a> · <a href="/assets/r074s/fig-r074s-ball-clock-debt.pdf">附图</a></p></div>\n'''
...
def update_literature() -> None:
    ...
    for old, new, label in (
        ...
        page = replace_once_or_present(page, old, new, label)
    old = '<div class="route-step pause"><header><b>开放接口 · R0.74S</b><strong>一侧 ball completion 与 ℓ¹ debt</strong></header><p>下一步只检验抽象正性、线性与 tower 路线；任何 no-go 都不自动成为 PDE/NSE 反例。</p></div>'
    new = '<div class="route-step kept"><header><b>R0.74S</b><strong>一侧 ball-clock Abel 恒等式与 ℓ¹/ℓ² no-go</strong></header><p>三条 stopped channel、二次 ball ledger 与 terminal Abel identity 已证明；抽象时钟塔排除 scalar positivity、linearity、tower identities 单独推出匹配 ℓ² 压缩。不是 PDE/NSE 反例。<a href="/notes/r0-74s.html">研究笔记</a> <a href="#r074s-boundary">主张边界</a></p></div><div class="route-step pause"><header><b>开放接口 · R0.74T</b><strong>跨通道动力学符号或有限 genealogy</strong></header><p>下一步不再增加同类 positive completion，而要保留 root/outer/weight-drop 之间的动力学关系。</p></div>'
    page = replace_once_or_present(page, old, new, "literature route")
    boundary = '<h3 id="r074s-boundary">R0.74S 的文献与主张边界</h3><p>本节是基于既有 suitable-weak local-energy 与 shell-clock 框架的内部代数路线审计；不声称从抽象 tower 得到新的 Navier--Stokes 定理，也不声称新颖性或优先权。</p><div class="boundary"><strong>R0.74S 的公开边界</strong><p>PROVED：one-sided ball clocks、stopped orientations、quadratic ledger、terminal Abel identity。FINITE：5/5、7/7、55/55、4/4。NO-GO：仅排除 scalar positivity、cutoff linearity 与 tower identities 的独立机制；见证不是速度、压力、耗散测度或 NSE 解。OPEN：跨通道动力学符号、finite genealogy、Q.1、正则性与奇点。<strong>NOT PDE/NSE. NOT CLAY.</strong> <a href="/notes/r0-74s.html">阅读完整中文笔记</a>。</p></div>\n'
...
def update_notes_index() -> None:
    ...
    for old, new, label in (
        ...
        page = replace_once_or_present(page, old, new, label)
    entry = '''          <li class="note-entry" data-note="r0-74s"><article><div class="entry-copy"><p class="note-code">R0.74S</p><h3>一侧球时钟为何仍留下 ℓ¹ 债务</h3></div><nav class="entry-files" aria-label="R0.74S files"><a class="file-link html" href="/notes/r0-74s.html" aria-label="Read R0.74S HTML">HTML</a><a class="file-link pdf" href="/notes/r0-74s.pdf" aria-label="Download R0.74S PDF">PDF</a></nav></article></li>\n'''
