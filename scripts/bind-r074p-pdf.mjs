...
// Render and cryptographically bind the complete Chinese R0.74P reader PDF.
...
try {
  const htmlRelative = "public/notes/r0-74p.html";
  const pdfRelative = "public/notes/r0-74p.pdf";
  const provenanceRelative = "research/r074p_note_pdf_render.json";
  const bindingRelative = "research/r074p_pdf_bindings.json";
  const url = `http://127.0.0.1:${address.port}/notes/r0-74p.html?lang=zh`;
  ...
  const title = "R0.74P｜哪些时间可观测量真正看见了缺失的尺度？";
  ...
  const binding = {
    schemaVersion: "r074p-note-synchronized-pdf-binding-v1",
    release: "R0.74P",
    kind: "note",
    ...
    claimBoundary: {
      ...
      positiveOrderWindowNoGo: "PROVED",
      defectCompletedClock: "PROVED",
      targetShellTwoSidedScale: "PROVED",
      completeMatchedSquareFunctionUpperBound: "OPEN",
      pdeL1ToMatchedL2Compression: "OPEN",
      prescribedCentreScalePacking: "OPEN",
  ...
  process.stdout.write(`${JSON.stringify({ applied: true, release: "R0.74P", pageCount: structure.pageCount, sha256: sha256(pdf), bytes: pdf.length }, null, 2)}\n`);
} finally {
