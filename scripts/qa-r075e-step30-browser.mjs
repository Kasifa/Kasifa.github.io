...
const outputRoot = resolve(process.env.R075E_STEP30_BROWSER_QA_OUTPUT ?? "/tmp/r075e-step30-browser-qa");
const externalBase = process.env.R075E_PUBLIC_BASE_URL;
...
try {
  for (const scenario of scenarios) {
    ...
    const url = new URL("/notes/r0-75e.html", baseUrl);
    url.searchParams.set("lang", scenario.lang);
    url.searchParams.set("qa", "r075e-step30");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        figureSections: document.querySelectorAll('#figure, [src*="r075e"], [href*="assets/r075e"]').length,
        differenceFrequencyProved: /DIFFERENCE-FREQUENCY IDENTITY PROVED/.test(visibleText),
        diagonalFluxZero: /DIAGONAL FLUX ZERO/.test(visibleText),
        realZeroModePaid: /REAL ZERO MODE ALL-PAYMENT PAID/.test(visibleText),
        complexSingletonDiagnostic: /COMPLEX SINGLETON DIAGNOSTIC ONLY/.test(visibleText),
        realPairMayCouple: /REAL [±+]N PAIR MAY COUPLE/.test(visibleText),
        generalCrossModeOpen: /GENERAL CROSS-MODE GATE OPEN/.test(visibleText),
    ...
    assert.equal(audit.figureSections, 0, `${scenario.id} no R0.75E formal figure`);
    assert.equal(audit.differenceFrequencyProved, true, `${scenario.id} difference-frequency boundary`);
    assert.equal(audit.diagonalFluxZero, true, `${scenario.id} diagonal boundary`);
    assert.equal(audit.realZeroModePaid, true, `${scenario.id} real zero-mode boundary`);
    assert.equal(audit.complexSingletonDiagnostic, true, `${scenario.id} singleton boundary`);
    assert.equal(audit.realPairMayCouple, true, `${scenario.id} real-pair boundary`);
    assert.equal(audit.generalCrossModeOpen, true, `${scenario.id} general cross-mode boundary`);
