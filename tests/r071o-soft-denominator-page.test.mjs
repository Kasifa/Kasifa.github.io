...
test("publishes R0.71O as v1.00 with 139 notes, route 49, recap 79, and R0.71P next", async () => {
  const [{ home, note, recap, literature }, noteNames] = await Promise.all([
    publishedPages(),
    readdir(notesRoot),
  ]);

  assert.equal(noteNames.filter((name) => name.endsWith(".html")).length, 139);
  assert.match(home, /<strong>v1\.00<\/strong>网页版本/);
  assert.match(home, /<strong>139<\/strong>公开研究笔记/);
  assert.match(home, /<strong>R0\.71O<\/strong>最新研究节点/);
  assert.match(home, /<span class="route-range">R0\.69P–R0\.71O<\/span>/);
  assert.match(home, /展开 49 篇公开笔记/);
  assert.match(home, /累计回顾收录 79 个节点；全站现有 139 篇公开研究笔记/);
  assert.match(home, /NEXT · R0\.71P/);

  const currentRoute = home.match(
    /<nav class="route-note-links" aria-label="R0\.69P–R0\.71O">([\s\S]*?)<\/nav>/,
  );
  assert.ok(currentRoute);
  assert.equal(occurrenceCount(currentRoute[1], 'href="/notes/'), 49);
  ...
  assert.match(literature, /R0\.69P–R0\.71O/);
  assert.match(literature, /开放接口 · R0\.71P/);
  ...
  for (const [page, minimum] of [
    ...
    assert.match(page, /src="\/i18n-en\.js\?v=1\.00"/);
  }
});
