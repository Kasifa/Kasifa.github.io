...
test("retains R0.71V artifacts while v1.15 publishes R0.72B as current", async () => {
  ...
  assert.match(
    home,
    /<details class="tree-notes" open>[\s\S]*?aria-label="R0\.69P–R0\.72B"/,
  );
  assert.match(home, /href="#r070a">R0\.70A–R0\.72B 完成版本<\/a>/);
  ...
  assert.ok(route);
  assert.equal(count(route[1], 'href="/notes/'), 62);
