...
const outputRoot = resolve(process.env.R075W_STEP48_BROWSER_QA_OUTPUT ?? "/tmp/r075w-step48-browser-qa");
const externalBase = process.env.R075W_PUBLIC_BASE_URL;
...
const targets = [
  { id: "note", path: "/notes/r0-75w.html", h1Zh: /单个 dyadic 二谐波剪切的全载频 signed-flux 付款/, h1En: /Full-frequency signed-flux payment for one dyadic two-harmonic shear/i },
  { id: "recap", path: "/recap-r0-61-r0-75w.html", h1Zh: /exact pair 的全载频 signed flux/, h1En: /full-frequency signed flux for an exact pair/i },
];
const results = [];
...
try {
  for (const scenario of scenarios) {
    for (const target of targets) {
      ...
      url.searchParams.set("qa", "r075w-step48");
      ...
      const audit = await page.evaluate(() => {
        ...
        return {
          ...
          step48MathJaxErrors: document.querySelectorAll("#s-374 mjx-merror, #s-375 mjx-merror, #s-376 mjx-merror, #s-377 mjx-merror, #s-378 mjx-merror, #s-379 mjx-merror, #s-380 mjx-merror, #s-381 mjx-merror, #s-382 mjx-merror, #s-383 mjx-merror").length,
          ...
          figureSections: document.querySelectorAll('#figure, [src*="assets/r075w"], [href*="assets/r075w"]').length,
          exactPair: /EXACTLY TWO HARMONICS|exact diffusive dyadic two-harmonic/i.test(visibleText),
          lowCarrier: /LOW CARRIER PAID|low-carrier local energy|低载频/i.test(visibleText),
          fullFrequency: /FULL FREQUENCY|full-frequency|全载频/i.test(visibleText),
          tuvw: /T[^\n]*high-carrier spatial coercivity[\s\S]*U[^\n]*difference[\s\S]*V[^\n]*high-carrier self\/sum[\s\S]*W[^\n]*low-carrier/i.test(visibleText),
      ...
      if (target.id === "note") assert.equal(audit.step48MathJaxErrors, 0, `${id} Step 48 MathJax errors`);
      ...
      assert.equal(audit.figureSections, 0, `${id} no R0.75W formal figure`);
      for (const key of target.id === "note"
        ? ["exactPair", "lowCarrier", "fullFrequency", "noveltyBoundary", "clayBoundary"]
        : ["exactPair", "lowCarrier", "fullFrequency", "tuvw", "noveltyBoundary", "clayBoundary"]
