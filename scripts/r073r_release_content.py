#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Canonical-source reader and UI fragments for the fail-closed R0.73R release.
...
FIGURE_SOURCE_RELATIVE = f"research/figures/r073r/{FIGURE_ID}"
FIGURE_ARCHIVE_RELATIVE = f"figures/r073r/{FIGURE_ID}"
...
REPORT_SOURCE = "research/r073r_report-source.md"
DICTIONARY_SOURCE = "research/r073r_bilingual_dictionary.md"
...
ALLOWED_PUBLICATION_STATES = {
    "formulaCertificateValidation": frozenset(("PENDING", "PASS")),
    "formulaCertificatePackage": frozenset(("PENDING", "PRESEAL_PENDING", "PRESEAL_PASS", "CLOSED")),
...
class ReleaseContent:
    ...
    def math_next_gate_zh(self) -> str:
        """Keep the canonical mathematical R0.73S gate."""
        if self.next_release not in self.next_gate_zh:
            raise CanonicalSourceError("R0.73R mathematical next-gate marker drift")
    ...
    def note_hero(self) -> str:
        return (
            ...
            '      <div><div class="eyebrow">研究笔记 R0.73R · '
            'SHELLWISE PHASE CERTIFICATE / CLASSICAL BESOV BOUNDARY</div>\n'
            f'        <h1>{html.escape(self.public_title_zh)}</h1>\n'
            f'        <p class="lead">{html.escape(self.lead_zh)}</p></div>\n'
            '      <div class="stamp"><span class="state">状态 · R0.73R 完成</span>'
            '<strong>Shellwise phase certificate + matched Fourier pair</strong>'
            f'<p>版本 R0.73R · {html.escape(self.date)}</p>'
            '<p>heat--Besov equivalence：VERIFIED CLASSICAL</p>'
            '<p>matched phase separation：CLOSED AFTER AUDIT</p>'
            '<p>zero nonlinearity：CLOSED</p>'
    ...
    def note_article(self) -> str:
        ...
        boundary = (
            ...
            '<h2>经典机制、本地精确综合与开放问题严格分列</h2>'
            f'<p>{html.escape(self.closed_ledger)}</p>'
            f'<p>{html.escape(self.finite_ledger)}</p>'
            f'<p>{html.escape(self.open_ledger)}</p>'
            '<p>热半群与 Besov 等价属于经典结果；本地证明和有限公式包只承担逐壳接口、'
            '匹配 Fourier 对与六次矩复算。两族非线性项均为零。'
            '这些结果不证明任意 L2-small 数据安全，也不改变 Clay 状态。'
            'NOT CLAY。</p></section>'
        )
        figure = (
            '        <section id="figure"><div class="section-no">F / Journal figure</div>'
            '<h2>同支撑同幅值下的相位相干分离</h2>'
            f'<p><img src="/assets/r073r/{FIGURE_ID}.svg" '
            'alt="R0.73R matched Dirichlet and Rudin--Shapiro phase-coherence diagnostic"></p>'
            f'<p><a href="/assets/r073r/{FIGURE_ID}.pdf">下载矢量 PDF</a> · '
            f'<a href="/assets/r073r/{FIGURE_ID}.png">下载 600 dpi PNG</a> · '
            f'<a href="/assets/r073r/{FIGURE_ID}.svg">打开 SVG</a></p>'
            '<p>附图只呈现精确 Fourier 支撑与解析标度。它不是 Navier--Stokes 仿真；'
            '两族的对流非线性都严格为零，也不认证任意数据全局正则。</p></section>'
        )
        reproduction = (
            ...
            'research/r073r_lp_caloric_certificate_proof.md">LP--caloric certificate proof</a> · '
            '<a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/'
            'research/r073r_primary_literature_audit.md">primary literature audit</a> · '
            '<a href="https://github.com/Kasifa/Kasifa.github.io/tree/main/'
            'research/certificates/r073r">finite diagnostic package</a></p>'
            f'<p><a href="/assets/r073r/{FIGURE_ID}.pdf">journal figure PDF</a> · '
            '<a href="/notes/r0-73r.pdf">synchronized note PDF</a> · '
            '<a href="/recap-r0-61-r0-73r.html">134-node cumulative recap</a> · '
            '<a href="/recap-r0-61-r0-73r.pdf">synchronized recap PDF</a></p>'
            '</section>'
        )
        return (
            "      <article>\n" + body + "\n" + figure + "\n" + boundary
    ...
    def home_card(self) -> str:
        return (
            '          <div class="task-one" id="r073r" data-release="r073r" '
            'style="margin-top:2rem">\n'
            f'            <p class="eyebrow">研究笔记 R0.73R · {html.escape(self.date)}</p>'
            ...
            'NOT CLAY。</p>\n'
            '            <p><a href="/notes/r0-73r.html"><strong>阅读 R0.73R '
            '研究笔记 →</strong></a><br><a href="/notes/r0-73r.pdf">下载同步 PDF</a> · '
            f'<a href="/assets/r073r/{FIGURE_ID}.pdf">下载期刊附图 PDF</a> · '
            '<a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/'
            'research/r073r_lp_caloric_certificate_proof.md">查看连续证明</a> · '
            '<a href="/recap-r0-61-r0-73r.html">打开累计回顾</a></p>\n'
    ...
    def latest_spotlight(self) -> str:
        return (
            ...
            f'        <div><p class="eyebrow">LATEST RELEASE · R0.73R · '
            ...
            '<a class="route-map-latest" href="/notes/r0-73r.pdf">阅读最新 R0.73R '
            '研究笔记 →</a><a href="/recap-r0-61-r0-73r.html">134 节累计回顾</a>'
            '<a href="/notes/">194 篇研究笔记总索引</a>'
            '<a href="#r073r">查看首页完整 R0.73R 卡片</a></nav>\n'
            '      </header><div class="route-legend" aria-label="最新发布计数">'
            '<span><i class="route-legend-mark kept" aria-hidden="true"></i>'
            'R0.70A–R0.73R · 96 节已公开</span>'
            '<span><i class="route-legend-mark kept" aria-hidden="true"></i>'
            '72 节完整封存</span><span><i class="route-legend-mark current" '
            'aria-hidden="true"></i>当前端点 R0.73R</span></div></div>\n'
            '    </section>'
        )
    ...
    def recap_phase(self) -> str:
        return (
            f'            <article class="phase"><h3>R0.73R · '
            ...
            '<div class="links"><a href="/notes/r0-73r.html">R0.73R</a>'
            f'<a href="/assets/r073r/{FIGURE_ID}.pdf">R0.73R 附图</a>'
            '<a href="https://github.com/Kasifa/Kasifa.github.io/tree/main/'
            'research/certificates/r073r">R0.73R 有限诊断包</a></div></article>'
        )

    @property
    def literature_update(self) -> str:
        return (
            '<span class="route-r073r-deck-update">'
            + html.escape(self.literature_zh)
            + ' periodic heat--Besov equivalence=VERIFIED_CLASSICAL；'
            'uniform L2-only strong radius=OPEN；不承担新颖性或优先权声明。</span>'
        )
...
def _parse_sections(report: str) -> tuple[ReportSection, ...]:
    ...
    if not matches:
        raise CanonicalSourceError("R0.73R report source has no level-two sections")
...
def _build_public_report(*, forced_ready: bool) -> str:
    readiness = (
        "逐壳解析证明已经通过复核；有限公式诊断和正式附图仍按独立发布门管理。"
        if forced_ready else
        "逐壳解析证明仍在复核；发布生成器保持关闭。"
    )
    return r"""# R0.73R | A shellwise phase certificate for the critical heat trace

## 1. 直接结论

周期临界热流迹等价于逐壳 \(\ell^4\) Besov 预算。该等价是经典结果；
本节只把它写成可审计的 Fourier 接口。

## 2. 三层证书

精确向量三重卷积、三重和加法重数和支撑计数给出从精到粗的三个层级。

## 3. 匹配 Fourier 对

Dirichlet 与 Rudin--Shapiro 场共享支撑、逐模幅值和全部二次 Sobolev
数据，但临界热流迹相差 \(m^{2/3}\)。两族的非线性项都严格为零。
...
`matchedPhaseHeatTraceSeparation=CLOSED_AFTER_AUDIT`；
`zeroNonlinearityBoundary=CLOSED`；
`uniformL2OnlyStrongRadius=OPEN`；`clayConclusion=OPEN`。NOT CLAY。
...
def load_release_content(root: Path | None = None) -> ReleaseContent:
    ...
    if (
        "noveltyOrPriorityClaim=FORBIDDEN" not in combined
        or "VERIFIED_CLASSICAL" not in combined
    ):
        raise CanonicalSourceError("canonical sources lost classical/non-priority boundary")
    ...
    next_release = (
        ...
        if next_release_match else "R0.73S"
    )
    ...
    fallback_lead = (
        "周期临界热流迹等价于逐壳四次可和的能量与六次集中度预算；函数空间等价属于经典结果。"
    )
    fallback_home = (
        "R0.73R 给出三层 Fourier 证书，并用同支撑同幅值的匹配对显示二次能谱不能决定临界热流迹。"
    )
    fallback_recap = (
        "本节把经典热--Besov 机制改写为可核查接口，并严格保留零非线性与 NOT CLAY 边界。"
    )
    fallback_literature = (
        "Chemin--Gallagher 的周期热--Besov 等价与 Rudin--Shapiro 平坦性构成直接经典碰撞。"
    )
    fallback_next = (
        "R0.73S 将检查复杂度低于精确三重卷积的确定性相位代理量，失败则记录 no-go。"
    )
    ...
    finite = (
        "matchedSupportMagnitudeQuadraticData=CLOSED_EXACT；"
        "finiteFormulaDiagnosticOnly=TRUE；heatFlowIntegralComputed=FALSE；"
        "navierStokesSimulation=NOT_RUN；translationPath=LOCAL_DIRECT_NO_DGX"
    )
    opened = (
        "failureOfEntranceImpliesUnsafeDynamics=FALSE；"
        "uniformL2OnlyStrongRadius=OPEN；"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Read and validate canonical R0.73R release content without writing."
    )
