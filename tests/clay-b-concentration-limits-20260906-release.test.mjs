...
test("the homepage separates the solid R0 sequence from a dashed CB.1–CB.6 lane", async () => {
  ...
  const positions = independentChapters.map(([chapter, slug]) => {
    ...
    assert.ok(home.slice(row, row + 900).includes(chapter), `${chapter} route label`);
    return row;
  });
  assert.deepEqual(positions, [...positions].sort((a, b) => a - b));
  const concentrationRow = positions.at(-1);
  const route = home.slice(concentrationRow, clayBoundary);
  ...
  assert.equal(site.version, "2.50");
  ...
  assert.equal(site.latestIndependentChapter, "CB.6");
  assert.equal(site.nextIndependentChapter, "CB.7");
  assert.equal(manifest.siteVersion, "2.50");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.6");
});

test("historical milestone, canonical endpoint, and retained PDF assets remain byte-exact", async () => {
