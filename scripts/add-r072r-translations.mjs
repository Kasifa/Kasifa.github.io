...
row("在 complex 1:2:3 coefficient space 中，显式 polydisc \\(K=\\{|z_2-3/20|\\le1/100,\\ |z_3|\\le1/1000\\}\\) 的全部初始 profile 都满足 \\(Q_2(0)\\ge14/25>1/2\\)，却沿整个声明热路径始终恰有两个临界点。实际 shear 在 \\(0\\le y\\le1\\) 具有统一 \\((r,\\mathfrak C_0,\\mathfrak C_1)=(\\pi/48,144,240)\\)；同一 cell window 上得到系数一致的 full-superposition enhanced-dissipation corollary。", "In the complex 1:2:3 coefficient space, every initial profile in the explicit polydisc \\(K=\\{|z_2-3/20|\\le1/100,\\ |z_3|\\le1/1000\\}\\) satisfies \\(Q_2(0)\\ge14/25>1/2\\), yet retains exactly two critical points along the entire stated heat path. On \\(0\\le y\\le1\\), the physical shear has the uniform contract \\((r,\\mathfrak C_0,\\mathfrak C_1)=(\\pi/48,144,240)\\); the coefficient-uniform full-superposition enhanced-dissipation corollary holds on the same cell window.");
...
row("在本站这条研究路线中，R0.72R 给出一个整体处于旧 jet-safety cone 外、却具有统一 root localization、shape margins、heat-path ledger 与完整传播推论的四实维系数集。它可作为特殊 triangular mechanism 论文中的定量 lemma，也为逼近真实 caustic 提供有证书的起点。", "On this site's research route, R0.72R gives a four-real-dimensional coefficient set lying entirely outside the old jet-safety cone while retaining uniform root localization, shape margins, a heat-path ledger, and the complete propagation corollary. It can serve as a quantitative lemma in a paper on the special triangular mechanism and as a certified starting point for approaching the true caustic.");
...
row("新的严格增量是一个非空四实维紧致系数核：它整体位于 R0.72Q 加权锥外，具有 \\(y\\ge0\\) 的全热路径 root localization，并在 \\(0\\le y\\le1\\) 具有物理 shape contract 和 family-uniform ED。", "The new rigorous increment is a nonempty four-real-dimensional compact coefficient core lying entirely outside the R0.72Q weighted cone, with full-heat-path root localization for \\(y\\ge0\\), and a physical shape contract plus family-uniform enhanced dissipation on \\(0\\le y\\le1\\).");
...
row("累计回顾 R0.61–R0.72R · 2026-08-28", "Cumulative recap R0.61–R0.72R · 2026-08-28");
row("累计回顾保持二十八个问题阶段，并给出 R0.61–R0.72R 的完整逐节点索引。R0.72R 给出旧加权锥外的四实维 caustic-free core、对所有 \\(y\\ge0\\) 的全热路径 root localization，以及 \\(0\\le y\\le1\\) 上的物理 shape contract 与 coefficient-uniform full-superposition ED。", "The cumulative recap retains twenty-eight problem phases and gives a complete node-by-node index through R0.72R. R0.72R gives a four-real-dimensional caustic-free core beyond the old weighted cone, full-heat-path root localization for every \\(y\\ge0\\), and a physical shape contract plus coefficient-uniform full-superposition enhanced dissipation on \\(0\\le y\\le1\\).");
...
row("R0.72R 严格离开旧加权锥：显式四实维 polydisc 整体满足 \\(Q_2(0)\\ge14/25>1/2\\)，沿声明热路径对所有 \\(y\\ge0\\) 保持恰好两个临界点；物理 shape constants \\((\\pi/48,144,240)\\) 与 family-uniform ED 只在 \\(0\\le y\\le1\\) 的 cell window 使用。每条路径穿过 \\(Q_2=1/2\\) 时仍统一非退化；旧边界因此不是 caustic。这里只证明 caustic-free compact core，不作完整四维 chamber 分类。", "R0.72R rigorously leaves the old weighted cone: the explicit four-real-dimensional polydisc satisfies \\(Q_2(0)\\ge14/25>1/2\\) throughout and retains exactly two critical points for every \\(y\\ge0\\) along the stated heat path. The physical shape constants \\((\\pi/48,144,240)\\) and family-uniform enhanced dissipation are used only on the cell window \\(0\\le y\\le1\\). Every path remains uniformly nondegenerate when crossing \\(Q_2=1/2\\), so the old boundary is not the caustic. Only a caustic-free compact core is proved here, not a complete four-dimensional chamber classification.");
