...
const outputRoot = resolve(process.env.R075R_STEP43_BROWSER_QA_OUTPUT ?? "/tmp/r075r-step43-browser-qa");
const externalBase = process.env.R075R_PUBLIC_BASE_URL;
...
try {
  for (const scenario of scenarios) {
    ...
    const url = new URL("/notes/r0-75r.html", baseUrl);
    url.searchParams.set("lang", scenario.lang);
    url.searchParams.set("qa", "r075r-step43");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        step43MathJaxErrors: document.querySelectorAll("#s-335 mjx-merror, #s-336 mjx-merror, #s-337 mjx-merror, #s-338 mjx-merror, #s-339 mjx-merror, #s-340 mjx-merror, #s-341 mjx-merror, #s-342 mjx-merror, #s-343 mjx-merror").length,
        ...
        figureSections: document.querySelectorAll('#figure, [src*="r075r"], [href*="assets/r075r"]').length,
        negativeResult: /NEGATIVE RESULT/.test(visibleText),
        exactSmoothShear: /EXACT SMOOTH SHEAR/.test(visibleText),
        realHighBandPacket: /REAL HIGH-BAND PACKET/.test(visibleText),
        outerCapConcentration: /OUTER-CAP CONCENTRATION/.test(visibleText),
        dirichletKernel: /DIRICHLET KERNEL/.test(visibleText),
        signedFluxLower: /SIGNED FLUX LOWER/.test(visibleText),
        plateauCubicUpper: /PLATEAU CUBIC UPPER/.test(visibleText),
        amplitudeCancels: /AMPLITUDE CANCELS/.test(visibleText),
        plateauOnlyNoGo: /PLATEAU-ONLY NO-GO/.test(visibleText),
        fullSupportOpen: /FULL SUPPORT OPEN/.test(visibleText),
        versionMOpen: /VERSION-M OPEN/.test(visibleText),
    ...
    assert.equal(audit.step43MathJaxErrors, 0, `${scenario.id} Step 43 MathJax errors: ${JSON.stringify(audit.mathJaxErrorContexts.filter((entry) => /^s-3(?:3[5-9]|4[0-3])$/.test(entry.section ?? "")))}`);
    ...
    assert.equal(audit.figureSections, 0, `${scenario.id} no R0.75R formal figure`);
    for (const key of [
      "negativeResult", "exactSmoothShear", "realHighBandPacket", "outerCapConcentration", "dirichletKernel",
      "signedFluxLower", "plateauCubicUpper", "amplitudeCancels", "plateauOnlyNoGo", "fullSupportOpen", "versionMOpen", "e24Open",
