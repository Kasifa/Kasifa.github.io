...
// Cryptographically bind the synchronized R0.73U PDFs to their HTML sources
...
const usage =
  "usage: bind-r073u-pdfs.mjs (--apply | --check-only | " +
...
const outputPath = resolve(root, "research/r073u_pdf_bindings.json");
...
const releaseTitle = "R0.73U | Full tensors in the heat hierarchy: pressure is recoverable, but the even quadratic state is not dynamically closed";
const publicTitle = "R0.73U｜完整张量进入热层级：压力可以恢复，但偶二次状态的动力学并不闭合";
const documentTitle = "R0.73U｜Full tensors in the heat hierarchy: pressure is recoverable, but the even quadratic state is not dynamically closed";
const recapTitle = "R0.61–R0.73U｜R0.60 之后的研究回顾";
...
async function atomicWrite(path, payload) {
  ...
  const temporary = resolve(
    dirname(path),
    `.${basename(path)}.r073u-${process.pid}-${Date.now()}-${process.hrtime.bigint()}.tmp`,
  );
  try {
...
if (/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/.test(report + dictionary)) {
  throw new Error("R0.73U canonical report or dictionary contains control characters");
}
if (!report.startsWith("# " + releaseTitle + "\n") ||
    !report.includes("**Public title (zh):** " + publicTitle)) {
  throw new Error("R0.73U canonical report title ledger drift");
}
const boundaryCorpus = (report + "\n" + dictionary).replace(/\s+/g, " ");
for (const phrase of [
  "is not the classical two-point KHM tensor",
  "reconstructs instantaneous pressure at the same heat scale",
  "not as a physical-time stress closure",
  "does not give the flux \\(-\\tau_s:\\nabla v_s\\) a fixed sign",
  "is circular for arbitrary-data global regularity",
  "is not uniform control at zero scale",
  "cannot support a novelty or priority claim",
  ...
  if (!boundaryCorpus.includes(phrase)) {
    throw new Error("R0.73U canonical sources missing boundary phrase: " + phrase);
  }
}
for (const token of [
...
if (
  ...
  throw new Error("R0.73U HTML/accounting apply must precede PDF binding");
}

const documents = [
  {
    kind: "research-note",
    html: "public/notes/r0-73u.html",
    pdf: "public/notes/r0-73u.pdf",
    provenance: "research/r073u_note_pdf_render.json",
    route: "/notes/r0-73u.html",
    title: documentTitle,
    htmlMarkers: [publicTitle, "heatCovariancePSD=INTERNAL_EXACT", figureId],
  },
  {
    kind: "cumulative-recap",
    html: "public/recap-r0-61-r0-73u.html",
    pdf: "public/recap-r0-61-r0-73u.pdf",
    provenance: "research/r073u_recap_pdf_render.json",
    route: "/recap-r0-61-r0-73u.html",
    title: recapTitle,
    htmlMarkers: ["R0.73U", "137", "finiteGeneralTensorClosure=OPEN"],
  },
];
...
const manifest = {
  schemaVersion: "r073u-synchronized-pdf-bindings-v1",
  release: "R0.73U",
  canonicalTitleSource: {
    path: "research/r073u_report-source.md",
    sha256: sha256(Buffer.from(report)),
    releaseTitle,
    publicChineseTitle: publicTitle,
  },
  canonicalBoundarySource: {
    path: "research/r073u_bilingual_dictionary.md",
  ...
  claimBoundary: {
    ...
    conditionalCriticalStressRow: "INTERNAL_COROLLARY",
    conditionalCriticalStressRowAssumesL4tL6x: true,
    fixedPositiveScaleEnergyStressBound: "INTERNAL_COROLLARY",
    ...
    finiteFormulaDiagnosticChecks: 75,
    formalFigureChecks: 325,
    formalFigureRows: 138,
...
if (apply) {
  ...
  if (!current.equals(payload)) throw new Error("R0.73U PDF binding manifest is stale");
}
...
console.log(JSON.stringify({
  ...
  output: "research/r073u_pdf_bindings.json",
}));
