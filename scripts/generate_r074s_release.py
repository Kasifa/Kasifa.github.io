...
RELEASE = "r074s"
CODE = "R0.74S"
TITLE = "R0.74S｜共享 best-N budget 与终端 trace 障碍"
...
<title>{TITLE}</title><meta name="description" content="共享 best-N budget 精确重组；short branch 的 terminal trace 与 scalar-excess uniform packing 仍开放">
...
<body><nav class="top"><a href="/research-review.html">研究首页</a><span>R0.74S · 2026-09-03</span></nav><main><header class="hero"><div class="hero-inner"><div><div class="section-no">研究笔记 R0.74S · 完整中文版本</div><h1>{TITLE}</h1><p>两类 residual 的共享 best-\\(N\\) budget 是精确 infimal convolution；short branch 仍缺 terminal trace，scalar-excess branch 仍缺 uniform packing。<strong>S.261 / S.269 / S.272 OPEN. NOT CLAY.</strong></p><div class="labels"><span class="label">PROVED</span><span class="label">SHARED BUDGET</span><span class="label">NESTED TENT</span><span class="label">TERMINAL TRACE OPEN</span><span class="label">EXCESS PACKING OPEN</span><span class="label">NOT CLAY</span></div></div><div class="stamp"><strong>状态 · R0.74S STEP 11</strong><p>S.248–S.272：PROVED / OPEN 分列</p><p>shared infimal convolution：EXACT</p><p>short inverse-duration：PROVED</p><p>critical s² Carleson：LOG GAP</p><p>terminal trace S.261：OPEN</p><p>excess constants：1/5 与 3</p><p>uniform packing S.269：OPEN</p><p>combined S.272：OPEN</p><p>regularity：OPEN</p><p>无仿真 / NO DGX</p></div></div></header><article>
{report_body()}
<section id="figure"><div class="section-no">F / 上一阶段结构图</div><h2>Step 10 已付分支删除与 residual gate</h2><picture><source srcset="/assets/r074s/{FIGURE_ID}.svg" type="image/svg+xml"><img src="/assets/r074s/{FIGURE_ID}.png" alt="R0.74S Step 10 paid-branch deletion and residual gate"></picture><p><a href="/assets/r074s/{FIGURE_ID}.pdf">矢量 PDF</a> · <a href="/assets/r074s/{FIGURE_ID}.png">600 dpi PNG</a> · <a href="/assets/r074s/{FIGURE_ID}.svg">SVG</a> · <a href="/figures/r074s/{FIGURE_ID}/source-data.csv">source data</a> · <a href="/figures/r074s/{FIGURE_ID}/qa-report.md">视觉 QA</a></p><p class="note">这是 Step 10 的继承结构图。Step 11 是解析归约，没有另造新图；terminal trace 与 selected-excess packing 仍开放。</p></section>
<section id="reproduce"><div class="section-no">R / 冻结证据</div><h2>Step 11 解析主文、证书与独立审计</h2><p class="files"><a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_shared_budget_terminal_trace_obstruction.md">Step 11 最终解析主文</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_shared_budget_terminal_trace_primary_audit.md">Step 11 主审计</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_shared_budget_terminal_trace_independent_audit.md">Step 11 独立审计</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_shared_budget_terminal_trace_certificate.json">Step 11 机器证书 JSON</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_shared_budget_terminal_trace_certificate_report.md">Step 11 证书报告</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_claim_state_update.md">主张边界</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_literature_boundary.md">文献边界</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_report-source.md">中文 reader source</a></p><p><a href="/notes/r0-74s.pdf">同步研究笔记 PDF</a> · <a href="/recap-r0-61-r0-74o.html">上一大里程碑 recap（截止 R0.74O，157 节）</a> · <a href="/recap-r0-61-r0-74o.pdf">PDF</a></p><p class="note">Step 11：Python 14/14 exact、7/7 finite、34/34 structural、7/7 mutations；Ruby 7/7 groups、206,891 cases、6/6 artifact locks、7/7 dependency locks、59/59 note checks。有限证书不替代 inherited local-energy/PDE 解析证明。</p></section>
<section id="next"><div class="section-no">NEXT / 下一门槛</div><h2 style="margin:.35rem 0 .15rem;font-size:1.15rem">R0.74T</h2><p style="margin:.15rem 0">分别证明或反驳 short terminal anti-concentration (S.261) 与 uniform selected-excess packing (S.269)，再以相加后的一个共享固定 budget 检验 (S.272)。</p></section></article></main></body></html>'''
...
def update_home() -> None:
    ...
    pairs = (
        ('data-site-version="1.89"', 'data-site-version="1.90"', "home version"),
        ('/i18n-en.js?v=1.89', '/i18n-en.js?v=1.90', "home i18n"),
        ('/site-refresh.js?v=1.89.1', '/site-refresh.js?v=1.90.1', "home refresh"),
        ('<strong>v1.89</strong>网页版本', '<strong>v1.90</strong>网页版本', "home stat version"),
        ('<h3>R0.74S：删除已付分支后的 residual best-N gate</h3>', '<h3>R0.74S：共享 best-N budget 与 terminal trace 障碍</h3>', "current route title"),
        ('<p class="tree-current-summary">六类精确分区把四个已付分支压入一次 Q ledger 与一次 cubic ledger；R_sh 与 R_x 共享一个 residual best-N gate，S.243 仍开放。NOT CLAY。</p>', '<p class="tree-current-summary">共享 budget 的 infimal convolution 已精确闭合；short branch 的 terminal trace 与 scalar-excess uniform packing 仍开放。现有 multi-packet exact families 不否定 fixed positive N。NOT CLAY。</p>', "current route summary"),
        ('<p class="tree-path">canonical last exit → D-first six-class partition → single-ledger payment → two-mechanism residual gate</p>', '<p class="tree-path">shared infimal convolution → inverse-duration / nested tent → terminal trace boundary → selected-excess packing</p>', "current route path"),
        ('综述 v1.89 · 2026-09-03', '综述 v1.90 · 2026-09-03', "footer"),
    )
    for old, new, label in pairs:
        page = replace_once_or_present(page, old, new, label)
    page, count = re.subn(r'<div class="summary-item"><strong>我目前关注</strong><span>.*?</span></div>', '<div class="summary-item"><strong>我目前关注</strong><span>R0.74S Step 11 已把共享 exception budget 写成精确 infimal convolution。下一门槛是分别证明 short terminal anti-concentration S.261 与 selected-excess packing S.269，再合并为 S.272。</span></div>', page, count=1, flags=re.S)
    if count != 1:
        raise RuntimeError("home focus replacement failed")
    latest = '''<section class="route-overview latest-release-spotlight" id="latest-release" aria-labelledby="latest-release-title"><div class="route-overview-inner"><header class="route-map-header"><div><p class="eyebrow">LATEST RELEASE · R0.74S · 2026-09-03</p><h2 class="route-map-title" id="latest-release-title">R0.74S｜共享 best-N budget 与终端 trace 障碍</h2><p class="route-map-intro">共享 budget 精确闭合；short terminal trace 与 selected-excess packing 仍 OPEN。NOT CLAY.</p></div><nav class="route-map-actions" aria-label="最新发布快捷入口"><a class="route-map-latest" href="/notes/r0-74s.pdf">阅读最新 R0.74S 研究笔记 →</a><a href="/assets/r074s/fig-r074s-ball-clock-debt.pdf">上一阶段结构图</a><a href="/recap-r0-61-r0-74o.html">最新大里程碑 recap（R0.61–R0.74O，157 节）</a><a href="/notes/">221 篇研究笔记总索引</a><a href="#r074s">查看首页 R0.74S 卡片</a></nav></header><div class="route-legend" aria-label="最新发布计数"><span><i class="route-legend-mark kept" aria-hidden="true"></i>R0.70A–R0.74S · 123 节已公开</span><span><i class="route-legend-mark kept" aria-hidden="true"></i>98 节完整封存</span><span><i class="route-legend-mark current" aria-hidden="true"></i>当前端点 R0.74S</span></div></div></section>'''
    ...
    old_next = '<div class="tree-row"><article class="tree-node next"><div class="tree-node-head"><span class="route-range">NEXT · R0.74T</span><span class="tree-state current">下一检查点</span></div><h3>R0.74T 下一接口</h3><p>分别研究 R_sh 的 short non-D、Q-small packing 与 R_x 的 anomalous-defect / high-Rayleigh ancestry，最终以一个共享的固定 N0 exception budget 检验 S.243。</p></article></div>'
    new_next = '<div class="tree-row"><article class="tree-node next"><div class="tree-node-head"><span class="route-range">NEXT · R0.74T</span><span class="tree-state current">下一检查点</span></div><h3>R0.74T 下一接口</h3><p>分别证明或反驳 short terminal anti-concentration S.261 与 uniform selected-excess packing S.269，再用相加后的固定 budget 检验 S.272。</p></article></div>'
    page = replace_once_or_present(page, old_next, new_next, "next route")
    page = replace_once_or_present(page, 'terminal-window convex packing / arbitrary-clock triage / no-exception no-go → canonical last exit → six-class paid/residual partition → shared residual best-N gate</p>', 'terminal-window convex packing / arbitrary-clock triage / no-exception no-go → shared budget → terminal trace / selected-excess packing</p>', "path tail")
    card = '''          <div class="task-one" id="r074s" data-release="r074s" style="margin-top:2rem"><p class="eyebrow">研究笔记 R0.74S · 2026-09-03</p><h3>R0.74S｜共享 best-N budget 与终端 trace 障碍</h3><p>共享 budget 精确闭合；short terminal trace 与 selected-excess packing 仍 OPEN。NOT CLAY.</p><p><a href="/notes/r0-74s.html"><strong>阅读完整中文笔记 →</strong></a> · <a href="/notes/r0-74s.pdf">PDF</a> · <a href="/assets/r074s/fig-r074s-ball-clock-debt.pdf">上一阶段结构图</a></p></div>\n'''
...
def update_literature() -> None:
    ...
    for old, new, label in (
        ('data-site-version="1.89"', 'data-site-version="1.90"', "lit version"), ('/i18n-en.js?v=1.89', '/i18n-en.js?v=1.90', "lit i18n"), ('文献综述 v1.89 · 2026-09-03', '文献综述 v1.90 · 2026-09-03', "lit footer"),
    ):
        page = replace_once_or_present(page, old, new, label)
    old = '<div class="route-step kept"><header><b>R0.74S</b><strong>删除已付分支后的 residual best-N gate</strong></header><p>S.223–S.231 给出 D-first 六类分区与 single Q/cubic payments；S.232–S.240 在两类 residual 上证明 T/6&lt;r&lt;T/2，并建立 domain-safe best-N 等价。S.243 的 fixed-N PDE packing 仍开放。<a href="/notes/r0-74s.html">研究笔记</a> <a href="#r074s-boundary">主张边界</a></p></div><div class="route-step pause"><header><b>开放接口 · R0.74T</b><strong>two-mechanism residual packing</strong></header><p>分别研究 R_sh 与 R_x，最终用一个共享的固定 N0 exception budget 重组。</p></div>'
    new = '<div class="route-step kept"><header><b>R0.74S</b><strong>共享 best-N budget 与 terminal trace 障碍</strong></header><p>S.249 给出共享 budget 的 exact infimal convolution；S.253–S.259 给出 short inverse-duration、nested-tent 与 positive-depth 控制；S.262–S.265 给出 excess/residual 常数 1/5、3 与 nonuniform compactness。S.261、S.269、S.272 仍开放。<a href="/notes/r0-74s.html">研究笔记</a> <a href="#r074s-boundary">主张边界</a></p></div><div class="route-step pause"><header><b>开放接口 · R0.74T</b><strong>terminal trace / selected-excess packing</strong></header><p>分别证明或反驳 S.261 与 S.269，再以相加后的固定 budget 检验 S.272。</p></div>'
    page = replace_once_or_present(page, old, new, "literature route")
    boundary = '<h3 id="r074s-boundary">R0.74S 的文献与主张边界</h3><p>Step 11 在继承的 paid/residual partition 上做共享 budget、terminal trace 与 selected-excess packing 审计；不声称新颖性、优先权或新的 PDE packing theorem。</p><div class="boundary"><strong>R0.74S Step 11 公开边界</strong><p>PROVED：shared-budget exact infimal convolution；short inverse-duration、nested-tent 与 positive-depth control；critical s² Carleson logarithmic obstruction at coefficient level；excess/residual best-N constants 1/5 and 3；fixed-solution nonuniform tightness；N+1-target falsification test。ABSTRACT STRESS TESTS：不是 NSE counterexamples。FINITE：Python 14/14 exact、7/7 finite、34/34 structural、7/7 mutations；Ruby 7/7 groups、206,891 cases、59/59 note checks。OPEN：S.261、S.269、S.272、S.243、Q.12、Q.1 与正则性。现有 multi-packet exact families 不 refute fixed positive N。保留此前 no-go。<strong>NOT CLAY.</strong> <a href="/notes/r0-74s.html">阅读完整中文笔记</a>。</p></div>\n'
...
def update_notes_index() -> None:
    ...
    for old, new, label in (
        ('data-site-version="1.89"', 'data-site-version="1.90"', "index version"), ('/i18n-en.js?v=1.89', '/i18n-en.js?v=1.90', "index i18n"), ('/site-refresh.js?v=1.89', '/site-refresh.js?v=1.90', "index refresh"), ('研究笔记总索引 · v1.89 · 2026-09-03', '研究笔记总索引 · v1.90 · 2026-09-03', "index footer"),
    ):
        page = replace_once_or_present(page, old, new, label)
    entry = '''          <li class="note-entry" data-note="r0-74s"><article><div class="entry-copy"><p class="note-code">R0.74S</p><h3>共享 best-N budget 与终端 trace 障碍</h3></div><nav class="entry-files" aria-label="R0.74S files"><a class="file-link html" href="/notes/r0-74s.html" aria-label="Read R0.74S HTML">HTML</a><a class="file-link pdf" href="/notes/r0-74s.pdf" aria-label="Download R0.74S PDF">PDF</a></nav></article></li>\n'''
...
def update_accounting() -> None:
    ...
    write_json(PUBLIC / "site-version.json", {"schemaVersion": "research-site-version-v1", "version": VERSION, "latestRelease": CODE, "publicHtmlNoteCount": html_count, "postR060PublishedNodeCount": post_r060, "postR060RecapNodeCount": 157, "latestRecapRelease": "R0.74O", "publicPdfNoteCount": pdf_count, "publishedDate": "2026-09-03"})
