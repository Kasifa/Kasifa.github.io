...
// Render and bind the complete Chinese R0.74N note. This certifies publication
// bytes and PDF structure; it does not certify the mathematical theorem.
...
const release = "r074n";
const noteSlug = "r0-74n";
const title = "R0.74N｜把所有壳层合起来，完整领圈条件闭合了";
...
try {
  ...
  if (structure.title !== title) throw new Error("R0.74N PDF title drift");
  ...
  ) throw new Error("R0.74N render provenance mismatch");

  const binding = {
    schemaVersion: "r074n-synchronized-pdf-binding-v1",
    release: "R0.74N",
    ...
    claimBoundary: {
      ...
      matchingExactFamilyEndpointLaw: true,
      exteriorDissipationMatchingLowerBound: false,
      universalEndpointEstimate: false,
      arbitraryFlowEndpointEstimate: false,
  ...
  await writeFile(
    resolve(root, `research/${release}_pdf_bindings.json`),
