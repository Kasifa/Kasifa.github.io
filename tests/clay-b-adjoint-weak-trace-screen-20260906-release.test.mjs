...
const protectedArtifacts = {
  "public/notes/clay-b-common-adjoint-screen-20260906.html": "aa6e6f9c962dd06cd4079429993cb94540c91ec680094e3a9f20618f6b0b47d6",
...
test("the HTML-only AdjointWeakTraceScreen note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-adjoint-weak-trace-screen-20260906.html");
  for (const marker of [
    "ClayB-AdjointWeakTraceScreen-20260906", "CB.20", "零迹位于前向方程的初端",
    "The zero trace is at the initial endpoint of a forward equation", "有限模态揭示精确的端点成本",
    "Finite modes reveal the exact endpoint cost", "四个唯一性接口均缺少必要输入",
    "All four uniqueness interfaces lack required inputs", "PROVED IN STATED SCOPE", "CONDITIONAL",
    "ENDPOINT AUDIT", "LITERATURE", "FINITE CHECKS ONLY", "OPEN", "NOT CLAY",
  ...
  assert.equal(note.includes("/notes/clay-b-adjoint-weak-trace-screen-20260906.pdf"), false);
  assert.equal(note.includes("R0.76M"), false);
});

test("homepage keeps the solid R0 sequence separate from the dashed CB.1–CB.20 lane", async () => {
  ...
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-adjoint-weak-trace-screen-20260906"));
  ...
  assert.equal((home.match(/id="clay-b-adjoint-weak-trace-screen"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-common-adjoint-screen"/g) ?? []).length, 0);
  assert.equal((literature.match(/id="clay-b-adjoint-weak-trace-screen-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-adjoint-weak-trace-screen-20260906"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-adjoint-weak-trace-screen-20260906.pdf"), false);
  assert.equal(site.version, "2.64");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 20);
  assert.equal(site.latestIndependentNote, "ClayB-AdjointWeakTraceScreen-20260906");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-adjoint-weak-trace-screen-20260906.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.latestIndependentChapter, "CB.20");
  assert.equal(site.nextIndependentChapter, "CB.21");
  assert.equal(manifest.siteVersion, "2.64");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestPublication.releaseId, "clay-b-adjoint-weak-trace-screen-20260906");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-CommonAdjointScreen-20260906");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.20");
});
...
test("prior note, canonical endpoint, recap, and historical PDF remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-adjoint-weak-trace-screen-20260906.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-adjoint-weak-trace-screen-20260906.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-adjoint-weak-trace-screen-20260906.pdf")));
});
