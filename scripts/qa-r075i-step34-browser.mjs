...
const outputRoot = resolve(process.env.R075I_STEP34_BROWSER_QA_OUTPUT ?? "/tmp/r075i-step34-browser-qa");
const externalBase = process.env.R075I_PUBLIC_BASE_URL;
...
try {
  for (const scenario of scenarios) {
    ...
    const url = new URL("/notes/r0-75i.html", baseUrl);
    url.searchParams.set("lang", scenario.lang);
    url.searchParams.set("qa", "r075i-step34");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        figureSections: document.querySelectorAll('#figure, [src*="r075i"], [href*="assets/r075i"]').length,
        diffusionSafeOneBlock: /DIFFUSION-SAFE ONE BLOCK/.test(visibleText),
        noPdeUsed: /NO PDE USED/.test(visibleText),
        nEffExact: /N_EFF EXACT/.test(visibleText),
        sufficientOnly: /SUFFICIENT ONLY/.test(visibleText),
        thetaThreshold: /THETA < 8558\/35721/.test(visibleText),
        betaThreshold: /BETA > 27163\/35721/.test(visibleText),
        favorableRate: /ONE-BLOCK RATE -4279\/238140000/.test(visibleText),
        adverseRate: /UNIFORM RATE \+27163\/476280000/.test(visibleText),
        highParticipationNotNecessary: /HIGH PARTICIPATION NOT NECESSARY/.test(visibleText),
    ...
    assert.equal(audit.figureSections, 0, `${scenario.id} no R0.75I formal figure`);
    assert.equal(audit.diffusionSafeOneBlock, true, `${scenario.id} diffusion-safe one-block boundary`);
    assert.equal(audit.noPdeUsed, true, `${scenario.id} no-PDE boundary`);
    assert.equal(audit.nEffExact, true, `${scenario.id} exact participation boundary`);
    assert.equal(audit.sufficientOnly, true, `${scenario.id} sufficient-only boundary`);
    assert.equal(audit.thetaThreshold, true, `${scenario.id} theta threshold`);
    assert.equal(audit.betaThreshold, true, `${scenario.id} beta threshold`);
    assert.equal(audit.favorableRate, true, `${scenario.id} favorable rate`);
    assert.equal(audit.adverseRate, true, `${scenario.id} adverse uniform-block rate`);
    assert.equal(audit.highParticipationNotNecessary, true, `${scenario.id} high-participation diagnostic`);
