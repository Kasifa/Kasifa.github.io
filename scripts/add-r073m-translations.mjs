...
const snapshotPath = resolve(root, "scripts/i18n-snapshots/r073m-missing.json");
const usage = "usage: add-r073m-translations.mjs (--apply | --check-only | --capture-missing)";
...
if (unknownArguments.length) {
  throw new Error("Unknown R0.73M translation argument: " + unknownArguments[0] + "\n" + usage);
}
...
function assertInsideRoot(canonicalPath, label) {
  ...
  if (
    ...
    throw new Error(label + " escaped R073M_RELEASE_ROOT: " + canonicalPath);
  }
}
...
async function writeTranslationTransaction(writes) {
  if (writes.length !== 3 || new Set(writes.map(({ path }) => path)).size !== 3) {
    throw new Error("R0.73M translation transaction requires exactly three distinct files");
  }
  ...
  for (const [index, { path, payload }] of writes.entries()) {
    ...
    entries.push({
      ...
      temporary: resolve(dirname(path), `.${path.split("/").at(-1)}.r073m-${nonce}-${index}.tmp`),
      backup: resolve(dirname(path), `.${path.split("/").at(-1)}.r073m-${nonce}-${index}.bak`),
      backedUp: false,
      installed: false,
    });
  }
  ...
  try {
    ...
    if (rollbackErrors.length) {
      throw new AggregateError([error, ...rollbackErrors], "R0.73M translation rollback failed");
    }
    throw error;
  } finally {
...
async function writeSingleAtomic(path, payload) {
  ...
  const nonce = `${process.pid}-${Date.now()}-${process.hrtime.bigint()}`;
  const temporary = resolve(dirname(path), `.${path.split("/").at(-1)}.r073m-${nonce}.tmp`);
  try {
...
if (absentPages.length) {
  throw new Error(
    "R0.73M HTML must be generated before translation snapshot/application: " +
...
for (const relativePage of activePages) {
  ...
  if (!html.includes("/i18n-en.js?v=1.53")) {
    throw new Error(relativePage + ": expected i18n cache version v1.53");
  }
...
const retained = translations.filter((entry) => !/^r073m\d+$/.test(entry.id));
const retainedByChinese = new Map(retained.map((entry) => [entry.zh, entry]));
if (retainedByChinese.size !== retained.length) {
  throw new Error("Duplicate Chinese keys outside the R0.73M batch");
}
...
if (JSON.stringify(missingFiles) !== JSON.stringify(activePages)) {
  throw new Error("Unexpected R0.73M missing-string files: " + JSON.stringify(missingFiles));
}
...
if (captureMissing) {
  ...
  console.log(JSON.stringify({
    ...
    snapshot: "scripts/i18n-snapshots/r073m-missing.json",
  }));
  process.exit(0);
}
...
if (!Array.isArray(snapshot) || snapshot.length === 0) {
  throw new Error("Empty R0.73M snapshot");
}
...
if (JSON.stringify(snapshotChinese) !== JSON.stringify(liveChinese)) {
  ...
  throw new Error("R0.73M missing-string snapshot drift at row " + String(index + 1));
}
const englishByChinese = new Map(snapshot.map((entry) => [entry.zh, entry.en]));
if (englishByChinese.size !== snapshot.length) {
  throw new Error("Duplicate R0.73M snapshot keys");
}
...
const translatedEntries = missing.map((entry, index) => {
  ...
  if (!en || containsChinese(en)) {
    throw new Error("Invalid R0.73M English row: " + entry.zh);
  }
  if (/\b(?:we|our|ours|ourselves|us)\b/i.test(en)) {
    throw new Error("Plural voice in R0.73M English row: " + entry.zh);
  }
  if (JSON.stringify(extractProtectedTokens(en)) !== JSON.stringify(extractProtectedTokens(entry.zh))) {
    throw new Error("Protected-token mismatch for R0.73M row: " + entry.zh);
  }
  if (JSON.stringify(accountingTokens(en)) !== JSON.stringify(accountingTokens(entry.zh))) {
    throw new Error("Version/count-token mismatch for R0.73M row: " + entry.zh);
  }
  if (JSON.stringify(boundaryTokens(en)) !== JSON.stringify(boundaryTokens(entry.zh))) {
    throw new Error("Claim-state boundary drift for R0.73M row: " + entry.zh);
  }
  if (JSON.stringify(machineLedgerTokens(en)) !== JSON.stringify(machineLedgerTokens(entry.zh))) {
    throw new Error("Machine-ledger sequence drift for R0.73M row: " + entry.zh);
  }
  return { ...entry, id: "r073m" + String(index + 1).padStart(3, "0"), en };
});
...
if (checkOnly) {
  if (await readFile(snapshotPath, "utf8") !== snapshotOutput) {
    throw new Error("R0.73M snapshot is stale");
  }
  if (JSON.stringify(translations) !== JSON.stringify(finalTranslations)) {
    throw new Error("R0.73M translations are stale");
  }
  if (await readFile(bundlePath, "utf8") !== bundleOutput) {
    throw new Error("R0.73M bundle is stale");
  }
} else if (apply) {
