...
EXTERNAL = [
    ...
    "scripts/r074s_paid_branch_last_exit_certificate_independent.rb",
...
def main() -> None:
    ...
    step10 = json.loads((REPO / "research/r074s_paid_branch_last_exit_certificate.json").read_text(encoding="utf-8"))
    checks += [
        ...
        {"id": "step10_certificate_pass", "pass": step10["summary"] == {"exact_total": 12, "exact_passed": 12, "finite_total": 10, "finite_passed": 10, "structural_total": 79, "structural_passed": 79, "negative_mutations_total": 47, "negative_mutations_passed": 47}},
    ]
    ...
    checks.append({"id": "svg_boundary_language", "pass": all(value in svg for value in ["Paid-branch deletion", "one combined 6 B_Q", "T/6 &lt; r &lt; T/2", "S.243", "NOT CLAY"])})
    checks.append({"id": "caption_boundary_language", "pass": all(value in caption for value in ["six-class", "single", "two residual", "S.243", "NOT CLAY"])})
    ...
    (HERE / "qa-report.md").write_text("# R0.74S figure QA\n\n**PASS.** PDF/PNG pixel parity, 600 dpi geometry, print-size and grayscale derivatives, embedded-font SVG, exact source rows, and all certificate bindings passed. Quick Look SVG preview was generated locally. No simulation or DNS. **FOUR PAID CLASSES USE ONE Q LEDGER AND ONE CUBIC LEDGER; TWO RESIDUAL MECHANISMS SHARE ONE BEST-N GATE; S.243 OPEN; NOT CLAY.**\n", encoding="utf-8")
    ...
    figure_commit = "5fb2c4d1c410ce6a63c0a87e4b7935e4bc6303a4"
    outputs = []
    ...
    manifest = {
        ...
        "analyticalQuestion": "After deleting every already-paid branch, what part of the canonical last-exit tail remains?", "supportedClaim": "Four paid classes use one complete Q ledger and one complete cubic ledger; exactly two residual mechanisms remain, with T/6 < r < T/2, and they share one domain-safe best-N gate.",
        ...
        "claimBoundary": {"sixClassPaidResidualPartition": "PROVED_D_FIRST_EXACT", "lowRayleighExtraResidualClass": False, "oneQPaidLedger": "PROVED_SINGLE_6_B_Q", "oneCubicPaidLedger": "PROVED_SINGLE_C5", "residualClasses": ["R_sh", "R_x"], "residualTwoSidedComparability": "PROVED_T_OVER_6_LT_R_LT_T_OVER_2", "residualBestNReduction": "PROVED_DOMAIN_SAFE", "separateResidualExceptionBudgets": False, "terminalDDominanceLastExitPersistence": False, "fullTerminalBestNTail": "OPEN_Q12", "plateauBestNTail": "OPEN_WEAKER_RESTRICTION", "fixedUniversalN0ResidualEstimate": "OPEN_S243", "fixedScaleInequality": "OPEN_Q1", "globalRegularity": False, "notClay": True},
        ...
        "provenance": {"frozenResearchSourceSha256": sha(REPO / "research/r074s_paid_branch_last_exit_residual.md"), "compatibilityScope": "publication metadata and deterministic analytic summary"}, "claim_boundary": "FOUR PAID CLASSES USE ONE Q LEDGER AND ONE CUBIC LEDGER; TWO RESIDUAL MECHANISMS SHARE ONE BEST-N GATE; S.243 OPEN; NOT CLAY", "external_bindings": {source: sha(REPO / source) for source in EXTERNAL},
