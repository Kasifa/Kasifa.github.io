...
test("homepage current route reaches the materialized G through R0.74A boundary without duplication", async () => {
  ...
  if (endpoint.release.localeCompare("r074b") >= 0) {
    ...
    assert.ok(home.includes(["r075f", "r075g", "r075h", "r075i", "r075j", "r075k", "r075l", "r075m", "r075n", "r075o", "r075p", "r075q", "r075r", "r075s", "r075t", "r075u"].includes(endpoint.release) ? "NEXT · NOT AUTHORIZED" : ["r074x", "r074y", "r074z"].includes(endpoint.release) ? `NEXT · ${endpoint.next} FROZEN PACKAGE` : ["r074t", "r074u", "r074w"].includes(endpoint.release) ? "NEXT · FROZEN PACKAGE" : `NEXT · ${endpoint.next}`));
...
test("literature route records the materialized G through R0.74A boundary", async () => {
  ...
  assert.ok(topology.includes(["r075f", "r075g", "r075h", "r075i", "r075j", "r075k", "r075l", "r075m", "r075n", "r075o", "r075p", "r075q", "r075r", "r075s", "r075t", "r075u"].includes(endpoint.release) ? "开放接口 · 后续未授权" : endpoint.release === "r075a" ? "开放接口 · A.63" : ["r074t", "r074u", "r074w", "r074x", "r074y", "r074z"].includes(endpoint.release) ? "开放接口 · 等待冻结包" : `开放接口 · ${endpoint.next}`));
