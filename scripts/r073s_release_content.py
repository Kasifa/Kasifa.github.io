#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Read the frozen R0.73S sources and expose fail-closed release copy.

This module does not decide mathematical truth and does not write files.  It
extracts reader-facing copy from ``research/r073s_report-source.md``, checks
...
SITE_VERSION = "1.59"
NEXT_RELEASE = "R0.73T"
PUBLIC_TITLE_ZH = "R0.73S｜把三重卷积降到自相关：一个可算证书，两条不能越过的边界"
RELEASE_TITLE_EN = (
    "R0.73S | From triple convolution to autocorrelation: "
    "one computable certificate and two hard limits"
)
FIGURE_ID = "fig-r073s-quadratic-certificate"
FIGURE_SOURCE_RELATIVE = f"research/figures/r073s/{FIGURE_ID}"
FIGURE_ARCHIVE_RELATIVE = f"figures/r073s/{FIGURE_ID}"
...
REPORT_SOURCE = "research/r073s_report-source.md"
DICTIONARY_SOURCE = "research/r073s_bilingual_dictionary.md"
...
FINITE_LEDGER = (
    "finiteFormulaCertificateOnly=TRUE；heatFlowIntegralComputed=FALSE；"
    "navierStokesSimulation=NOT_RUN；runtimeBenchmark=FALSE；"
...
class ReleaseContent:
    ...
    def note_hero(self) -> str:
        return (
            ...
            '      <div><div class="eyebrow">研究笔记 R0.73S · '
            'AUTOCORRELATION CERTIFICATE / EXACT NO-GO BOUNDARY</div>\n'
            f'        <h1>{html.escape(self.public_title_zh)}</h1>\n'
            f'        <p class="lead">{html.escape(self.lead_zh)}</p></div>\n'
            '      <div class="stamp"><span class="state">状态 · R0.73S 完成</span>'
            '<strong>Quadratic autocorrelation + two hard limits</strong>'
            f'<p>版本 R0.73S · {html.escape(self.date)}</p>'
            '<p>base inequality：VERIFIED CLASSICAL</p>'
            '<p>finite certificate / no-go：CLOSED EXACT</p>'
            '<p>universal runtime lower bound：NOT PROVED</p>'
            '<p>L2-only / arbitrary 3D regularity / Clay：OPEN</p>'
            '<p>NOT CLAY</p></div>\n'
            '    </div></header>'
        )
    ...
    def note_article(self) -> str:
        ...
        figure = (
            ...
            '<h2>二次证书、尖锐障碍与同谱相位对</h2>'
            f'<p><img src="/assets/r073s/{FIGURE_ID}.svg" '
            'alt="R0.73S quadratic autocorrelation certificate and exact no-go diagnostics"></p>'
            f'<p><a href="/assets/r073s/{FIGURE_ID}.pdf">下载矢量 PDF</a> · '
            f'<a href="/assets/r073s/{FIGURE_ID}.png">下载 600 dpi PNG</a> · '
            f'<a href="/assets/r073s/{FIGURE_ID}.svg">打开 SVG</a></p>'
            '<p>附图只展示解析公式与有限精确复算。它不是 Navier--Stokes 仿真；'
            '环带见证族的对流非线性严格为零。</p></section>'
        )
        boundary = (
            ...
            '<p>基础不等式直接落在经典 Nikolskii、Hausdorff--Young、Parseval '
            '和加性能量框架内，不是新的调和分析定理。本地工作只承担逐壳充分证书、'
            '严格量词下的 no-go 和 R0.73R matched pair 转移。NOT CLAY。</p></section>'
        )
        reproduction = (
            ...
            'research/r073s_quadratic_autocorrelation_certificate.md">analytic certificate</a> · '
            '<a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/'
            'research/r073s_primary_literature_audit.md">primary literature audit</a> · '
            '<a href="https://github.com/Kasifa/Kasifa.github.io/tree/main/'
            'research/certificates/r073s">finite exact package</a></p>'
            f'<p><a href="/assets/r073s/{FIGURE_ID}.pdf">journal figure PDF</a> · '
            '<a href="/notes/r0-73s.pdf">synchronized note PDF</a> · '
            '<a href="/recap-r0-61-r0-73s.html">135-node cumulative recap</a> · '
            '<a href="/recap-r0-61-r0-73s.pdf">synchronized recap PDF</a></p></section>'
        )
        return "      <article>\n" + body + "\n" + figure + "\n" + boundary + "\n" + reproduction + "\n      </article>"
    ...
    def home_card(self) -> str:
        return (
            '          <div class="task-one" id="r073s" data-release="r073s" style="margin-top:2rem">\n'
            f'            <p class="eyebrow">研究笔记 R0.73S · {html.escape(self.date)}</p>'
            ...
            '            <p><a href="/notes/r0-73s.html"><strong>阅读 R0.73S 研究笔记 →</strong></a>'
            '<br><a href="/notes/r0-73s.pdf">下载同步 PDF</a> · '
            f'<a href="/assets/r073s/{FIGURE_ID}.pdf">下载期刊附图 PDF</a> · '
            '<a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/'
            'research/r073s_quadratic_autocorrelation_certificate.md">查看解析证明</a> · '
            '<a href="/recap-r0-61-r0-73s.html">打开累计回顾</a></p>\n'
    ...
    def recap_phase(self) -> str:
        return (
            f'            <article class="phase"><h3>R0.73S · {html.escape(self.release_title_en)}</h3>'
            ...
            '<div class="links"><a href="/notes/r0-73s.html">R0.73S</a>'
            f'<a href="/assets/r073s/{FIGURE_ID}.pdf">R0.73S 附图</a>'
            '<a href="https://github.com/Kasifa/Kasifa.github.io/tree/main/'
            'research/certificates/r073s">R0.73S 有限证书</a></div></article>'
        )

    @property
    def literature_update(self) -> str:
        return (
            '<span class="route-r073s-deck-update">'
            + html.escape(self.literature_zh)
            + ' quadratic autocorrelation inequality=VERIFIED_CLASSICAL；'
            'universal runtime lower bound=NOT_PROVED；不承担新颖性或优先权声明。</span>'
        )
...
def _regular_text(root: Path, relative: str) -> str:
    ...
    if re.search(r"[\x00-\x07\x0b\x0c\x0e-\x1f\x7f]", value):
        raise CanonicalSourceError("canonical source contains control characters: " + relative)
    # The current frozen draft contains one legacy U+0008 where ``\\beta``
    # was intended.  Source review must repair the canonical file before the
    # publication gate opens, but source-dry-run still needs to inventory the
    # draft without mutating it.
    return value.replace("\x08", r"\b")
...
def _markdown_blocks(markdown: str) -> str:
    ...
    if in_math:
        raise CanonicalSourceError("unterminated display math in R0.73S report")
    return "".join(output)
...
def _sections(report: str) -> tuple[ReportSection, ...]:
    ...
    if len(matches) != 10:
        raise CanonicalSourceError(f"R0.73S report must contain 10 sections, found {len(matches)}")
    used: set[str] = set()
...
def _certificate_final(root: Path) -> tuple[bool, str]:
    path = root / "research/certificates/r073s/manifest.json"
    if not path.is_file() or path.is_symlink():
...
def load_release_content(root: Path | None = None) -> ReleaseContent:
    ...
    if report_title != RELEASE_TITLE_EN:
        raise CanonicalSourceError("R0.73S report title drift: " + report_title)
    public_title = _one(r"(?m)^\*\*Public title \(zh\):\*\*\s*(.+?)\s*$", report, "public title")
    if public_title != PUBLIC_TITLE_ZH:
        raise CanonicalSourceError("R0.73S public title drift: " + public_title)
    ...
    for formula in (
        r"\|f\|_6^6", r"\|C\|_1\|C\|_2^2", "D_C^{1/12}",
        "A(z)=1-z-z^2-z^3+z^4", "311", "323", "Rudin--Shapiro",
    ):
        if formula not in combined.replace(" ", ""):
    ...
    if len(section_one) < 2 or len(section_seven) < 2 or not section_eight:
        raise CanonicalSourceError("R0.73S report lacks required public-copy paragraphs")
    ...
    if "formal figure and public rendering remain pending" in combined:
        failures.append("canonical-ledger-publication-pending")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Read and validate canonical R0.73S release content without writing."
    )
