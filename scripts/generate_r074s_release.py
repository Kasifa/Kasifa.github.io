...
RELEASE = "r074s"
CODE = "R0.74S"
TITLE = "R0.74S｜外侧 collar 对齐与 jump--corona 障碍"
FIGURE_ID = "fig-r074s-outer-collar-corona"
RECAP_HASHES = {
...
<title>{TITLE}</title><meta name="description" content="外侧 collar 同权对齐、临界密度无增益、jump skeleton 与未支付 corona；S.375 仍开放">
...
<body><nav class="top"><a href="/research-review.html">研究首页</a><span>R0.74S · 2026-09-03</span></nav><main><header class="hero"><div class="hero-inner"><div><div class="section-no">研究笔记 R0.74S · 完整中文版本</div><h1>{TITLE}</h1><p>外侧 derivative collar 与同权 payment annulus 精确对齐，临界密度阈值中的 \\(\\lambda\\) 也完全抵消；first-jump skeleton 在已写明的假设下收缩。 <strong>aligned spike 与 critical corona 只是 ABSTRACT METHOD OBSTRUCTIONS；S.358、S.376 保持 CONDITIONAL，S.342、S.375 仍 OPEN. NOT CLAY.</strong></p><div class="labels"><span class="label">PROVED GEOMETRY</span><span class="label">PROVED THRESHOLD</span><span class="label">ABSTRACT OBSTRUCTION</span><span class="label">CONDITIONAL S.358 / S.376</span><span class="label">OPEN S.342 / S.375</span><span class="label">NOT CLAY</span></div></div><div class="stamp"><strong>状态 · R0.74S STEP 14</strong><p>S.343–S.376：PROVED / CONDITIONAL / ABSTRACT METHOD OBSTRUCTION / OPEN</p><p>outer collar ratio：\\(1\\)</p><p>critical threshold：\\(2^{{1/3}}\\mathfrak M_R\\)</p><p>jump skeleton：PROVED</p><p>aligned spike：ABSTRACT</p><p>critical corona：ABSTRACT</p><p>S.358 / S.376：CONDITIONAL</p><p>S.342 / S.375：OPEN</p><p>regularity：OPEN</p><p>解析示意 · 非 simulation / DNS · NO DGX</p></div></div></header><article>
{report_body()}
<section id="figure"><div class="section-no">F / 正式解析示意图</div><h2>外侧 collar 对齐、临界阈值与 jump--corona 边界</h2><picture><source srcset="/assets/r074s/{FIGURE_ID}.svg" type="image/svg+xml"><img src="/assets/r074s/{FIGURE_ID}.png" alt="R0.74S Step 14 analytic schematic of the outer-collar and jump-corona boundary"></picture><p><a href="/assets/r074s/{FIGURE_ID}.pdf">矢量 PDF</a> · <a href="/assets/r074s/{FIGURE_ID}.png">600 dpi PNG</a> · <a href="/assets/r074s/{FIGURE_ID}.svg">SVG</a> · <a href="/figures/r074s/{FIGURE_ID}/source-data.csv">exact source data</a> · <a href="/figures/r074s/{FIGURE_ID}/manifest.json">manifest</a> · <a href="/figures/r074s/{FIGURE_ID}/qa-report.md">视觉 QA</a></p><p class="note">这是由精确公式与冻结证据生成的 <strong>analytic schematic</strong>，不是 simulation、DNS 或 NSE counterexample，也不是 regularity / Clay 证据。</p></section>
<section id="reproduce"><div class="section-no">R / 冻结证据</div><h2>Step 14 主文、证书与双重审计</h2><p class="files"><a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_outer_collar_corona_obstruction.md">Step 14 最终解析主文</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_outer_collar_corona_primary_audit.md">Step 14 主审计</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_outer_collar_corona_independent_audit.md">Step 14 独立审计</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_outer_collar_corona_certificate.json">Step 14 机器证书 JSON</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_outer_collar_corona_certificate_report.md">Step 14 证书报告</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_claim_state_update.md">主张边界</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_literature_boundary.md">文献边界</a> · <a href="https://github.com/Kasifa/Kasifa.github.io/blob/main/research/r074s_step14_report-source.md">Step 14 中文 reader source</a></p><p><a href="/notes/r0-74s.pdf">同步研究笔记 PDF</a> · <a href="/recap-r0-61-r0-74o.html">上一大里程碑 recap（截止 R0.74O，157 节，未改动）</a> · <a href="/recap-r0-61-r0-74o.pdf">PDF</a></p><p class="note">Step 14：Python 12/12 exact、9/9 finite groups（74,287 cases）、37/37 structural、3/3 dependency、49/49 negative；Ruby 7/7 groups、82,788 cases、6/6 artifact locks、2/2 dependency locks、68/68 note checks、1/1 primary-artifact group、2/2 negative groups（99 cases）。有限证书不替代 analytic proof。</p></section>
<section id="next"><div class="section-no">NEXT / 本次不启动</div><h2 style="margin:.35rem 0 .15rem;font-size:1.15rem">R0.74T / 后续研究</h2><p style="margin:.15rem 0">本次发布到 R0.74S Step 14 即停止。后续若另行启动，只能继续检验 S.375 的 PDE-level jump--corona payment 或其他明确的新结构；不得把 S.358、S.376 的 conditional interface 写成 theorem。</p></section></article></main></body></html>'''
...
def update_home() -> None:
    ...
    pairs = (
        ('data-site-version="1.92"', 'data-site-version="1.93"', "home version"),
        ('/i18n-en.js?v=1.92', '/i18n-en.js?v=1.93', "home i18n"),
        ('/site-refresh.js?v=1.92.1', '/site-refresh.js?v=1.93.1', "home refresh"),
        ('<strong>v1.92</strong>网页版本', '<strong>v1.93</strong>网页版本', "home stat version"),
        ('<h3>R0.74S：时间可积性上限与组合 Morrey 阈值</h3>', '<h3>R0.74S：外侧 collar 对齐与 jump--corona 障碍</h3>', "current route title"),
        ('<p class="tree-current-summary">固定解的时间模量已证；当前 linear-payment 路线严格达不到 2/3 目标。Morrey / Dini 结论保持 conditional，abstract witnesses 不是 NSE counterexamples。NOT CLAY。</p>', '<p class="tree-current-summary">外侧 collar 精确落在同权 payment annulus，临界 density threshold 无增益；jump skeleton 可收缩，但完整 corona 仍取决于 OPEN S.375。抽象障碍不是 NSE counterexamples。NOT CLAY。</p>', "current route summary"),
        ('<p class="tree-path">fixed-solution time sum → exact method ceiling → two-cap 2/3 threshold → critical cubic tree → PDE gates open</p>', '<p class="tree-path">four-channel flux → outer-collar same-weight alignment → threshold cancellation → jump skeleton / unpaid corona → S.375 open</p>', "current route path"),
        ('综述 v1.92 · 2026-09-03', '综述 v1.93 · 2026-09-03', "footer"),
    )
    for old, new, label in pairs:
        page = replace_once_or_present(page, old, new, label)
    page, count = re.subn(r'<div class="summary-item"><strong>我目前关注</strong><span>.*?</span></div>', '<div class="summary-item"><strong>我目前关注</strong><span>R0.74S Step 14 已锁定外侧 collar 同权对齐与临界 density cancellation；S.358、S.376 仍 conditional，S.342、S.375 与上游 PDE gates 仍 OPEN。</span></div>', page, count=1, flags=re.S)
    if count != 1:
        raise RuntimeError("home focus replacement failed")
    latest = '''<section class="route-overview latest-release-spotlight" id="latest-release" aria-labelledby="latest-release-title"><div class="route-overview-inner"><header class="route-map-header"><div><p class="eyebrow">LATEST RELEASE · R0.74S · 2026-09-03 · STEP 14</p><h2 class="route-map-title" id="latest-release-title">R0.74S｜外侧 collar 对齐与 jump--corona 障碍</h2><p class="route-map-intro">同权外侧 collar 与临界阈值无增益已证；first-jump skeleton 与未支付 corona 已分离。S.358、S.376 保持 CONDITIONAL，S.342、S.375 仍 OPEN。NOT CLAY.</p></div><nav class="route-map-actions" aria-label="最新发布快捷入口"><a class="route-map-latest" href="/notes/r0-74s.pdf">阅读最新 R0.74S 研究笔记 →</a><a href="/assets/r074s/fig-r074s-outer-collar-corona.pdf">Step 14 正式解析示意图</a><a href="/recap-r0-61-r0-74o.html">最新大里程碑 recap（R0.61–R0.74O，157 节）</a><a href="/notes/">221 篇研究笔记总索引</a><a href="#r074s">查看首页 R0.74S 卡片</a></nav></header><div class="route-legend" aria-label="最新发布计数"><span><i class="route-legend-mark kept" aria-hidden="true"></i>R0.70A–R0.74S · 123 节已公开</span><span><i class="route-legend-mark kept" aria-hidden="true"></i>98 节完整封存</span><span><i class="route-legend-mark current" aria-hidden="true"></i>当前端点 R0.74S Step 14</span></div></div></section>'''
    ...
    old_next = '<div class="tree-row"><article class="tree-node next"><div class="tree-node-head"><span class="route-range">NEXT · R0.74T</span><span class="tree-state current">未启动</span></div><h3>Step 14 / R0.74T 后续接口</h3><p>若另行启动：检验 PDE incidence / strict cubic Dini gain、S.342 与 bare-class S.328；本次发布不启动 Step 14。</p></article></div>'
    new_next = '<div class="tree-row"><article class="tree-node next"><div class="tree-node-head"><span class="route-range">NEXT · R0.74T</span><span class="tree-state current">未启动</span></div><h3>R0.74T 后续研究</h3><p>若另行启动：只继续检验 S.375 的 PDE-level jump--corona payment 或其他明确新结构；本次发布在 R0.74S Step 14 停止。</p></article></div>'
    page = replace_once_or_present(page, old_next, new_next, "next route")
    page = replace_once_or_present(page, 'terminal-window convex packing / arbitrary-clock triage / no-exception no-go → time-integrability ceiling → Morrey / cubic Dini thresholds → PDE gates open</p>', 'terminal-window convex packing / arbitrary-clock triage / no-exception no-go → time-integrability ceiling → outer-collar alignment → jump skeleton / corona → S.375 open</p>', "path tail")
    card = '''          <div class="task-one" id="r074s" data-release="r074s" style="margin-top:2rem"><p class="eyebrow">研究笔记 R0.74S Step 14 · 2026-09-03</p><h3>R0.74S｜外侧 collar 对齐与 jump--corona 障碍</h3><p>同权外侧 collar、临界 density cancellation 与 first-jump skeleton 已锁定；aligned spike / critical corona 是抽象方法障碍，S.375 仍 OPEN。NOT CLAY.</p><p><a href="/notes/r0-74s.html"><strong>阅读完整中文笔记 →</strong></a> · <a href="/notes/r0-74s.pdf">PDF</a> · <a href="/assets/r074s/fig-r074s-outer-collar-corona.pdf">正式解析示意图</a></p></div>\n'''
...
def update_literature() -> None:
    ...
    for old, new, label in (
        ('data-site-version="1.92"', 'data-site-version="1.93"', "lit version"), ('/i18n-en.js?v=1.92', '/i18n-en.js?v=1.93', "lit i18n"), ('文献综述 v1.92 · 2026-09-03', '文献综述 v1.93 · 2026-09-03', "lit footer"),
    ):
        page = replace_once_or_present(page, old, new, label)
    old = '<div class="route-step kept"><header><b>R0.74S</b><strong>时间可积性上限与组合 Morrey 阈值</strong></header><p>S.307–S.325 给出 fixed-solution 时间模量、linear-payment exact ceiling 与 abstract saturation；S.326–S.341 给出 conditional Morrey inference、two-cap 2/3 threshold、heat-shear screen 与 critical cubic tree / Dini interface。S.342 与 PDE gates 仍开放。<a href="/notes/r0-74s.html">研究笔记</a> <a href="#r074s-boundary">主张边界</a></p></div><div class="route-step pause"><header><b>开放接口 · R0.74T</b><strong>本次未启动</strong></header><p>若另行启动：检验 PDE incidence / strict cubic Dini gain、S.342 与 bare-class S.328。</p></div>'
    new = '<div class="route-step kept"><header><b>R0.74S</b><strong>外侧 collar 对齐与 jump--corona 障碍</strong></header><p>S.343–S.374 锁定 shell-scale flux、同权 outer-collar 对齐、critical density cancellation、first-jump skeleton 与 heat-shear screen；S.358、S.376 保持 conditional，S.342、S.375 仍开放。<a href="/notes/r0-74s.html">研究笔记</a> <a href="#r074s-boundary">主张边界</a></p></div><div class="route-step pause"><header><b>开放接口 · R0.74T</b><strong>本次未启动</strong></header><p>若另行启动：检验 S.375 的 PDE-level jump--corona payment 或其他明确新结构。</p></div>'
    page = replace_once_or_present(page, old, new, "literature route")
    boundary = '<h3 id="r074s-boundary">R0.74S 的文献与主张边界</h3><p>Step 14 证明 outer-collar 同权对齐、临界 density cancellation 与条件内的 first-jump contraction；aligned spike 和 critical corona 只作为 abstract method obstructions，不声称 novelty 或 priority。</p><div class="boundary"><strong>R0.74S Step 14 公开边界</strong><p>PROVED：S.343–S.352、S.356–S.357、S.359–S.368、S.371–S.374 中的 shell-scale flux、collar geometry、incidence algebra、threshold cancellation、jump skeleton 与 heat-shear identities。CONDITIONAL：S.358 依赖 S.356–S.357 的 budgets；S.376 依赖 OPEN S.375。ABSTRACT METHOD OBSTRUCTION：S.353–S.355 与 S.370 不是 NSE counterexamples。FINITE：Python 12/12 exact、9/9 finite groups（74,287 cases）、37/37 structural、3/3 dependency、49/49 negative；Ruby 7/7 groups、82,788 cases、68/68 note checks、99 negative cases。OPEN：S.342、S.375、S.288、S.303、S.272、Q.12、Q.1 与正则性。 <strong>NOT CLAY.</strong> <a href="/notes/r0-74s.html">阅读完整中文笔记</a>。</p></div>\n'
...
def update_notes_index() -> None:
    ...
    for old, new, label in (
        ('data-site-version="1.92"', 'data-site-version="1.93"', "index version"), ('/i18n-en.js?v=1.92', '/i18n-en.js?v=1.93', "index i18n"), ('/site-refresh.js?v=1.92', '/site-refresh.js?v=1.93', "index refresh"), ('研究笔记总索引 · v1.92 · 2026-09-03', '研究笔记总索引 · v1.93 · 2026-09-03', "index footer"),
    ):
        page = replace_once_or_present(page, old, new, label)
    entry = '''          <li class="note-entry" data-note="r0-74s"><article><div class="entry-copy"><p class="note-code">R0.74S · STEP 14</p><h3>外侧 collar 对齐与 jump--corona 障碍</h3></div><nav class="entry-files" aria-label="R0.74S files"><a class="file-link html" href="/notes/r0-74s.html" aria-label="Read R0.74S HTML">HTML</a><a class="file-link pdf" href="/notes/r0-74s.pdf" aria-label="Download R0.74S PDF">PDF</a></nav></article></li>\n'''
