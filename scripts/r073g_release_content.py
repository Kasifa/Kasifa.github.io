...
NOTE_ARTICLE = r'''      <article>
        ...
        <section id="finite"><div class="section-no">10 / Finite diagnostic boundary</div><h2>有限 Fourier 计算只诊断 Sobolev 成本与行泄漏</h2><p>主程序在 28 个正式网格点上比较一维约化恒等式与 generic convolution kernel，最大 scale-one 差为 \(5.21\times10^{-16}\)；独立 FFT/Leray 实现在 5 个预注册哨兵点上的最大差为 \(6.01\times10^{-16}\)。cutoff 比较、有限 top eigenvector、\(H^3/L^2\) 成本和 \(K_z=0,\pm2\) 泄漏都不证明 continuum top cluster、非线性定理、自然阈值或三维正则性。</p></section>
...
HOME_LATEST_SPOTLIGHT = r'''    <section class="route-overview latest-release-spotlight" id="latest-release" aria-labelledby="latest-release-title">
      <div class="route-overview-inner"><header class="route-map-header">
        <div><p class="eyebrow">LATEST RELEASE · R0.73G · 2026-08-30</p><h2 class="route-map-title" id="latest-release-title">过小种子的非线性相对放大与精确二维屏障</h2><p class="route-map-intro">八项 R0.73G 闭合项已分别认证，五个过强推断被拒绝。自然种子、order-one departure、transverse 3D 与 Clay 保持 OPEN。</p></div>
