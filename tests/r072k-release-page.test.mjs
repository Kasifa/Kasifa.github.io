...
test("publishes the 101-node post-R0.60 recap in 27 phases", async () => {
  const recap = await readFile(
    resolve(publicRoot, "recap-r0-61-r0-72k.html"),
    "utf8",
  );

  assert.match(recap, /R0\.61–R0\.72K 的 101 节公开笔记/);
  assert.match(recap, /回顾截止时公开笔记：161/);
  assert.match(recap, /63<\/strong><span>R0\.70A–R0\.72K 已公开版本/);
  assert.match(recap, /39<\/strong><span>当前 formal-figure 合同下完整封存/);
  assert.match(recap, /24<\/strong><span>旧版 formal-figure 档案待回补/);
  assert.match(recap, /27<\/strong><span>按问题划分的研究阶段/);
  assert.equal((recap.match(/<article class="phase">/g) ?? []).length, 27);
  ...
  assert.match(recap, /href="\/recap-r0-61-r0-72j\.html"/);
  assert.match(recap, /href="\/notes\/r0-72k\.html"/);
  assert.match(recap, /R0\.72L/);
  assert.match(recap, /strong coupling/i);
  assert.match(recap, /multiscale|多尺度/i);
  assert.match(recap, /directional zero sampling/i);
  assert.match(recap, /complete complex-target ledger/i);
  assert.match(recap, /R0\.72K 的 directional root-slope theorem/);
  assert.match(recap, /系数 2 尖锐，首根项必要/);
  assert.doesNotMatch(recap, /千禧年问题(?:已经|已被|得到)(?:解决|证明)/);
});

test("retains K while synchronizing v1.28, latest O, next P, and archive counts", async () => {
  ...
  for (const source of [
    ...
    "10.1017/S0013091500017417",
    "10.1090/S0025-5718-04-01708-9",
    "10.1112/S002461079700536X",
  ]) {
...
test("R0.72K table of contents names only existing section anchors", async () => {
  const note = await readFile(resolve(publicRoot, "notes/r0-72k.html"), "utf8");
...
test("the K note keeps the formal figure responsive and all release links visible", async () => {
  const note = await readFile(resolve(publicRoot, "notes/r0-72k.html"), "utf8");
  assert.match(note, /article img\{max-width:100%;height:auto\}/);
  for (const token of [
    "research/r072k_report-source.md",
    "research/r072k_gap_matrix.md",
    "research/r072k_literature_audit.md",
    "research/r072k_independent_audit.md",
    "research/certificates/r072k",
    "figures/r072k-directional-roots/fig-r072k-directional-roots",
    "/figures/r0-72k-directional-roots.pdf",
    "/figures/r0-72k-directional-roots.png",
    "/figures/r0-72k-directional-roots.svg",
    "/notes/r0-72k.pdf",
    "/recap-r0-61-r0-72k.html",
    "/recap-r0-61-r0-72k.pdf",
  ]) {
    assert.ok(note.includes(token), token);
  }
});

test("publishes nontrivial synchronized K note and recap PDFs", async () => {
  const [notePdf, recapPdf] = await Promise.all([
    readFile(resolve(publicRoot, "notes/r0-72k.pdf")),
    readFile(resolve(publicRoot, "recap-r0-61-r0-72k.pdf")),
  ]);
...
test("the release generator starts from J and exposes deterministic K targets", async () => {
  const generator = await readFile(
    resolve(root, "scripts/generate_r072k_release.py"),
    "utf8",
  );
  ...
  assert.match(generator, /data-site-version=\\?"1\.24/);
  assert.match(generator, /if "r072k" not in inventory\[key\]/);
  assert.match(generator, /expected 161 public HTML notes/);
  assert.match(generator, /"recapNodes": 101/);
  assert.match(generator, /"phases": 27/);
});
