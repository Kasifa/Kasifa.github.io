...
const outputRoot = resolve(process.env.R075S_STEP44_BROWSER_QA_OUTPUT ?? "/tmp/r075s-step44-browser-qa");
const externalBase = process.env.R075S_PUBLIC_BASE_URL;
...
try {
  for (const scenario of scenarios) {
    ...
    const url = new URL("/notes/r0-75s.html", baseUrl);
    url.searchParams.set("lang", scenario.lang);
    url.searchParams.set("qa", "r075s-step44");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        step44MathJaxErrors: document.querySelectorAll("#s-344 mjx-merror, #s-345 mjx-merror, #s-346 mjx-merror, #s-347 mjx-merror, #s-348 mjx-merror, #s-349 mjx-merror, #s-350 mjx-merror").length,
        ...
        figureSections: document.querySelectorAll('#figure, [src*="r075s"], [href*="assets/r075s"]').length,
        positiveTheorem: /POSITIVE THEOREM/.test(visibleText),
        completeClock: /COMPLETE CLOCK/.test(visibleText),
        allIntegerFrequencies: /ALL INTEGER FREQUENCIES/.test(visibleText),
        oneRealHarmonic: /ONE REAL HARMONIC/.test(visibleText),
        exactSmoothShear: /EXACT SMOOTH SHEAR/.test(visibleText),
        radialReduction: /RADIAL REDUCTION/.test(visibleText),
        movingPhaseLemma: /MOVING-PHASE LEMMA/.test(visibleText),
        lowHighCoverage: /LOW\/HIGH COVERAGE/.test(visibleText),
        amplitudeCancels: /AMPLITUDE CANCELS/.test(visibleText),
        versionMConditional: /VERSION-M CONDITIONAL/.test(visibleText),
        multimodeOpen: /MULTIMODE OPEN/.test(visibleText),
    ...
    assert.equal(audit.step44MathJaxErrors, 0, `${scenario.id} Step 44 MathJax errors: ${JSON.stringify(audit.mathJaxErrorContexts.filter((entry) => /^s-3(?:4[4-9]|50)$/.test(entry.section ?? "")))}`);
    ...
    assert.equal(audit.figureSections, 0, `${scenario.id} no R0.75S formal figure`);
    for (const key of [
      "positiveTheorem", "completeClock", "allIntegerFrequencies", "oneRealHarmonic", "exactSmoothShear",
      "radialReduction", "movingPhaseLemma", "lowHighCoverage", "amplitudeCancels", "versionMConditional", "multimodeOpen",
    ...
    if (scenario.lang === "en") {
      ...
      assert.match(audit.h1, /Full-frequency complete-clock collar payment/);
      assert.equal(audit.switcher, "中文");
      assert.ok(audit.chinesePdfLabels >= 1, `${scenario.id} Chinese-PDF label`);
    } else {
      assert.match(audit.h1, /单实谐波的全频率完整时钟/);
