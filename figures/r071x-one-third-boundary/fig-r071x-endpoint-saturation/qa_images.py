...
def main() -> None:
    ...
    (ROOT / "qa-report.md").write_text(
        "# R0.71X endpoint-saturation figure QA\n\n"
        ...
        f"- Panel A indexed q powers, guides, fits, and raw-value semantics inspection: {manual}\n"
        f"- Panel B four evidence layers and no-substitution language inspection: {manual}\n"
        f"- Panel C delta collapse and truncation inset inspection: {manual}\n"
        f"- atomProxy-not-J_* labels and caption inspection: {manual}\n"
        ...
        "- finite retained-coset and not-DNS footer visible: PASS\n"
        "- delta=1/128 continuum-IFT-radius non-claim visible: PASS\n"
        "- universal D^(1/3) and regularity non-claims visible: PASS\n",
        encoding="utf-8",
    )
