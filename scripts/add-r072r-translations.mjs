...
row("研究笔记 R0.72R：旧加权充分锥外的显式四实维 caustic-free core、0≤y≤1 的统一 shape contract，以及 K 上 fixed-pattern commensurate 1:2:3 triangular affine-row full-superposition ED corollary。", "Research note R0.72R: an explicit four-real-dimensional caustic-free core beyond the old weighted sufficient cone, a uniform shape contract on 0≤y≤1, and a fixed-pattern commensurate 1:2:3 triangular affine-row full-superposition enhanced-dissipation corollary on K.");
...
row("在 complex 1:2:3 coefficient space 中，显式 polydisc \\(K=\\{|z_2-3/20|\\le1/100,\\ |z_3|\\le1/1000\\}\\) 的全部初始 profile 都满足 \\(Q_2(0)\\ge14/25>1/2\\)，却沿整个声明热路径始终恰有两个临界点。实际 shear 在 \\(0\\le y\\le1\\) 具有统一 \\((r,\\mathfrak C_0,\\mathfrak C_1)=(\\pi/48,144,240)\\)；同一 cell window 上，固定 commensurate 1:2:3 triangular affine-row propagator 得到系数一致的 full-superposition enhanced-dissipation corollary。", "In the complex 1:2:3 coefficient space, every initial profile in the explicit polydisc \\(K=\\{|z_2-3/20|\\le1/100,\\ |z_3|\\le1/1000\\}\\) satisfies \\(Q_2(0)\\ge14/25>1/2\\), yet retains exactly two critical points along the entire stated heat path. On \\(0\\le y\\le1\\), the physical shear has the uniform contract \\((r,\\mathfrak C_0,\\mathfrak C_1)=(\\pi/48,144,240)\\); on the same cell window, the fixed commensurate 1:2:3 triangular affine-row propagator has a coefficient-uniform full-superposition enhanced-dissipation corollary.");
...
row("正式附图在实系数切片上区分旧充分锥、\\(K\\) 的实迹与真实 unit-circle caustic", "On the real-coefficient slice, the formal figure separates the old sufficient cone, the real trace of \\(K\\), and the true unit-circle caustic");
...
row("在本站这条研究路线中，R0.72R 给出一个整体处于旧 jet-safety cone 外的四实维系数集：对所有 \\(y\\ge0\\) 具有统一 root localization；在 \\(0\\le y\\le1\\) 上具有 physical shape margins、heat-path ledger 与 fixed-pattern commensurate 1:2:3 triangular affine-row full-superposition ED corollary。它可作为特殊 triangular mechanism 论文中的定量 lemma，也为逼近真实 caustic 提供有证书的起点。", "On this site's research route, R0.72R gives a four-real-dimensional coefficient set lying entirely outside the old jet-safety cone, with uniform root localization for every \\(y\\ge0\\), and with physical shape margins, a heat-path ledger, and a fixed-pattern commensurate 1:2:3 triangular affine-row full-superposition enhanced-dissipation corollary on \\(0\\le y\\le1\\). It can serve as a quantitative lemma in a paper on the special triangular mechanism and as a certified starting point for approaching the true caustic.");
...
row("新的严格增量是一个非空四实维紧致系数核：它整体位于 R0.72Q 加权锥外，具有 \\(y\\ge0\\) 的全热路径 root localization，并在 \\(0\\le y\\le1\\) 具有物理 shape contract 和 fixed-pattern commensurate 1:2:3 triangular affine-row family-uniform ED。", "The new rigorous increment is a nonempty four-real-dimensional compact coefficient core lying entirely outside the R0.72Q weighted cone, with full-heat-path root localization for \\(y\\ge0\\), and a physical shape contract plus fixed-pattern commensurate 1:2:3 triangular affine-row family-uniform enhanced dissipation on \\(0\\le y\\le1\\).");
...
row("R0.72R 的 four-real-dimensional caustic-free core：整个 \\(K\\) 严格位于旧 \\(Q_2\\le1/2\\) 充分锥外，沿声明热路径对所有 \\(y\\ge0\\) 仍恰有两个临界点；在 \\(0\\le y\\le1\\) 上，物理 shear 具有 shape constants \\((\\pi/48,144,240)\\)，且声明的 fixed-pattern commensurate 1:2:3 triangular affine-row propagator 具有 coefficient-uniform ED corollary。", "The R0.72R four-real-dimensional caustic-free core: all of \\(K\\) lies strictly outside the old sufficient cone \\(Q_2\\le1/2\\) and retains exactly two critical points for every \\(y\\ge0\\) along the stated heat path. On \\(0\\le y\\le1\\), the physical shear has shape constants \\((\\pi/48,144,240)\\), and the stated fixed-pattern commensurate 1:2:3 triangular affine-row propagator has a coefficient-uniform enhanced-dissipation corollary.");
...
row("R0.72R 证明该 \\(Q_2\\) 条件只是一条充分锥：显式四实维 polydisc \\(K\\) 整体满足 \\(Q_2(0)\\ge14/25>1/2\\)，却沿每条声明热路径对所有 \\(y\\ge0\\) 始终恰有两个临界点；在 \\(0\\le y\\le1\\) 上，物理 shear 具有 shape constants \\((\\pi/48,144,240)\\)，且声明的 fixed-pattern 1:2:3 triangular affine-row propagator 具有 coefficient-uniform full-superposition ED corollary。", "R0.72R proves that the \\(Q_2\\) condition is only a sufficient cone: the explicit four-real-dimensional polydisc \\(K\\) satisfies \\(Q_2(0)\\ge14/25>1/2\\) throughout and retains exactly two critical points for every \\(y\\ge0\\) along each stated heat path. On \\(0\\le y\\le1\\), the physical shear has shape constants \\((\\pi/48,144,240)\\), and the stated fixed-pattern 1:2:3 triangular affine-row propagator has a coefficient-uniform full-superposition enhanced-dissipation corollary.");
...
row("累计回顾 R0.61–R0.72R · 2026-08-28", "Cumulative recap R0.61–R0.72R · 2026-08-28");
row("累计回顾保持二十八个问题阶段，并给出 R0.61–R0.72R 的完整逐节点索引。R0.72R 给出旧加权锥外的四实维 caustic-free core、对所有 \\(y\\ge0\\) 的全热路径 root localization，以及 \\(0\\le y\\le1\\) 上的物理 shape contract 与 fixed-pattern commensurate 1:2:3 triangular affine-row coefficient-uniform full-superposition ED。", "The cumulative recap retains twenty-eight problem phases and gives a complete node-by-node index through R0.72R. R0.72R gives a four-real-dimensional caustic-free core beyond the old weighted cone, full-heat-path root localization for every \\(y\\ge0\\), and a physical shape contract plus fixed-pattern commensurate 1:2:3 triangular affine-row coefficient-uniform full-superposition enhanced dissipation on \\(0\\le y\\le1\\).");
...
row("显式 \\(K=\\{|z_2-3/20|\\le1/100,\\ |z_3|\\le1/1000\\}\\) 整体满足 \\(Q_2(0)\\ge14/25\\)，却沿声明热路径对所有 \\(y\\ge0\\) 始终恰有两个临界点；实际 shear 在 \\(0\\le y\\le1\\) 可取 \\((r,C_0,C_1)=(\\pi/48,144,240)\\)。", "The explicit set \\(K=\\{|z_2-3/20|\\le1/100,\\ |z_3|\\le1/1000\\}\\) satisfies \\(Q_2(0)\\ge14/25\\) throughout and retains exactly two critical points for every \\(y\\ge0\\) along the stated heat path; on \\(0\\le y\\le1\\), the physical shear admits \\((r,C_0,C_1)=(\\pi/48,144,240)\\).");
...
row("family-uniform ED 仍属于固定 commensurate 1:2:3 triangular affine-row class，并只在 \\(0\\le y\\le1\\) 的 cell window 使用；wall crossing、任意时变相位与一般三维问题未证明。", "Family-uniform enhanced dissipation remains confined to the fixed commensurate 1:2:3 triangular affine-row class and is used only on the cell window \\(0\\le y\\le1\\); wall crossing, arbitrary time-dependent phases, and the general three-dimensional problem are not proved.");
...
row("R0.72R 严格离开旧加权锥：显式四实维 polydisc 整体满足 \\(Q_2(0)\\ge14/25>1/2\\)，沿声明热路径对所有 \\(y\\ge0\\) 保持恰好两个临界点；物理 shape constants \\((\\pi/48,144,240)\\) 与 fixed-pattern commensurate 1:2:3 triangular affine-row family-uniform ED 只在 \\(0\\le y\\le1\\) 的 cell window 使用。每条路径穿过 \\(Q_2=1/2\\) 时仍统一非退化；旧边界因此不是 caustic。这里只证明 caustic-free compact core，不作完整四维 chamber 分类。", "R0.72R rigorously leaves the old weighted cone: the explicit four-real-dimensional polydisc satisfies \\(Q_2(0)\\ge14/25>1/2\\) throughout and retains exactly two critical points for every \\(y\\ge0\\) along the stated heat path. The physical shape constants \\((\\pi/48,144,240)\\) and fixed-pattern commensurate 1:2:3 triangular affine-row family-uniform enhanced dissipation are used only on the cell window \\(0\\le y\\le1\\). Every path remains uniformly nondegenerate when crossing \\(Q_2=1/2\\), so the old boundary is not the caustic. Only a caustic-free compact core is proved here, not a complete four-dimensional chamber classification.");
