...
// R0.73T translation stage. English copy is accepted only from the local
...
const usage = "usage: add-r073t-translations.mjs (--apply | --check-only | --capture-missing)";
...
const snapshotPath = resolve(root, "scripts/i18n-snapshots/r073t-missing.json");
...
// in this executable. Populate it only after the R0.73T HTML transaction has
...
async function safeAtomicWrite(path, payload) {
  ...
  const temporary = resolve(
    dirname(path),
    `.${path.split("/").at(-1)}.r073t-${process.pid}-${Date.now()}-${process.hrtime.bigint()}.tmp`,
  );
  try {
...
async function writeTransaction(writes) {
  if (writes.length !== 3 || new Set(writes.map(({ path }) => path)).size !== 3) {
    throw new Error("R0.73T translation transaction requires three distinct files");
  }
  const nonce = `${process.pid}-${Date.now()}-${process.hrtime.bigint()}`;
  const rows = writes.map(({ path, payload }, index) => ({
    path,
    payload,
    temporary: resolve(dirname(path), `.${path.split("/").at(-1)}.r073t-${nonce}-${index}.tmp`),
    backup: resolve(dirname(path), `.${path.split("/").at(-1)}.r073t-${nonce}-${index}.bak`),
    backedUp: false,
    installed: false,
  }));
  try {
...
const dictionary = await regularText(dictionaryPath, "R0.73T bilingual dictionary");
const report = await regularText(reportPath, "R0.73T report source");
const releaseTitle = metadataField(dictionary, "Release title");
const publicTitle = metadataField(dictionary, "Public title \\(zh\\)");
if (releaseTitle !== "R0.73T | Dynamic autocorrelation and the pressure-tensor barrier" ||
    publicTitle !== "R0.73T｜自相关进入动力学：一个临界一侧估计与压力张量障碍") {
  throw new Error("R0.73T bilingual dictionary canonical title ledger is absent or drifted");
}
for (const token of requiredDictionaryTokens) {
  if (!dictionary.includes(token)) throw new Error("R0.73T dictionary missing " + token);
}
...
for (const stale of [
  ...
  if (dictionary.includes(stale) || report.includes(stale)) {
    throw new Error("R0.73T canonical source still carries " + stale);
  }
}
if (!/^\*\*Next release:\*\*\s*R0\.73U\s*$/m.test(dictionary)) {
  throw new Error("R0.73T dictionary next-release gate is not frozen to R0.73U");
}
...
if (
  ...
  throw new Error("R0.73T HTML/accounting apply must precede translation work");
}
...
for (const relativePage of activePages) {
  ...
  if (!html.includes("/i18n-en.js?v=1.60")) {
    throw new Error(relativePage + ": expected i18n cache version v1.60");
  }
  if (!html.includes("R0.73T")) throw new Error(relativePage + ": R0.73T marker absent");
...
if (JSON.stringify(releaseBatchFiles) !== JSON.stringify(activePages)) {
  throw new Error("unexpected R0.73T release-batch files: " + JSON.stringify(releaseBatchFiles));
}
...
function capturedReasonCodes(entry) {
  ...
  if (releaseId.test(existing.id)) {
    return ["R073T_RELEASE_BATCH", ...forcedCodes, ...issueCodes];
  }
...
if (action === "--capture-missing") {
  ...
  console.log(JSON.stringify({
    ...
    snapshot: "scripts/i18n-snapshots/r073t-missing.json",
...
const snapshot = JSON.parse(await regularText(snapshotPath, "R0.73T translation snapshot"));
if (!Array.isArray(snapshot) || snapshot.length === 0) throw new Error("empty R0.73T snapshot");
if (snapshot.some((entry) => !entry || typeof entry !== "object" || Array.isArray(entry) ||
  ...
  throw new Error("R0.73T accepts only local-direct-reviewed translation rows");
}
const snapshotByChinese = new Map();
for (const entry of snapshot) {
  if (snapshotByChinese.has(entry.zh)) throw new Error("duplicate R0.73T snapshot key");
  if (!sourceByChinese.has(entry.zh)) throw new Error("R0.73T snapshot contains a non-live key: " + entry.zh);
  if (new Set(entry.reasonCodes).size !== entry.reasonCodes.length) {
    throw new Error("R0.73T snapshot contains duplicate reason codes: " + entry.zh);
  }
  if (new Set(entry.reviewedIssues).size !== entry.reviewedIssues.length) {
    throw new Error("R0.73T snapshot contains duplicate reviewed issues: " + entry.zh);
  }
  if (entry.captureBindingSha256 !== captureBindingSha256(entry)) {
    throw new Error("R0.73T snapshot capture binding drift: " + entry.zh);
  }
  snapshotByChinese.set(entry.zh, entry);
}
for (const entry of releaseBatch) {
  if (!snapshotByChinese.has(entry.zh)) {
    throw new Error("R0.73T snapshot is missing a release-batch row: " + entry.zh);
  }
}
for (const entry of currentLegacyRepairs) {
  if (!snapshotByChinese.has(entry.zh)) {
    ...
    throw new Error(
      "R0.73T snapshot is missing a reviewable historical row " + entry.zh + ": " +
      (issues.length
        ? issues.map(({ code, message }) => `${code} (${message})`).join(", ")
        : "HUMAN_SEMANTIC_REVIEW"),
    );
...
for (const entry of snapshot) {
  if (entry.reviewedIssues.some((code) => !issueReasonCodes.has(code))) {
    throw new Error("R0.73T snapshot contains an unknown reviewed issue: " + entry.zh);
  }
  const existing = existingByChinese.get(entry.zh);
  if (entry.sourceIdAtCapture !== null) {
    if (!existing || existing.id !== entry.sourceIdAtCapture) {
      throw new Error("R0.73T snapshot source-id binding drift: " + entry.zh);
    }
    if (entry.capturedEnglishSha256 === null) {
      throw new Error("R0.73T existing-row capture hash is absent: " + entry.zh);
    }
  } else if (existing && !releaseId.test(existing.id)) {
    throw new Error("R0.73T new-row source-id lifecycle drift: " + entry.zh);
  } else if (entry.capturedEnglishSha256 !== null) {
    throw new Error("R0.73T new-row capture hash must be null: " + entry.zh);
  }
  ...
  const expectedReasonCodes = entry.sourceIdAtCapture === null
    ...
    : releaseId.test(entry.sourceIdAtCapture)
      ? ["R073T_RELEASE_BATCH", ...forcedCodes, ...entry.reviewedIssues]
      : [...forcedCodes, ...entry.reviewedIssues];
  if (JSON.stringify(entry.reasonCodes) !== JSON.stringify(expectedReasonCodes)) {
    throw new Error("R0.73T snapshot reasonCodes binding drift: " + entry.zh);
  }
  ...
  if (existing && entry.capturedEnglishSha256 !== null) {
    ...
    if (currentEnglishSha256 === entry.capturedEnglishSha256) {
      if (JSON.stringify(entry.reviewedIssues) !== JSON.stringify(currentIssueCodes)) {
        throw new Error("R0.73T snapshot captured-issue binding drift: " + entry.zh);
      }
    } else if (currentEnglishSha256 !== sha256Text(englishIssues(entry.zh, entry.en).en)) {
      throw new Error(
        "R0.73T existing English is neither captured nor reviewed snapshot English: " + entry.zh,
      );
    }
  }
  ...
  if (!existing || releaseId.test(existing.id) ||
    ...
    throw new Error("R0.73T snapshot has an unproved historical repair row: " + entry.zh);
  }
}
...
if (JSON.stringify(snapshot.map(({ zh }) => zh)) !== JSON.stringify(reviewBatch.map(({ zh }) => zh))) {
  throw new Error("R0.73T review snapshot order or membership drift");
}
...
for (const entry of snapshot) {
  ...
  if (entry.resolution === "semantic-equivalent-approved") {
    ...
    if (review.issues.length) {
      throw new Error(
        "R0.73T review snapshot requires corrected English for " + entry.zh + ": " +
        review.issues.map(({ code, message }) => `${code} (${message})`).join(", "),
      );
    }
    reviewedEnglishByChinese.set(entry.zh, review.en);
...
function allocateReleaseId() {
  while (true) {
    const candidate = "r073t" + String(nextReleaseId).padStart(3, "0");
    nextReleaseId += 1;
...
if (action === "--check-only") {
  if (await regularText(snapshotPath, "R0.73T translation snapshot") !== snapshotOutput) {
    throw new Error("R0.73T translation snapshot is stale");
  }
  if (JSON.stringify(translations) !== JSON.stringify(finalTranslations)) {
    throw new Error("R0.73T translations are stale");
  }
  if (await regularText(bundlePath, "public/i18n-en.js") !== bundleOutput) {
    throw new Error("R0.73T translation bundle is stale");
  }
} else {
