...
const packageRoot = new URL(
  "../figures/r069d-nonlinear-decoupling/fig-r069d-nonlinear-decoupling/",
  import.meta.url,
);
const publicRoot = new URL("../public/", import.meta.url);
const sha256 = (payload) => createHash("sha256").update(payload).digest("hex");

test("archives the formal R0.69D nonlinear-decoupling figure", async () => {
  const [manifestText, branch, depth, validationText, svg, png, pdf] =
    await Promise.all([
      readFile(new URL("manifest.json", packageRoot), "utf8"),
      readFile(new URL("quadratic-branch.csv", packageRoot), "utf8"),
      readFile(new URL("depth-budgets.csv", packageRoot), "utf8"),
  ...
  assert.equal(manifest.figureId, "fig-r069d-nonlinear-decoupling");
  ...
  assert.match(manifest.supportedClaim, /unique nearby nonlinear branch/);
  assert.match(manifest.claimBoundary, /not an unconditional/);
  assert.equal(branch.trim().split("\n").length, 502);
  assert.equal(depth.trim().split("\n").length, 4 * 86 + 1);
  assert.equal(validation.status, "passed");
  assert.ok(Object.values(validation.checks).every(Boolean));
  assert.match(svg, /bounded reference resolvent forces nonlinear packet decoupling/i);
...
test("publishes byte-exact mirrors of the R0.69D figure", async () => {
  for (const extension of ["svg", "png", "pdf"]) {
    const [archived, published] = await Promise.all([
      ...
      readFile(
        new URL("figures/r0-69d-nonlinear-decoupling." + extension, publicRoot),
      ),
    ]);
