...
const protectedArtifacts = {
  "public/notes/clay-b-euler-compactness-screen-20260906.html": "12ebf99efccdd50dece598c443fdb60455c85ba54bec4146dd4c79af561aefbd",
...
test("the HTML-only EnergyAtomCostScreen note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-energy-atom-cost-screen-20260906.html");
  for (const marker of [
    "ClayB-EnergyAtomCostScreen-20260906", "CB.18", "有定量代价，但没有形成矛盾",
    "There is a quantitative cost, but no contradiction", "原子不提供抛物宽度",
    "An atom does not provide parabolic width", "嵌套窗口不能重复计费",
    "Nested windows cannot be charged repeatedly", "CONDITIONAL NECESSITY", "SIGNED FLUX",
    "GLOBAL TAIL COST", "ALGEBRAIC COMPATIBILITY", "OPEN", "NOT CLAY",
  ...
  assert.equal(note.includes("/notes/clay-b-energy-atom-cost-screen-20260906.pdf"), false);
  assert.equal(note.includes("R0.76M"), false);
});

test("homepage keeps the solid R0 sequence separate from the dashed CB.1–CB.18 lane", async () => {
  ...
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-energy-atom-cost-screen-20260906"));
  ...
  assert.equal((home.match(/id="clay-b-energy-atom-cost-screen"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-euler-compactness-screen"/g) ?? []).length, 0);
  assert.equal((literature.match(/id="clay-b-energy-atom-cost-screen-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-energy-atom-cost-screen-20260906"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-energy-atom-cost-screen-20260906.pdf"), false);
  assert.equal(site.version, "2.62");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 18);
  assert.equal(site.latestIndependentNote, "ClayB-EnergyAtomCostScreen-20260906");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-energy-atom-cost-screen-20260906.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.latestIndependentChapter, "CB.18");
  assert.equal(site.nextIndependentChapter, "CB.19");
  assert.equal(manifest.siteVersion, "2.62");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestPublication.releaseId, "clay-b-energy-atom-cost-screen-20260906");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-EulerCompactnessScreen-20260906");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.18");
});
...
test("prior note, canonical endpoint, recap, and historical PDF remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-energy-atom-cost-screen-20260906.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-energy-atom-cost-screen-20260906.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-energy-atom-cost-screen-20260906.pdf")));
});
