...
const outputRoot = resolve(process.env.R075U_STEP46_BROWSER_QA_OUTPUT ?? "/tmp/r075u-step46-browser-qa");
const externalBase = process.env.R075U_PUBLIC_BASE_URL;
...
try {
  for (const scenario of scenarios) {
    ...
    const url = new URL("/notes/r0-75u.html", baseUrl);
    url.searchParams.set("lang", scenario.lang);
    url.searchParams.set("qa", "r075u-step46");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        step46MathJaxErrors: document.querySelectorAll("#s-358 mjx-merror, #s-359 mjx-merror, #s-360 mjx-merror, #s-361 mjx-merror, #s-362 mjx-merror, #s-363 mjx-merror").length,
        ...
        figureSections: document.querySelectorAll('#figure, [src*="r075u"], [href*="assets/r075u"]').length,
        differenceRowPaid: /DIFFERENCE ROW PAID/.test(visibleText),
        exactlyTwoHarmonics: /EXACTLY TWO HARMONICS/.test(visibleText),
        oneDyadicPair: /ONE DYADIC PAIR/.test(visibleText),
        highCarrier: /HIGH CARRIER/.test(visibleText),
        completeClock: /COMPLETE CLOCK/.test(visibleText),
        weightedPhaseLemma: /WEIGHTED PHASE LEMMA/.test(visibleText),
        phaseDistanceMoment: /PHASE-DISTANCE MOMENT/.test(visibleText),
        slowFastRegimes: /SLOW \/ FAST REGIMES/.test(visibleText),
        amplitudeCancels: /AMPLITUDE CANCELS/.test(visibleText),
        rPowersCancel: /R POWERS CANCEL/.test(visibleText),
        exactRate: /EXACT RATE -2\/11907/.test(visibleText),
        versionMConditional: /VERSION-M CONDITIONAL/.test(visibleText),
        selfSumBlockOpen: /SELF \/ SUM BLOCK OPEN/.test(visibleText),
    ...
    assert.equal(audit.step46MathJaxErrors, 0, `${scenario.id} Step 46 MathJax errors: ${JSON.stringify(audit.mathJaxErrorContexts.filter((entry) => /^s-3(?:5[8-9]|6[0-3])$/.test(entry.section ?? "")))}`);
    ...
    assert.equal(audit.figureSections, 0, `${scenario.id} no R0.75U formal figure`);
    for (const key of [
      "differenceRowPaid", "exactlyTwoHarmonics", "oneDyadicPair", "highCarrier", "completeClock",
      "weightedPhaseLemma", "phaseDistanceMoment", "slowFastRegimes", "amplitudeCancels", "rPowersCancel", "exactRate",
      "versionMConditional", "selfSumBlockOpen",
    ...
    if (scenario.lang === "en") {
      ...
      assert.match(audit.h1, /Complete-clock difference-frequency payment for one two-harmonic dyadic pair/);
      assert.equal(audit.switcher, "中文");
      assert.ok(audit.chinesePdfLabels >= 1, `${scenario.id} Chinese-PDF label`);
    } else {
      assert.match(audit.h1, /单个二谐波 dyadic pair 的差频项完整时钟付款/);
