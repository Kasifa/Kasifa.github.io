#!/usr/bin/env python3
"""Build the R0.72P archive manifest and SHA-256 ledger."""
...
FIGURE_COMMAND = (
    "python3 figures/r072p-superposition-gate/"
    "fig-r072p-superposition-gate/plot.py with the six frozen lineage arguments"
)
PACKAGE_SOURCES = (
    "README.md",
    "caption.md",
...
def require_formal_runtime_gate(
    ...
    if (
        ...
        raise RuntimeError("temporary, dirty, untracked, or source-mismatched R0.72P certificates are forbidden")
...
def main(status: str) -> None:
    ...
    if not visual:
        raise RuntimeError("R072P_VISUAL_QA_INSPECTED=true is required")
    ...
    if not source_paths or source_paths[0] != "research/r072p_report-source.md":
    ...
    if status == "formal":
        source_commit = os.environ.get("R072P_SOURCE_COMMIT", "").strip().lower()
        certificate_commit = os.environ.get("R072P_CERTIFICATE_COMMIT", "").strip().lower()
        dirty = os.environ.get("R072P_DIRTY_AT_CERTIFIED_RUN", "").strip().lower()
        ...
        if source_commit != runtime_source_commit:
            raise RuntimeError("R072P_SOURCE_COMMIT does not match the formal runtime source commit")
        if results.get("repositoryCommitAtBuild") != certificate_commit:
            raise RuntimeError(
                "results.repositoryCommitAtBuild must equal R072P_CERTIFICATE_COMMIT"
            )
    ...
    data_schemas = {
        "config.json": "runtime-input placeholders, exact theorem parameters, panel definitions, dimensions, validation thresholds, publication target, and palette",
        ...
        "data.csv": "panel, route, series, kind, coordinates, y parameter, lambda, R, p, source, pointer, claim status, and note; Panel D is a fixed-polynomial-coupling asymptotic representative",
    ...
    payload = {
        ...
        "computation": {
            "kind": "exact-audit",
            "configuration": (
                "two carriers (R,2R), B=2, p=1/sqrt(2), a fixed nonzero coefficient cone inside |lambda|<=1/8, and one orthogonal affine residue row; Panel D keeps the exact L_(R,epsilon) ledger separate from its fixed-polynomial-coupling representative; the canonical certificate directory is independently closed by its flat SHA256SUMS"
            ),
            "precision": "dense IEEE binary64 presentation grids evaluated from analytic formulas; Panel D is an explicitly labeled asymptotic representative, while certificate files gate lineage and are not interpolated",
            ...
            "arbitraryCarrierCountClosed": False,
            "arbitraryCoefficientGeometryClosed": False,
            "randomSeed": None,
            "monitoring": {
                "enabled": True,
        ...
        "dataSummary": {
            ...
            "parameters": {
                "N": int(parameters["carrierCount"]),
                "B": float(parameters["coherenceB"]),
                "p": float(parameters["p"]),
                "lambdaSafe": float(parameters["lambdaSafe"]),
                "lambdaLowerMustBePositive": bool(parameters["lambdaLowerMustBePositive"]),
                "shapeRadius": float(parameters["shapeRadius"]),
        ...
        "qa": {
            ...
            "wallAndSafeConeInspected": visual,
            "conditionalProvedMarkersInspected": visual,
