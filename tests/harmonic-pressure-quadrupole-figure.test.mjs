...
const figureRoot = new URL(
  "../figures/r069j-quadrupole/fig-r069j-quadrupole/",
  import.meta.url,
);

test("archives the formal R0.69J harmonic-quadrupole figure", async () => {
  ...
  assert.equal(manifest.figureId, "fig-r069j-quadrupole");
  assert.equal(manifest.git.sourceCommit, "7271dd542389ab22b24f6f54980e7d2763188c2f");
  assert.equal(manifest.git.certificateCommit, "83076596d7f2664d81888d9acb24f134d9b3ac5f");
  ...
  assert.ok(caption.includes("remainder envelope proportional to \\(r/R\\)"));
  assert.match(contract, /neither proves Navier-+Stokes regularity\s+nor constructs a\s+singularity/i);
  ...
  for (const output of manifest.figure.outputs) {
    ...
    const publicPayload = await readFile(
      new URL(`../public/figures/r0-69j-quadrupole.${extension}`, import.meta.url),
    );
    assert.deepEqual(publicPayload, payload, output.path + " public mirror mismatch");
  }
});

test("the R0.69J figure package passes the strict validator", () => {
  const run = spawnSync(
...
test("pins a deterministic R0.69J SVG hash salt", async () => {
