...
const protectedArtifacts = {
  "public/notes/clay-b-lagged-pressure-reduction-20260906.html": "d2ededa9db908a140e95ce7b546895d01f777b2e425957afc9476fc13e505d36",
...
const independentChapters = [
  ...
  ["CB.13", "clay-b-recent-source-screen-20260906"],
];

test("the HTML-only RecentSourceScreen note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-recent-source-screen-20260906.html");
  for (const marker of [
    "ClayB-RecentSourceScreen-20260906", "CB.13", "积分 H¹ 能量确实在窗口上趋零", "integral H¹ energy of the recent genuine nonlinear source does become small",
    "真正能产生矛盾的 Q_J 仍是开放输入", "The Q_J that would create a contradiction remains an open input",
    "不是无散速度、NS 轨道", "not a divergence-free velocity, NS trajectory",
    "N¹ᐟ² 或 N⁹ᐟ⁸", "N¹ᐟ² or N⁹ᐟ⁸", "R 不等于 P_&gt;N u", "R is not P_&gt;N u",
    "CONDITIONAL COMPARISON", "METHOD SCREEN", "OPEN", "NOT CLAY",
  ...
  assert.equal(note.includes("/notes/clay-b-recent-source-screen-20260906.pdf"), false);
  assert.equal(note.includes("R0.76M"), false);
});

test("homepage keeps the solid R0 sequence separate from the dashed CB.1–CB.13 lane", async () => {
  ...
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-recent-source-screen-20260906"));
  ...
  const positions = independentChapters.map(([chapter, slug]) => {
    ...
    assert.ok(home.slice(row, row + 1700).includes(chapter), `${chapter} route label`);
    return row;
  });
  assert.deepEqual(positions, [...positions].sort((a, b) => a - b));
  assert.equal((home.match(/class="route-overview independent-release-spotlight"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-recent-source-screen"/g) ?? []).length, 1);
  assert.equal((literature.match(/id="clay-b-recent-source-screen-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-recent-source-screen-20260906"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-recent-source-screen-20260906.pdf"), false);
  assert.equal(site.version, "2.57");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 13);
  assert.equal(site.latestIndependentNote, "ClayB-RecentSourceScreen-20260906");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-recent-source-screen-20260906.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.latestIndependentChapter, "CB.13");
  assert.equal(site.nextIndependentChapter, "CB.14");
  assert.equal(manifest.siteVersion, "2.57");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestPublication.releaseId, "clay-b-recent-source-screen-20260906");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-LaggedPressureReduction-20260906");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.13");
});
...
test("historical milestones, endpoint, prior note, and retained PDFs remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-recent-source-screen-20260906.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-recent-source-screen-20260906.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-recent-source-screen-20260906.pdf")));
});
