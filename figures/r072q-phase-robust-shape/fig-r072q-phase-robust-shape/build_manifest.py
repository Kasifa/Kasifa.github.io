#!/usr/bin/env python3
"""Build the R0.72Q archive manifest and SHA-256 ledger."""
...
FIGURE_COMMAND = (
    "python3 figures/r072q-phase-robust-shape/"
    "fig-r072q-phase-robust-shape/plot.py with the six frozen lineage arguments"
)
PACKAGE_SOURCES = (
    "README.md",
    "caption.md",
...
def require_formal_runtime_gate(
    ...
    if (
        ...
        raise RuntimeError("temporary, dirty, untracked, or source-mismatched R0.72Q certificates are forbidden")
...
def main(status: str) -> None:
    ...
    if not visual:
        raise RuntimeError("R072Q_VISUAL_QA_INSPECTED=true is required")
    ...
    if not source_paths or source_paths[0] != "research/r072q_report-source.md":
    ...
    if status == "formal":
        source_commit = os.environ.get("R072Q_SOURCE_COMMIT", "").strip().lower()
        certificate_commit = os.environ.get("R072Q_CERTIFICATE_COMMIT", "").strip().lower()
        dirty = os.environ.get("R072Q_DIRTY_AT_CERTIFIED_RUN", "").strip().lower()
        ...
        if source_commit != runtime_source_commit:
            raise RuntimeError("R072Q_SOURCE_COMMIT does not match the formal runtime source commit")
        if results.get("repositoryCommitAtBuild") != certificate_commit:
            raise RuntimeError(
                "results.repositoryCommitAtBuild must equal R072Q_CERTIFICATE_COMMIT"
            )
    ...
    data_schemas = {
        ...
        "data.csv": "panel, route, series, kind, Cartesian coordinates, phase parameter, phase-ray angle, radius, distance, source, pointer, claim status, and note",
    ...
    payload = {
        ...
        "computation": {
            "kind": "exact-audit",
            "configuration": (
                "fixed M with Q2<=1/2; exact 1:2 degeneracy caustic, arbitrary-phase safe disk |z|<1/4, Q2-contract slice |z|<=1/8, unique phase-ray wall, and the formal W=e^{-y}F shape contract (r,C0,C1)=(pi/12,81,36); the canonical certificate directory is independently closed by its flat SHA256SUMS"
            ),
            ...
            "fixedMArbitraryPhaseWithinQ2Closed": True,
            "growingMClosed": False,
        ...
        "dataSummary": {
            ...
            "parameters": {
                "fixedMRequired": bool(parameters["fixedMRequired"]),
                "q2Maximum": float(parameters["q2Maximum"]),
                "q1Maximum": float(parameters["q1Maximum"]),
                "oneTwoCausticFreeRadius": float(parameters["oneTwoCausticFreeRadius"]),
                "oneTwoContractRadius": float(parameters["oneTwoContractRadius"]),
                "oneTwoWallMaximumRadius": float(parameters["oneTwoWallMaximumRadius"]),
                "criticalLocalization": float(parameters["criticalLocalization"]),
                "curvatureZone": float(parameters["curvatureZone"]),
                "curvatureMu": float(parameters["curvatureMu"]),
                "shapeC0": float(parameters["shapeC0"]),
                "shapeC1": float(parameters["shapeC1"]),
                "normalizedFShapeC1": float(parameters["normalizedFShapeC1"]),
                "normalizedFAwayGapLower": float(parameters["normalizedFAwayGapLower"]),
            },
        ...
        "qa": {
            ...
            "causticAndSafeDisksInspected": visual,
            "phaseRayWallInspected": visual,
