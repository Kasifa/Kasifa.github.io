  ...
  for (const item of cases) {
    ...
    const state = await page.evaluate(({ expectedLang, kind }) => {
      ...
      return {
        ...
        openBoundaryVisible: kind !== "note" || /S\.261/.test(body) && /S\.269/.test(body) && /S\.272/.test(body) && /OPEN|开放/.test(body),
        noGoBoundaryVisible: kind !== "note" || /no-go/i.test(body) && /exact.{0,24}famil|精确族/i.test(body) && /fixed positive|固定正/.test(body),
        formalFigureVisible: kind !== "note" || /上一阶段结构图|previous-stage structure figure/i.test(body),
