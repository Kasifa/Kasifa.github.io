...
test("publishes R0.71S as v1.04 with 143 notes, route 53, recap 83, 45 releases, and R0.71T next", async () => {
  ...
  for (const [page, minimum] of [
    ...
    assert.match(page, /src="\/i18n-en\.js\?v=1\.04"/);
  }
});
...
test("ships hash-identical R0.71S figure mirrors and 45 continuous completed releases", async () => {
  const [
    { home },
    svg,
    pdf,
