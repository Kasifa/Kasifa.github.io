...
1. Recompute `source-data.csv` from the exact Step 14 formulas.
...
5. Confirm that titles, collar geometry, formula labels, status boxes, and the explicit
   `ANALYTIC SCHEMATIC | NOT SIMULATION OR DNS` boundary are legible.
6. Confirm that no panel visually represents an abstract spike or tree as a
