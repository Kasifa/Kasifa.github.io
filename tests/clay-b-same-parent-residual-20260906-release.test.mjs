...
const protectedArtifacts = {
  "public/notes/clay-b-convex-pressure-trace-20260906.html": "ec4c6c61da3e808d103a19db853e6ec92c950c301504c8546a84cdea8f19a4dd",
...
test("the HTML-only SameParentResidual note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-same-parent-residual-20260906.html");
  for (const marker of [
    "ClayB-SameParentResidual-20260906", "CB.22", "目标点没有残差能量原子",
    "The residual energy has no atom at the target point", "正向残差方程保留已付源",
    "The forward residual equation retains its paid source", "完整周期混合压力获得普通时间 little-o",
    "Full-periodic mixed pressure has ordinary-time little-o", "CONDITIONAL", "DIRECT DERIVATION",
    "PAID SOURCE", "LITERATURE", "FINITE CHECKS ONLY", "OPEN", "NOT CLAY",
  ...
  assert.equal(note.includes("/notes/clay-b-same-parent-residual-20260906.pdf"), false);
  assert.equal(note.includes("R0.76M"), false);
});

test("homepage keeps the solid R0 sequence separate from the dashed CB.1–CB.22 lane", async () => {
  ...
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-same-parent-residual-20260906"));
  ...
  assert.equal((home.match(/id="clay-b-same-parent-residual"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-convex-pressure-trace"/g) ?? []).length, 0);
  assert.equal((literature.match(/id="clay-b-same-parent-residual-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-same-parent-residual-20260906"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-same-parent-residual-20260906.pdf"), false);
  assert.equal(site.version, "2.66");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 22);
  assert.equal(site.latestIndependentNote, "ClayB-SameParentResidual-20260906");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-same-parent-residual-20260906.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.latestIndependentChapter, "CB.22");
  assert.equal(site.nextIndependentChapter, "CB.23");
  assert.equal(manifest.siteVersion, "2.66");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestPublication.releaseId, "clay-b-same-parent-residual-20260906");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-ConvexPressureTrace-20260906");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.22");
});
...
test("prior note, canonical endpoint, recap, and historical PDF remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-same-parent-residual-20260906.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-same-parent-residual-20260906.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-same-parent-residual-20260906.pdf")));
});
