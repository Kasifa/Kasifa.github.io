...
const outputRoot = resolve(process.env.R075F_STEP31_BROWSER_QA_OUTPUT ?? "/tmp/r075f-step31-browser-qa");
const externalBase = process.env.R075F_PUBLIC_BASE_URL;
...
try {
  for (const scenario of scenarios) {
    ...
    const url = new URL("/notes/r0-75f.html", baseUrl);
    url.searchParams.set("lang", scenario.lang);
    url.searchParams.set("qa", "r075f-step31");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        figureSections: document.querySelectorAll('#figure, [src*="r075f"], [href*="assets/r075f"]').length,
        modalProductProved: /MODAL PRODUCT IDENTITY PROVED/.test(visibleText),
        offDiagonalReconstructed: /OFF-DIAGONAL LEDGER RECONSTRUCTED/.test(visibleText),
        phaseTautological: /PHASE SUBSTITUTION TAUTOLOGICAL/.test(visibleText),
        positivityFalse: /POSITIVITY-ONLY CONTROL FALSE/.test(visibleText),
        notE24Counterexample: /NOT AN E\.24 COUNTEREXAMPLE/.test(visibleText),
        dynamicRoutesViable: /DYNAMIC ROUTES REMAIN VIABLE/.test(visibleText),
    ...
    assert.equal(audit.figureSections, 0, `${scenario.id} no R0.75F formal figure`);
    assert.equal(audit.modalProductProved, true, `${scenario.id} modal-product boundary`);
    assert.equal(audit.offDiagonalReconstructed, true, `${scenario.id} off-diagonal boundary`);
    assert.equal(audit.phaseTautological, true, `${scenario.id} phase-substitution boundary`);
    assert.equal(audit.positivityFalse, true, `${scenario.id} positivity-only boundary`);
    assert.equal(audit.notE24Counterexample, true, `${scenario.id} E.24 counterexample boundary`);
    assert.equal(audit.dynamicRoutesViable, true, `${scenario.id} dynamic-route boundary`);
