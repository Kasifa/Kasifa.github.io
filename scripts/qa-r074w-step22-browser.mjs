...
const outputRoot = resolve(process.env.R074W_STEP22_BROWSER_QA_OUTPUT ?? "/tmp/r074w-step22-browser-qa");
...
try {
  ...
  for (const scenario of scenarios) {
    ...
    const response = await page.goto(
      `http://127.0.0.1:${address.port}/notes/r0-74w.html?lang=${scenario.lang}`,
    ...
    const audit = await page.evaluate((expectedFigureId) => {
      ...
      return {
        ...
        relativeProbabilityLabels: [...document.querySelectorAll(".label")].filter((node) => node.textContent.includes("RELATIVE PROBABILITY")).length,
        fixedDeletionOpen: /fixed deletion.{0,80}OPEN|fixed-deletion.{0,80}OPEN/i.test(visibleText),
    ...
    assert.match(audit.figureCurrentSource, /r074w\/fig-r074w-remote-adjacent-inward-threshold\.svg/);
    assert.equal(audit.relativeProbabilityLabels, 1, `${scenario.id} relative-probability prominence`);
    assert.equal(audit.fixedDeletionOpen, true, `${scenario.id} fixed-deletion boundary`);
    ...
    if (scenario.lang === "en") {
      ...
      assert.match(audit.h1, /Remote adjacent-inward common-shear threshold/);
      assert.equal(audit.switcher, "中文");
      assert.ok(audit.chinesePdfLabels >= 1, `${scenario.id} Chinese-PDF label`);
    } else {
      assert.match(audit.h1, /远端相邻内壳/);
