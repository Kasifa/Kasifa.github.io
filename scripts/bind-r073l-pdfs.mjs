...
const outputPath = resolve(root, "research/r073l_pdf_bindings.json");
...
const documents = [
  {
    ...
    title: "R0.73L｜Parameter-uniform nonselfadjoint adiabatic tracking",
  },
  {
    kind: "cumulative-recap",
    html: "public/recap-r0-61-r0-73l.html",
    pdf: "public/recap-r0-61-r0-73l.pdf",
    title: "R0.61–R0.73L｜R0.60 之后的研究回顾",
  },
];

const usage = "usage: bind-r073l-pdfs.mjs (--apply | --check-only)";
...
async function atomicWrite(path, payload) {
  const temporary = resolve(
    dirname(path),
    `.${path.split("/").at(-1)}.r073l-${process.pid}-${Date.now()}-${process.hrtime.bigint()}.tmp`,
  );
  try {
...
const manifest = {
  schemaVersion: "r073l-synchronized-pdf-bindings-v1",
  release: "R0.73L",
  renderer: {
...
if (apply) {
  ...
  const current = await regularBytes("research/r073l_pdf_bindings.json");
  if (!current.equals(payload)) throw new Error("R0.73L PDF binding manifest is stale");
}
...
console.log(JSON.stringify({
  ...
  output: "research/r073l_pdf_bindings.json",
}));
