...
const outputRoot = resolve(process.env.R074V_STEP21_BROWSER_QA_OUTPUT ?? "/tmp/r074v-step21-browser-qa");
...
try {
  ...
  for (const scenario of scenarios) {
    ...
    const response = await page.goto(
      `http://127.0.0.1:${address.port}/notes/r0-74v.html?lang=${scenario.lang}`,
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        formalFigureSections: document.querySelectorAll('#figure, [src*="r074v"], [href*="assets/r074v"]').length,
        routeMemoLabels: [...document.querySelectorAll(".label")].filter((node) => node.textContent.includes("ROUTE MEMO")).length,
    ...
    assert.equal(audit.imageCount, 0, `${scenario.id} route-only note has no figure images`);
    assert.equal(audit.formalFigureSections, 0, `${scenario.id} no R0.74V formal figure`);
    assert.equal(audit.routeMemoLabels, 1, `${scenario.id} route memo prominence`);
