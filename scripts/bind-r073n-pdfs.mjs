...
// Cryptographically bind the two synchronized R0.73N PDFs to their HTML
...
const usage = "usage: bind-r073n-pdfs.mjs (--apply | --check-only)";
...
const outputPath = resolve(root, "research/r073n_pdf_bindings.json");
...
function canonicalTitles(dictionary) {
  if (/[\x00-\x08\x0b\x0c\x0e-\x1f\x7f]/.test(dictionary)) {
    throw new Error("R0.73N bilingual dictionary contains control characters");
  }
  const releaseTitle = dictionary.match(/^\*\*Release title:\*\*\s*\*?(.+?)\*?\s*$/m)?.[1]
    ?.replace(/^\*|\*$/g, "").trim();
  const synchronized = dictionary.match(
    /^## 8\. Synchronized title\s*\n\s*```text\s*\n([^\n]+)\n([^\n]+)\n```/m,
  );
  if (!synchronized) throw new Error("R0.73N synchronized-title ledger missing");
  if (synchronized[1].trim() !== releaseTitle) {
    throw new Error("R0.73N release-title ledgers disagree");
  }
  for (const token of [
  ...
  return {
    ...
    recap: "R0.61–R0.73N｜R0.60 之后的研究回顾",
  };
}
...
if (
  ...
  throw new Error("R0.73N HTML/accounting apply must precede PDF binding");
}
const documents = [
  {
    kind: "research-note",
    html: "public/notes/r0-73n.html",
    pdf: "public/notes/r0-73n.pdf",
    title: titles.note,
  },
  {
    kind: "cumulative-recap",
    html: "public/recap-r0-61-r0-73n.html",
    pdf: "public/recap-r0-61-r0-73n.pdf",
    title: titles.recap,
  },
];
...
const manifest = {
  schemaVersion: "r073n-synchronized-pdf-bindings-v1",
  release: "R0.73N",
  canonicalTitleSource: {
    path: "research/r073n_bilingual_dictionary.md",
...
if (apply) {
  ...
  if (!current.equals(payload)) throw new Error("R0.73N PDF binding manifest is stale");
}
...
console.log(JSON.stringify({
  ...
  output: "research/r073n_pdf_bindings.json",
}));
