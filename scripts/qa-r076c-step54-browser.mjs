...
const outputRoot = resolve(process.env.R076C_STEP54_BROWSER_QA_OUTPUT ?? "/tmp/r076c-step54-browser-qa");
const externalBase = process.env.R076C_PUBLIC_BASE_URL;
...
const target = {
  id: "note",
  path: "/notes/r0-76c.html",
  h1Zh: /固定有限倍频带剪切的全频通量支付/,
  h1En: /Full-frequency flux payment for fixed finite dyadic-band shears/i,
};
const results = [];
...
try {
  for (const scenario of scenarios) {
    ...
    url.searchParams.set("qa", "r076c-step54");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        step54MathJaxErrors: document.querySelectorAll("#s-425 mjx-merror, #s-426 mjx-merror, #s-427 mjx-merror, #s-428 mjx-merror, #s-429 mjx-merror, #s-430 mjx-merror, #s-431 mjx-merror, #s-432 mjx-merror").length,
        ...
        figureSections: document.querySelectorAll('#figure, [src*="assets/r076c"], [href*="assets/r076c"]').length,
        ...
        allCarriers: /ALL CARRIERS|all carriers|全部 carrier|no carrier upper bound/i.test(visibleText),
        inverseRadiusBranch: /N_1 R <= 1 · B|n_1 R <= 1|inverse-radius|逆半径/i.test(visibleText),
        ultraHighBranch: /N_1 R > 1 · C|n_1 R > 1|ultra-high heat-clock|超高热时钟/i.test(visibleText),
        pointwiseFamily: /C\.14 POINTWISE FAMILY|pointwise exponential-polynomial|逐点满足 C\.12/i.test(visibleText),
        ...
        selfCross: /ALL SELF \/ CROSS TERMS|self[^\n]{0,80}cross term|self\/cross terms/i.test(visibleText),
    ...
    assert.equal(audit.step54MathJaxErrors, 0, `${id} Step 54 MathJax errors`);
    ...
    assert.equal(audit.figureSections, 0, `${id} no R0.76C formal figure`);
    for (const key of ["fixedQ", "integerModes", "realPhases", "allCarriers", "inverseRadiusBranch", "ultraHighBranch", "pointwiseFamily", "weightedPower", "terminalPower", "completeSquare", "selfCross", "growingQOpen", "versionM", "noveltyBoundary", "clayBoundary"])
