...
EXPECTED_MAIN_HASH=20f32790b53f2b0f5cb39b7071bd2cda96ddb4e15f75211e1682f4ba37dd0bb2
EXPECTED_PRIMARY_HASH=af47153c4e1f4c5749f68c3f89d7533c5d95f3c0c6f15b0c775a9e35317c807e
EXPECTED_SOURCE_HASH=3aea1d04dce4987c3883c1b93bec04e714ee17b540fb6a99546d084efa326f74
EXPECTED_FIXTURES_HASH=32e1dcf71a77ba0d28e3924fcb7e7aeb4d2840aa08ba2b2e352bb4d20d0464af
EXPECTED_EXPECTED_HASH=0a2d3d086381029941310ae502b4cf9462e025d0c75e62dd87c07334728a6ba8
EXPECTED_PY_HASH=0afbee1f11de12cefc85aee64cbdb8c92925ad2db33cdae8d0582b79dbc01f85
EXPECTED_RUBY_HASH=ea5036ffed18ce5d1ff33addeff6086ab3603bcedf2373ca6dec7ca3e4963fa2
EXPECTED_CERT_HASH=dcca5611f40b5de9cfcc76fccc3ed35a0219a8baedbb488574223809686c652d
EXPECTED_REPORT_HASH=f77d2e636e65ff07f662adc72fa16f13ab4edb57addf8422536fa67a0b36660c
EXPECTED_RUBY_REPORT_HASH=c034a9d3f01e784733fd35052ec4b9574c9ee4596ad44e466d07a78773953a68

TMP_ROOT=$(mktemp -d /tmp/r076g-certificate-qa.XXXXXX)
...
test "$(digest "$ROOT/research/r075b_bulk_clock_outer_padding_gate.md")" = 430feb9efc151e2b968dd1b7f785a19dc0e38416270ff8ed0275cfc6429b1a5a
...
test "$(digest "$ROOT/research/r076f_exponential_spatial_observation_lower_bound.md")" = 48204fcbf8fe9af3f0fdc7720844c3dd8362d8767caf73de016eda7250b70973

for seed in 0 1 42; do
  env PYTHONHASHSEED="$seed" R076G_JSON="$TMP_ROOT/certificate-$seed.json" \
    R076G_REPORT="$TMP_ROOT/report-$seed.md" python3 -Werror -B "$PY_SCRIPT" \
...
env R076G_JSON="$CERT" ruby "$RUBY_SCRIPT" >"$TMP_ROOT/ruby-canonical.stdout"
...
for mutation in $mutations; do
  if env PYTHONHASHSEED=0 R076G_MUTATION="$mutation" \
    R076G_JSON="$TMP_ROOT/python-$mutation.json" R076G_REPORT="$TMP_ROOT/python-$mutation.md" \
  ...
  if env R076G_JSON="$CERT" R076G_RUBY_MUTATION="$mutation" \
    R076G_RUBY_REPORT="$TMP_ROOT/ruby-$mutation.md" ruby "$RUBY_SCRIPT" \
...
if env R076G_MUTATION=unknown_mutation R076G_JSON="$TMP_ROOT/unknown-python.json" \
  R076G_REPORT="$TMP_ROOT/unknown-python.md" python3 -Werror -B "$PY_SCRIPT" \
  >"$TMP_ROOT/unknown-python.stdout" 2>"$TMP_ROOT/unknown-python.stderr"; then
  printf '%s\n' 'Python accepted an unknown mutation' >&2
  exit 1
fi
grep -q 'unknown R076G_MUTATION' "$TMP_ROOT/unknown-python.stderr"

if env R076G_JSON="$CERT" R076G_RUBY_MUTATION=unknown_mutation \
  R076G_RUBY_REPORT="$TMP_ROOT/unknown-ruby.md" ruby "$RUBY_SCRIPT" \
  >"$TMP_ROOT/unknown-ruby.stdout" 2>"$TMP_ROOT/unknown-ruby.stderr"; then
  printf '%s\n' 'Ruby accepted an unknown mutation' >&2
  exit 1
fi
grep -q 'unknown R076G_RUBY_MUTATION' "$TMP_ROOT/unknown-ruby.stderr"
...
tags = [int(value) for value in re.findall(r"\\tag\{G\.(\d+)\}", text)]
if tags != list(range(1, 41)):
    raise SystemExit(f"bad G tag sequence: {tags}")
if len(re.findall(r"(?m)^\\\[$", text)) != 40 or len(re.findall(r"(?m)^\\\]$", text)) != 40:
    raise SystemExit("bad display inventory")
for required in (
    r"q=2m+1", r"2m,2m+1,\ldots,4m", r"v=\frac{BR}{a}=-\beta",
    r"\left(\frac97\right)^{4m}", r"\frac{q(L)}{L^2}\longrightarrow\frac2{3969}",
    r"\frac{4\log(9/7)}{3969}-\frac2{11907}>0",
    "not the full physical plateau mass", "**NOT CLAY.**",
):
...
if "\\delta_c" in text or "delta_c" in paths[1].read_text(encoding="utf-8"):
    raise SystemExit("undefined delta_c")
...
env R076G_JSON="$CERT" ruby "$RUBY_SCRIPT" >"$TMP_ROOT/ruby-regenerate.stdout"
...
core_count=$(find "$ROOT/research" "$ROOT/scripts" -maxdepth 1 -type f \
  \( -name 'r076g_*' -o -name 'r076g-*' \) \
  ! -name 'r076g_publication_handoff*' | wc -l | tr -d ' ')
...
printf '{"suite":"r076g-complete-clock-central-fibre-flux-lower-bound","status":"PASS","assertions":%s,"mutations":%s,"pythonHashSeeds":3,"coreFiles":12}\n' "$assertions" "$mutation_total"
