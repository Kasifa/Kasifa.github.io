...
const outputRoot = resolve(process.env.R074U_STEP20_BROWSER_QA_OUTPUT ?? "/tmp/r074u-step20-browser-qa");
...
try {
  ...
  for (const scenario of scenarios) {
    ...
    const response = await page.goto(
      `http://127.0.0.1:${address.port}/notes/r0-74u.html?lang=${scenario.lang}`,
    ...
    const audit = await page.evaluate(() => {
      ...
      const figure = document.querySelector('#figure img[src*="intrinsic-certified-residence"]');
