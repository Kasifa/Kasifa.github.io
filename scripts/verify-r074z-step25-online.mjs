...
const baseUrl = new URL(process.env.R074Z_PUBLIC_BASE_URL ?? "https://kasifa.github.io/");
const commit = process.env.R074Z_DEPLOYED_COMMIT ?? "unversioned";
const figureId = "fig-r074z-remote-persistence-gate";
const publicFigureRoot = `public/figures/r074z/${figureId}`;
const sha256 = (bytes) => createHash("sha256").update(bytes).digest("hex");

const fixedObjects = [
  ["public/notes/r0-74z.html", "/notes/r0-74z.html"],
  ["public/notes/r0-74z.pdf", "/notes/r0-74z.pdf"],
  ...
  [`public/assets/r074z/${figureId}.pdf`, `/assets/r074z/${figureId}.pdf`],
  [`public/assets/r074z/${figureId}.png`, `/assets/r074z/${figureId}.png`],
  [`public/assets/r074z/${figureId}.svg`, `/assets/r074z/${figureId}.svg`],
];

const treeish = process.env.R074Z_VERIFY_TREEISH ?? "HEAD";
const figureNames = execFileSync(
  "git",
  ["ls-tree", "-r", "--name-only", treeish, `research/figures/r074z/${figureId}`],
...
const objects = [
  ...
  ...figureNames.map((name) => [
    ...
    `/figures/r074z/${figureId}/${encodeURIComponent(name)}`,
  ]),
];
assert.equal(objects.length, 37, "Step 25 online publication inventory");
