...
const protectedArtifacts = {
  "public/notes/clay-b-same-parent-residual-20260906.html": "0876091a050a78ba54ceff8943f86b4d1e1fc4e01458560a8ecc69a7c0ef0055",
...
const independentChapters = [
  ...
  ["CB.19", "clay-b-common-adjoint-screen-20260906"], ["CB.20", "clay-b-adjoint-weak-trace-screen-20260906"],
  ["CB.21", "clay-b-convex-pressure-trace-20260906"], ["CB.22", "clay-b-same-parent-residual-20260906"],
  ["CB.23", "clay-b-signed-mixed-pressure-20260907"],
];

test("the HTML-only SignedMixedPressure note is bilingual, bounded, and figure-free", async () => {
  const note = await text("public/notes/clay-b-signed-mixed-pressure-20260907.html");
  for (const marker of [
    "ClayB-SignedMixedPressure-20260907", "CB.23", "不显含幅度的投影上界",
    "An amplitude-independent projected bound", "两个不同且未付的充分接口",
    "Two distinct and unpaid sufficient interfaces", "联合压力原函数只按有符号累计消失",
    "Only the signed primitive of joint pressure vanishes", "PROVED", "CONDITIONAL",
    "DIRECT DERIVATION", "LITERATURE", "FINITE", "FINITE CHECKS ONLY", "OPEN", "NOT CLAY",
  ...
  assert.equal(note.includes("/notes/clay-b-signed-mixed-pressure-20260907.pdf"), false);
  assert.equal(note.includes("R0.76M"), false);
});

test("homepage keeps the solid R0 sequence separate from the dashed CB.1–CB.23 lane", async () => {
  ...
  for (const page of [home, literature, index]) assert.ok(page.includes("clay-b-signed-mixed-pressure-20260907"));
  ...
  const positions = independentChapters.map(([chapter, slug]) => {
    const rowClass = slug.replace(/-2026090[567]$/, "") + "-row";
    const row = home.indexOf(`class="tree-row ${rowClass}"`, clayStart);
    assert.ok(row >= clayStart && row < clayBoundary, `${chapter} row position`);
    assert.ok(home.slice(row, row + 3000).includes(chapter), `${chapter} route label`);
    return row;
  });
  assert.deepEqual(positions, [...positions].sort((a, b) => a - b));
  assert.equal((home.match(/class="route-overview independent-release-spotlight"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-signed-mixed-pressure"/g) ?? []).length, 1);
  assert.equal((home.match(/id="clay-b-same-parent-residual"/g) ?? []).length, 0);
  assert.equal((literature.match(/id="clay-b-signed-mixed-pressure-boundary"/g) ?? []).length, 1);
  assert.equal((index.match(/data-note="clay-b-signed-mixed-pressure-20260907"/g) ?? []).length, 1);
  ...
  assert.equal(noteFiles.includes("clay-b-signed-mixed-pressure-20260907.pdf"), false);
  assert.equal(site.version, "2.67");
  assert.equal(site.latestRelease, "R0.76L");
  assert.equal(site.publicIndependentNoteCount, 23);
  assert.equal(site.latestIndependentNote, "ClayB-SignedMixedPressure-20260907");
  assert.equal(site.latestIndependentResearchHtml, "/notes/clay-b-signed-mixed-pressure-20260907.html");
  assert.equal(site.latestIndependentResearchPdf, null);
  assert.equal(site.latestIndependentChapter, "CB.23");
  assert.equal(site.nextIndependentChapter, "CB.24");
  assert.equal(manifest.siteVersion, "2.67");
  assert.equal(manifest.latestCompletedRelease, "r076l");
  assert.equal(manifest.latestPublication.releaseId, "clay-b-signed-mixed-pressure-20260907");
  assert.equal(manifest.latestPublication.logicalPredecessor, "ClayB-SameParentResidual-20260906");
  ...
  assert.equal(manifest.latestPublication.chapter, "CB.23");
});
...
test("prior note, canonical endpoint, recap, and historical PDF remain byte-exact", async () => {
  ...
  await assert.rejects(access(resolve(root, "public/notes/clay-b-signed-mixed-pressure-20260907.pdf")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-signed-mixed-pressure-20260907.html")));
  await assert.rejects(access(resolve(root, "public/recap-clay-b-signed-mixed-pressure-20260907.pdf")));
});
