...
const outputRoot = resolve(process.env.R075T_STEP45_BROWSER_QA_OUTPUT ?? "/tmp/r075t-step45-browser-qa");
const externalBase = process.env.R075T_PUBLIC_BASE_URL;
...
try {
  for (const scenario of scenarios) {
    ...
    const url = new URL("/notes/r0-75t.html", baseUrl);
    url.searchParams.set("lang", scenario.lang);
    url.searchParams.set("qa", "r075t-step45");
    ...
    const audit = await page.evaluate(() => {
      ...
      return {
        ...
        step45MathJaxErrors: document.querySelectorAll("#s-351 mjx-merror, #s-352 mjx-merror, #s-353 mjx-merror, #s-354 mjx-merror, #s-355 mjx-merror, #s-356 mjx-merror, #s-357 mjx-merror").length,
        ...
        figureSections: document.querySelectorAll('#figure, [src*="r075t"], [href*="assets/r075t"]').length,
        spatialTheorem: /SPATIAL THEOREM/.test(visibleText),
        ...
        exactPlateauFibre: /EXACT PLATEAU FIBRE/.test(visibleText),
        slowEnvelopeCoercivity: /SLOW-ENVELOPE COERCIVITY/.test(visibleText),
        beatDefect: /BEAT DEFECT/.test(visibleText),
        sharpDegeneracy: /SHARP DEGENERACY/.test(visibleText),
        unequalHeatRates: /UNEQUAL HEAT RATES/.test(visibleText),
        fourFrequencyFlux: /FOUR-FREQUENCY FLUX/.test(visibleText),
        temporalPaymentOpen: /TEMPORAL PAYMENT OPEN/.test(visibleText),
    ...
    assert.equal(audit.step45MathJaxErrors, 0, `${scenario.id} Step 45 MathJax errors: ${JSON.stringify(audit.mathJaxErrorContexts.filter((entry) => /^s-35[1-7]$/.test(entry.section ?? "")))}`);
    ...
    assert.equal(audit.figureSections, 0, `${scenario.id} no R0.75T formal figure`);
    for (const key of [
      "spatialTheorem", "exactlyTwoHarmonics", "oneDyadicPair", "highCarrier", "exactPlateauFibre",
      "slowEnvelopeCoercivity", "beatDefect", "sharpDegeneracy", "unequalHeatRates", "fourFrequencyFlux", "temporalPaymentOpen",
    ...
    if (scenario.lang === "en") {
      ...
      assert.match(audit.h1, /Spatial collar coercivity for one two-harmonic dyadic pair/);
      assert.equal(audit.switcher, "中文");
      assert.ok(audit.chinesePdfLabels >= 1, `${scenario.id} Chinese-PDF label`);
    } else {
      assert.match(audit.h1, /单个二谐波 dyadic pair 的空间 collar coercivity/);
