...
test("publishes R0.71J as v0.95 with 134 notes and 74 recap nodes", async () => {
  const [{ home, note, recap, literature }, noteNames] = await Promise.all([
    publishedPages(),
    readdir(notesRoot),
  ]);

  assert.equal(noteNames.filter((name) => name.endsWith(".html")).length, 134);
  assert.match(home, /<strong>v0\.95<\/strong>网页版本/);
  assert.match(home, /<strong>134<\/strong>公开研究笔记/);
  assert.match(home, /<strong>R0\.71J<\/strong>最新研究节点/);
  assert.match(home, /展开 44 篇公开笔记/);
  assert.match(home, /累计回顾收录 74 个节点；全站现有 134 篇公开研究笔记/);
  ...
  assert.match(literature, /R0\.69P–R0\.71J/);
...
test("states the exact defect, broad-parent witness, and quantified full-frame gap", async () => {
  ...
  assert.match(literature, /ordinary full-frequency-frame summation/);
});
...
test("ships synchronized PDFs and three journal figure formats", async () => {
  ...
  assert.match(home, /href="\/recap-r0-61-r0-71j\.pdf"/);
