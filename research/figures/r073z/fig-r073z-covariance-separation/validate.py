#!/usr/bin/env python3
"""Fail-closed validator and metadata writer for Figure R0.73Z-1."""
...
def verify_source_binding(config: dict[str, Any]) -> dict[str, Any]:
    ...
    require(certificate.get("status") == "PASS", "covariance certificate status is not PASS")
    require(
        certificate.get("payload_sha256") == binding["certificatePayloadSha256"],
        "covariance certificate payload hash drift",
    )
    claim = certificate.get("claim_boundary", {})
    require(claim.get("notClay") is True and claim.get("clayProblemSolved") is False, "certificate Clay boundary drift")
    require(claim.get("interiorSuitableWeakFiniteness") == "OPEN", "interior finiteness qualifier drift")
    return {"commit": head, "files": observed, "certificatePayloadSha256": certificate["payload_sha256"]}
...
def load_rows() -> list[dict[str, str]]:
    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        ...
        rows = list(reader)
    require(len(rows) == 201, f"CSV row count {len(rows)} != 201")
    return rows
...
def verify_formulas(rows: list[dict[str, str]], config: dict[str, Any]) -> dict[str, Any]:
    ...
    require([len(by_panel[p]) for p in ("A", "B", "C")] == [64, 16, 121], "panel grain mismatch")

    for index, row in enumerate(by_panel["A"], start=1):
        require(int(row["sample_index"]) == index, f"A sample index drift at {index}")
        require(int(row["n"]) == index, f"A frequency drift at {index}")
        close(float(row["energy_over_pi_cubed"]), 6.0, f"A energy row {index}")
        close(float(row["D32_lower_bound_frequency_factor"]), float(index), f"A factor row {index}")
        require("c suppressed" in row["normalization"], f"A suppressed-c qualifier missing at row {index}")
        require("NOT CLAY" in row["claim_qualifier"], f"A NOT CLAY qualifier missing at row {index}")
    ...
    for index, row in enumerate(by_panel["B"], start=1):
        ...
        expected = Fraction(1, 3) * (1 - Fraction(1, 4**index))
        stored_exact = Fraction(row["l2_partial_sum_exact"])
        require(stored_exact == expected, f"B exact fraction drift at J={index}")
        close(float(row["l2_partial_sum"]), float(expected), f"B decimal row {index}")
        require(row["l2_limit_exact"] == "1/3", f"B exact limit drift at J={index}")
        require(Fraction(row["lacunary_unit_term"]) == 1, f"B unit term drift at J={index}")
        require(int(row["lacunary_partial_lower_sum"]) == index, f"B partial lower sum drift at J={index}")
        require("finite display is not the divergence proof" in row["claim_qualifier"], f"B proof boundary missing at J={index}")

    c_min = float(config["panelC"]["heatScaleMinimum"])
    c_max = float(config["panelC"]["heatScaleMaximum"])
    point_count = int(config["panelC"]["points"])
    ratio = c_max / c_min
    c_maxima = {"D_s": 0.0, "Q_s_magnitude": 0.0, "div_Q_s": 0.0}
    for zero_index, row in enumerate(by_panel["C"]):
        index = zero_index + 1
        require(int(row["sample_index"]) == index, f"C index drift at {index}")
        expected_s = c_min * ratio ** (zero_index / (point_count - 1))
        stored_s = float(row["s"])
        close(stored_s, expected_s, f"C s row {index}")
        expected_r = math.exp(-expected_s)
        close(float(row["r_exp_minus_s"]), expected_r, f"C r row {index}")
        expected_d = (1.0 - expected_r**2) * (1.0 + 0.5 * expected_r**2)
        base = expected_r**3 - expected_r**5
        expected_q = math.sqrt(6.0) * base / 8.0
        expected_div = 3.0 * base / 4.0
        values = {
            "D_s": float(row["D_s"]),
            "Q_s_magnitude": float(row["Q_s_magnitude"]),
            "div_Q_s": float(row["div_Q_s"]),
        }
        close(values["D_s"], expected_d, f"C D row {index}")
        close(values["Q_s_magnitude"], expected_q, f"C |Q| row {index}")
        close(values["div_Q_s"], expected_div, f"C divQ row {index}")
        require(float(row["Pi_s"]) == 0.0 and float(row["centered_S_s"]) == 0.0, f"C production not zero at row {index}")
        require(all(value > 0.0 for value in values.values()), f"C positivity failed at row {index}")
        require("not finite sampling" in row["claim_qualifier"], f"C sampling boundary missing at row {index}")
        for key, value in values.items():
            c_maxima[key] = max(c_maxima[key], value)

    require(float(by_panel["B"][-1]["l2_partial_sum"]) < 1.0 / 3.0, "B partial sum must remain below 1/3")
    require(int(by_panel["B"][-1]["lacunary_partial_lower_sum"]) == 16, "B displayed lower sum maximum mismatch")
    return {
        "rowCount": len(rows),
...
def verify_claim_boundary(contract: dict[str, Any], *, include_manifest: bool) -> None:
    ...
    require(claim["interiorSuitableWeakFiniteness"] == "OPEN", "OPEN qualifier drift")
    ...
    for name in ("README.md", "caption.md"):
        require("NOT CLAY" in (PACKAGE / name).read_text(encoding="utf-8"), f"NOT CLAY missing from {name}")
    if include_manifest:
...
def write_baseline(path: Path, hashes: dict[str, str]) -> None:
    payload = {"schema": "r073z-determinism-baseline-v1", "fileCount": len(hashes), "sha256": hashes}
...
def compare_baseline(path: Path, hashes: dict[str, str]) -> dict[str, Any]:
    ...
    require(payload.get("schema") == "r073z-determinism-baseline-v1", "determinism baseline schema drift")
...
def write_metadata(
    ...
    *,
    ...
    validation = {
        ...
        "rasterAudit": raster,
        "schema": "r073z-figure-validation-v1",
    ...
    qa_lines = [
        "# R0.73Z figure QA report",
        "",
        "Status: **PASS**",
        "",
        "- Source binding: PASS (commit, nine file hashes, certificate PASS/payload).",
        "- Formula reconstruction: PASS (all 201 rows; tolerance 5e-13).",
        "- Panel A: PASS (energy exactly 6; displayed factor exactly n; c not fabricated).",
        "- Panel B: PASS (exact rational sums; unit term 1; partial lower sum J).",
        "- Panel C: PASS (Pi=S=0; D, |Q|, div Q positive on displayed finite scales and formula-exact).",
        ...
        "",
        "The images visualize normalized analytic consequences. Finite data are not used as proof of any quantified statement. **NOT CLAY.**",
        "",
    ]
    ...
    manifest = {
        ...
        "compute": {
            ...
            "workload": "201 closed-form rows and vector/raster export; no DNS",
        },
        ...
        "sourceData": {
            ...
            "warning": "normalized analytic consequences; finite rows do not prove quantifiers",
        },
...
def main() -> int:
    ...
    if args.verify_only:
        ...
        print(
            "PASS: verified source binding, 201 formulas, vector exports, raster QA, "
