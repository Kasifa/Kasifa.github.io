...
// Render and cryptographically bind the synchronized R0.74A note PDF. This
...
if (!["--apply", "--check-only"].includes(mode)) {
  throw new Error("usage: bind-r074a-pdfs.mjs [--apply|--check-only]");
}
const apply = mode === "--apply";
const htmlPath = resolve(root, "public/notes/r0-74a.html");
const pdfPath = resolve(root, "public/notes/r0-74a.pdf");
const provenancePath = resolve(root, "research/r074a_note_pdf_render.json");
...
const title = "R0.74A｜混合 heat covariance 的局部 size lemma";
const recap = new Map([
...
async function renderPdf() {
  ...
  try {
    ...
    const url = `http://127.0.0.1:${address.port}/notes/r0-74a.html?lang=zh`;
    await run(process.execPath, [
      "scripts/render-note-pdf.mjs", url, "public/notes/r0-74a.pdf", "-",
      "public/notes/r0-74a.html", "research/r074a_note_pdf_render.json",
    ], {
      env: {
...
async function validateAndBind(writeBinding) {
  ...
  const htmlRecord = { path: "public/notes/r0-74a.html", bytes: html.length, sha256: sha256(html) };
  const pdfRecord = {
    path: "public/notes/r0-74a.pdf", bytes: pdf.length, sha256: sha256(pdf),
  ...
  const binding = {
    ...
    documents: [{
      ...
      provenance: {
        path: "research/r074a_note_pdf_render.json", bytes: provenanceBytes.length,
  ...
  if (writeBinding) await atomicJson(bindingPath, binding);
    ...
    if (JSON.stringify(current) !== JSON.stringify(binding)) {
      throw new Error("R0.74A PDF binding manifest is stale");
    }
  }
...
if (apply) {
  ...
  // The rebuilt index can reactivate an earlier count-specific translation.
  await run(process.execPath, [i18nBuilder, "translations/en.json"]);
  await validateAndBind(true);
}
const result = await validateAndBind(false);
console.log(JSON.stringify({
  applied: apply, checked: true, documents: 1,
  output: "research/r074a_pdf_bindings.json",
