...
// R0.73N translation stage.  Full-sentence translations come only from the
...
const usage = "usage: add-r073n-translations.mjs (--apply | --check-only | --capture-missing)";
...
const root = resolve(process.env.R073N_RELEASE_ROOT ?? resolve(import.meta.dirname, ".."));
const publicDirectory = resolve(root, "public");
const translationPath = resolve(root, "translations/en.json");
const bundlePath = resolve(publicDirectory, "i18n-en.js");
const snapshotPath = resolve(root, "scripts/i18n-snapshots/r073n-missing.json");
const dictionaryPath = resolve(root, "research/r073n_bilingual_dictionary.md");
const action = argumentsList[0];
const activePages = [
  "literature-review.html",
  "notes/index.html",
  "notes/r0-73n.html",
  "recap-r0-61-r0-73n.html",
...
function assertInsideRoot(path, label) {
  ...
  if (!offset || offset === ".." || offset.startsWith("../") || offset.startsWith("..\\")) {
    throw new Error(label + " escaped R073N_RELEASE_ROOT");
  }
}
...
async function safeAtomicWrite(path, payload) {
  ...
  const temporary = resolve(
    dirname(path),
    `.${path.split("/").at(-1)}.r073n-${process.pid}-${Date.now()}-${process.hrtime.bigint()}.tmp`,
  );
  try {
...
async function writeTransaction(writes) {
  if (writes.length !== 3 || new Set(writes.map(({ path }) => path)).size !== 3) {
    throw new Error("R0.73N translation transaction requires three distinct files");
  }
  const nonce = `${process.pid}-${Date.now()}-${process.hrtime.bigint()}`;
  const rows = writes.map(({ path, payload }, index) => ({
    path,
    payload,
    temporary: resolve(dirname(path), `.${path.split("/").at(-1)}.r073n-${nonce}-${index}.tmp`),
    backup: resolve(dirname(path), `.${path.split("/").at(-1)}.r073n-${nonce}-${index}.bak`),
    backedUp: false,
    installed: false,
  }));
  try {
...
const dictionary = await regularText(dictionaryPath, "R0.73N bilingual dictionary");
const releaseTitle = dictionary.match(/^\*\*Release title:\*\*\s*\*?(.+?)\*?\s*$/m)?.[1]?.replace(/^\*|\*$/g, "").trim();
if (releaseTitle !== "R0.73N | Fixed-member finite-strain stability and the family-transfer obstruction") {
  throw new Error("R0.73N bilingual dictionary release title drift");
}
for (const token of requiredDictionaryTokens) {
  if (!dictionary.includes(token)) throw new Error("R0.73N dictionary missing " + token);
}
if (dictionary.includes("finiteDiagnosticValidation=VALIDATED_PRESEAL") ||
    /\bpublicRelease=/.test(dictionary)) {
  throw new Error("R0.73N dictionary still carries prepublication provenance");
}
if (!/^\*\*Next release:\*\*\s*R0\.73O\s*$/m.test(dictionary)) {
  throw new Error("R0.73N dictionary next-release gate is not frozen to R0.73O");
}
if (!/bounded/i.test(dictionary) || !/priority/i.test(dictionary)) {
  throw new Error("R0.73N dictionary lost bounded-search/non-priority boundary");
}
...
if (
  releaseManifest.latestCompletedRelease !== "r073n" ||
  releaseManifest.siteVersion !== "1.54" ||
  releaseManifest.nextRelease !== "r073o" ||
  siteVersion.latestRelease !== "R0.73N" ||
  siteVersion.version !== "1.54"
) {
  throw new Error("R0.73N HTML/accounting apply must precede translation work");
}

for (const relativePage of activePages) {
  const html = await regularText(resolve(publicDirectory, relativePage), relativePage);
  if (!html.includes("/i18n-en.js?v=1.54")) {
    throw new Error(relativePage + ": expected i18n cache version v1.54");
  }
  if (!html.includes("R0.73N")) throw new Error(relativePage + ": R0.73N marker absent");
...
const retained = translations.filter((entry) => !/^r073n\d+$/.test(entry.id));
if (new Set(retained.map((entry) => entry.zh)).size !== retained.length) {
  throw new Error("duplicate Chinese key outside R0.73N translation batch");
}
...
if (JSON.stringify(missingFiles) !== JSON.stringify(activePages)) {
  throw new Error("unexpected R0.73N missing-string files: " + JSON.stringify(missingFiles));
}
...
if (action === "--capture-missing") {
  ...
  console.log(JSON.stringify({ captured: skeleton.length, snapshot: "scripts/i18n-snapshots/r073n-missing.json" }));
  process.exit(0);
}

const snapshot = JSON.parse(await regularText(snapshotPath, "R0.73N translation snapshot"));
if (!Array.isArray(snapshot) || snapshot.length === 0) throw new Error("empty R0.73N snapshot");
if (JSON.stringify(snapshot.map(({ zh }) => zh)) !== JSON.stringify(missing.map(({ zh }) => zh))) {
  throw new Error("R0.73N missing-string snapshot drift");
}
const englishByChinese = new Map(snapshot.map(({ zh, en }) => [zh, en]));
if (englishByChinese.size !== snapshot.length) throw new Error("duplicate R0.73N snapshot key");
...
const translated = missing.map((entry, index) => {
  ...
  if (!en || containsChinese(en)) throw new Error("invalid R0.73N English row: " + entry.zh);
  if (/\b(?:we|our|ours|ourselves|us)\b/i.test(en)) {
    throw new Error("collective English voice in R0.73N row: " + entry.zh);
  }
  if (JSON.stringify(extractProtectedTokens(en)) !== JSON.stringify(extractProtectedTokens(entry.zh))) {
    throw new Error("protected-token mismatch for R0.73N row: " + entry.zh);
  }
  if (JSON.stringify(accountingTokens(en)) !== JSON.stringify(accountingTokens(entry.zh))) {
    throw new Error("version/count-token mismatch for R0.73N row: " + entry.zh);
  }
  if (JSON.stringify(boundaryTokens(en)) !== JSON.stringify(boundaryTokens(entry.zh))) {
    throw new Error("claim-state boundary drift for R0.73N row: " + entry.zh);
  }
  if (JSON.stringify(machineTokens(en)) !== JSON.stringify(machineTokens(entry.zh))) {
    throw new Error("machine-ledger drift for R0.73N row: " + entry.zh);
  }
  return { ...entry, id: "r073n" + String(index + 1).padStart(3, "0"), en };
});
...
if (action === "--check-only") {
  if (await regularText(snapshotPath, "R0.73N translation snapshot") !== snapshotOutput) {
    throw new Error("R0.73N translation snapshot is stale");
  }
  if (JSON.stringify(translations) !== JSON.stringify(finalTranslations)) {
    throw new Error("R0.73N translations are stale");
  }
  if (await regularText(bundlePath, "public/i18n-en.js") !== bundleOutput) {
    throw new Error("R0.73N translation bundle is stale");
  }
} else {
...
console.log(JSON.stringify({
  ...
  snapshot: "scripts/i18n-snapshots/r073n-missing.json",
